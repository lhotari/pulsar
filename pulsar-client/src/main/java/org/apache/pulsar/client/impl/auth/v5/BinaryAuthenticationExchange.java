 ...
 *
 * <p>One exchange owns one {@link AuthenticationCallContext} (and its state slot) for the lifetime of a
 * single connection attempt, so multi-round conversation state survives across challenge rounds. The
 * exchange is <em>not</em> thread-safe and is not shared across connections; {@code ClientCnx} serializes
 * its rounds (it issues the next round only after the previous future completes).
 *
 ...
 *       {@link BinaryAuthDataProvider#getAuthDataAsync} on the same call context.</li>
