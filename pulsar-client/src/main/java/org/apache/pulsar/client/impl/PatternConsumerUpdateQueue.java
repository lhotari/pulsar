...
public class PatternConsumerUpdateQueue {
    ...
    synchronized void triggerNextTask() {
        ...
        switch (task.type) {
            ...
            case TOPICS_CHANGED: {
                ...
                newTaskFuture = topicsChangeListener.onTopicsAdded(topicsAddedOrRemovedTask.addedTopics)
                        .thenCompose(__ ->
                                topicsChangeListener.onTopicsRemoved(topicsAddedOrRemovedTask.removedTopics))
                        .thenRun(() -> {
                            String localHash = getTopicsHash();
    ...
    public String getTopicsHash() {
        return TopicList.calculateHash(patternConsumer.getPartitions());
    }
