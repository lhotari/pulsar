...
/**
 * v5-native Athenz authentication (PIP-478). A single-pass credential served over both transports:
 * the binary protocol sends the current ZTS role token as {@code auth_data}, and HTTP sends the role
 * token under the configured Athenz role header. The built-in v4 {@code AuthenticationAthenz} is a
 * thin shim over this implementation; the ZTS role-token acquisition and the 90-minute role-token
 * cache remain on the v4 shim, which supplies the current role token and the HTTP role header to this
 * body via the role-token supplier.
 *
 * <p>Role-token acquisition performs network I/O. As with the existing v4 Athenz code, the supplier
 * may block on the ZTS client; the async-offload happens at the {@code ClientCnx}/adapter layer, so
 ...
 * this concrete built-in body is serializable so the v4 {@code AuthenticationAthenz} shim (whose
 * interface requires {@code Serializable}, for Functions/connector frameworks) round-trips. The
 * role-token supplier is itself serializable.
 */
public class AthenzAuthenticationV5 implements SinglePassAuthentication, Serializable {
    ...
    public CompletableFuture<BinaryProtocolAuthData> getAuthDataAsync(AuthenticationCallContext ctx) {
        return CompletableFuture.completedFuture(new DefaultBinaryProtocolAuthData(AUTH_METHOD_NAME,
                roleTokenProvider.roleToken().getBytes(StandardCharsets.UTF_8)));
    }
