 ...
 *
 * <p>It also implements {@link AsyncAuthenticationDriver} so that {@code ClientCnx} authenticates via
 * the asynchronous (non event-loop-blocking) path. The synchronous v4 {@link #getAuthData(String)}
 * path is preserved for callers that do not use the async driver, by blocking on the async methods.
 *
...
public class V5ToV4AuthenticationAdapter
     ...
     * @param scheduler the scheduler the plugin should offload blocking work to
     * @param httpClientFactory the HTTP client factory (may be {@code null} for now)
    ...
    public V5ToV4AuthenticationAdapter(Authentication v5, ScheduledExecutorService scheduler,
            PulsarHttpClientFactory httpClientFactory, String clientInstanceId,
    ...
    public String getAuthMethodName() {
        return v5.capability(BinaryProtocolAuthDataProvider.class)
                .map(BinaryProtocolAuthDataProvider::authMethodName)
                .orElse("none");
    }
    ...
    public void start() throws PulsarClientException {
        AuthenticationInitContext initContext = new SimpleAuthInitContext(httpClientFactory, scheduler, clock,
                openTelemetry, clientInstanceId, params);
        try {
    ...
    public CompletableFuture<AuthData> getAuthDataAsync(String brokerHostName) {
        Optional<BinaryProtocolAuthDataProvider> provider =
                v5.capability(BinaryProtocolAuthDataProvider.class);
        if (provider.isEmpty()) {
    ...
    public CompletableFuture<AuthData> authenticateAsync(AuthData challenge, String brokerHostName) {
        Optional<ChallengeResponseHandler> handler = v5.capability(ChallengeResponseHandler.class);
        if (handler.isEmpty() || isSentinel(challenge)) {
            return getAuthDataAsync(brokerHostName);
        }
        AuthenticationCallContext callContext = new SimpleAuthCallContext(brokerHostName, 0);
        return handler.get()
                .respondToChallengeAsync(callContext, AuthChallenge.of(challenge.getBytes()))
    ...
    /**
     * v5 {@link Authentication} plugins are not serializable. Serialization of this adapter is
     * therefore unsupported. Configure authentication on a remote/forked context via the
     ...
     *
     * <p>This bridge only carries the binary-protocol command credential. HTTP auth headers are
     * served through the v5 {@link org.apache.pulsar.client.api.v5.auth.HttpAuthHeadersProvider}
     * capability via the dedicated HTTP auth path, not through this binary-protocol bridge.
     */
