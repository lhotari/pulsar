...
function ci_build_cpp_and_python_clients() {
  (
  set -xe
  ...
  $PWD/pulsar-client-cpp/docker-build.sh
  )
}

function ci_project_version() {
...
function ci_restore_server_distribution() {
  local project_version=$(ci_project_version)
  local destination_dir=distribution/server/target
  local destination_file="${destination_dir}/apache-pulsar-${project_version}-bin.tar.gz"
  if [ ! -f "${destination_file}" ]; then
    local m2_distribution_file="$HOME/.m2/repository/org/apache/pulsar/pulsar-server-distribution/${project_version}/pulsar-server-distribution-${project_version}-bin.tar.gz"
    if [ -f "${m2_distribution_file}" ]; then
      mkdir -p "${destination_dir}"
      cp -v "${m2_distribution_file}" "${destination_file}"
      return 0
    else
      echo "Cannot find ${m2_distribution_file}"
      return 1
    fi
  else
    return 0
...
function ci_run_cpp_and_python_tests() {
  (
  set -xe
  # python tests fail if pulsar-functions/instance/target/python-instance doesn't exist
  if [ ! -d pulsar-functions/instance/target/python-instance ]; then
    ci_mvn -pl pulsar-functions/instance compile -DskipTests
  fi
  ci_retry $PWD/pulsar-client-cpp/docker-tests.sh
  )
}
...
function ci_check_license_headers() {
  (
    ci_mvn -T 8 initialize apache-rat:check license:check
  )
}
