...
mvn_run_integration_test() {
  ...
  if [[ "$1" == "--coverage" ]]; then
      ...
      coverage_arg="-Pcoverage -Dintegrationtest.coverage.enabled=true -Dintegrationtest.coverage.dir=/tmp/jacocoDir"
  ...
  if [[ $build_only -ne 1 ]]; then
    ...
    mvn -B -ntp -pl "$modules" $coverage_arg -DskipDocker -DskipSourceReleaseAssembly=true -Dspotbugs.skip=true -Dlicense.skip=true -Dcheckstyle.skip=true -Drat.skip=true -DredirectTestOutputToFile=false $clean_arg verify "$@"
    echo "::endgroup::"
    set +x
