...
PR, comment **`/pulsarbot rerun`** on the PR to re-run the failed workflows (or
**`/pulsarbot rerun-failure-checks`** to re-run only the failed jobs). It only takes effect once
**all** jobs from the previous run have completed, so wait for the run to finish (including any
still-running or queued jobs) before commenting.
