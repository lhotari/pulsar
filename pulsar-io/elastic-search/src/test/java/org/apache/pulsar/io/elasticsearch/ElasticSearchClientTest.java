...
@Slf4j
public abstract class ElasticSearchClientTest extends ElasticSearchTestBase {
    public static final String INDEX = "myindex";

    private ElasticsearchContainer container;
    private Network network;

    public ElasticSearchClientTest(String elasticImageName) {
    ...
    public void testClientInstance() throws Exception {
        try (ElasticSearchClient client = new ElasticSearchClient(new ElasticSearchConfig()
            ...
            if (elasticImageName.equals(OPENSEARCH) || elasticImageName.equals(ELASTICSEARCH_7)) {
                ...
                OpenSearchHighLevelRestClient osRestHighLevelClient =
                ...
                org.elasticsearch.client.RestClient restClient =
                        (org.elasticsearch.client.RestClient) restClientFiled.get(transport);
                assertNotNull(restClient);

                Field compressionEnabledFiled =
                        org.elasticsearch.client.RestClient.class.getDeclaredField("compressionEnabled");
                compressionEnabledFiled.setAccessible(true);
                boolean compressionEnabled = (boolean) compressionEnabledFiled.get(restClient);
                assertTrue(compressionEnabled);
            }
    ...
    public void testMalformedDocFails() throws Exception {
        ...
        try (ElasticSearchClient client = new ElasticSearchClient(config, sinkContext);) {
            ...
            assertThrows(Exception.class, () -> client.bulkIndex(mockRecord, Pair.of("3",
                    "{\"a\":3}")));
            assertEquals(mockRecord.getAcked(), 1);
