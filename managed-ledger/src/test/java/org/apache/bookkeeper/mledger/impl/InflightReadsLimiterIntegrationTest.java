...
public class InflightReadsLimiterIntegrationTest extends MockedBookKeeperTestCase {
    ...
    public void testPreciseLimitation(String missingCase) throws Exception {
        ...
        entryCache.asyncReadEntry(spyCurrentLedger, 125, 125, __ -> true, cb0, ctx);
        cb0.entries.join();
        ...
        threadFactory.newThread(() -> {
            entryCache.asyncReadEntry(spyCurrentLedger, start1, end1, __ -> true, cb1, ctx);
        }).start();
        threadFactory.newThread(() -> {
            try {
            ...
            entryCache.asyncReadEntry(spyCurrentLedger, start2, end2, __ -> true, cb2, ctx);
        }).start();
