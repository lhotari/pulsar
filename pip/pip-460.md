...
Since the bucket granulary is fixed, it's possible that multiple consumers will receive the same bucketed entry. Clients should ignore messages that contain keys that aren't part of their key assignment. The acknowledgement model on the broker side must also be able to handle this case.
