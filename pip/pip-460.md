...
- This PIP defines the overall design direction. High level architecture and detailed design for each sub-problem is covered by dedicated sub-PIPs (see [Sub-PIPs](#sub-pips)).
...
>
> PIP-460 is intentionally a vision and problem statement document. Its purpose is to establish the motivation, goals, and key challenges that need to be resolved, and to seek community alignment on the direction before detailed design work begins.
...
| **Split**                         | An operation that seals a range segment and creates two or more child segments, each covering a sub-portion of the parent's keyspace. A local operation on the owning broker. |
...
| **QueueConsumer**                 | A consumer type that subscribes to all active segments with shared dispatch and no controller interaction. No cross-segment ordering guarantees. |
...
One proposed solution is a **fixed-width bucket scheme** embedded in the message entry format. The full key hash space is divided into a fixed number of buckets (configurable at broker, namespace, and topic level). Range segment split boundaries are always aligned to bucket boundaries. Within a batch entry, messages are grouped by bucket, and an extensible entry header records the bucket layout so that a broker or consumer can route or filter by bucket without deserializing the full payload. This makes it possible to split a batch entry and route individual buckets to the consumers they are assigned to. The entry format supports a non-bucketed mode for use cases where total message ordering is required. The new entry format also affects the `MessageId` representation and must support selective decompression and decryption per bucket.

The same bucket-based header is required for geo-replication: when a batch arrives at a remote cluster with a different segment topology, the receiving broker uses the header to route the entry to the correct destination segment without decompressing or decrypting the payload.

Since the bucket granulary is fixed, it's possible that multiple consumers will receive the same bucketed entry. Clients should ignore messages that contains keys that aren't part of their key assignment. The acknowledgement model on the broker side must also be able to handle this case.
One of the goals of this design is also to be able to send only the buckets that are relevant to the consumer. 

Because existing clients are not compatible with scalable topics, the message entry format can be changed without backward compatibility constraints. The detailed wire format, encoding, compression/encryption approach, and bucket count defaults will be addressed in a dedicated sub-PIP.
