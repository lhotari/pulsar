...
`PulsarClientImpl:904`). A V5 client with the memory limit turned off has no bound of any kind between the application and the wire.
...
**Why it can promise never to refuse: one reserved slot per producer.** A budget cannot be enforced by a call
that always accepts unless the accepting call is also the one that closes the gate. The producer therefore
reserves capacity for exactly one message — one pending-message position and one maximum-sized payload's worth
of accounted bytes — outside the capacity available to ordinary admission. Ordinary sends draw on the rest.
When a submission finds the ordinary budget exhausted, it takes the reserved slot, and the same atomic step
that installs the message closes the gate and populates `whenSendable()`. The slot is funded when the producer
is created, not borrowed on demand, so failure to reserve it is a configuration error discovered at creation
rather than a surprise at the first message.

That bounds the overshoot exactly: **at most one message per producer beyond the ordinary budget**, plus the
byte reservation. It is the same shape as a Netty write-buffer high watermark, where a write past the mark is
still queued and writability goes false — with one deliberate departure. Netty and Vert.x let a writer keep
writing indefinitely after the mark; here the second `submitAsync()` while a `whenSendable()` is outstanding
throws `SendNotReadyException` **synchronously, before the builder is consumed or the payload serialised**.
Ignoring the signal is a programming error, and hiding it in the returned future would recreate the
late-discovery problem this API exists to remove.

The exception is only reachable two ways. The first is ignoring `whenSendable()`. The second is calling
`submitAsync()` from several threads on one producer — which is already meaningless, since an ordered stream
has one sending loop by definition. Applications that do not need order have `trySendAsync()` and the
unchanged `sendAsync()`.

**`Producer.whenSendable()` survives, but only for one job.** The reviewer's instinct — that it is subsumed by
the submission — is right for the post-send case, which is exactly where a producer-level advisory is
dangerous, because between its completion and your send another producer can take the capacity. What it is
still needed for is the *pre-pull* case: a closed-loop application should be able to learn that it can publish
...
| Blocking | `send()`, or staged `send()` + `collect()` for batching | Waits, bounded by `maxSendBlockTime` |
...
```java
// Ordered: pull only when there is room, and stop when told to.
void pump() {
    producer.whenSendable().thenComposeAsync(ignored -> {
        var record = source.next();                       // acquired only once there is room
        var submission = producer.newMessage().value(record).submitAsync();
        submission.messageId().exceptionally(this::onSendFailed);
        source.advance();                                 // accepted, so the source may advance
        return submission.whenSendable().orElse(COMPLETED);
    }, appExecutor).thenRun(this::pump);
}
```

The second example is the whole argument in eight lines: `source.advance()` is unconditional because
`submitAsync()` cannot have refused, so there is no state in which the application holds a record the client
has rejected and must decide where to put it.
...
So a single-threaded blocking publisher — which is what an ordered publisher is — produces **one BookKeeper
entry per message and pays a broker round trip for each**, deterministically. It is round-trip-bound, not
clock-bound. Two qualifications keep this honest: with M concurrent blocking senders, batches of up to M do
...
public interface Producer<T> extends Closeable {
    /**
     * Stage a message for batched transmission and return once it has been accepted, without
     * waiting for the broker. Blocks while admission is unavailable, bounded by
     * maxSendBlockTime. Throws if maxStagedMessages outcomes are already awaiting collection.
     */
    default long stage(TypedMessageBuilder<T> message) throws PulsarClientException;
...
public interface SendOutcome {
    long index();                      // the value returned by stage(), so the caller can map back to its source
...
- **Outstanding sends** are bounded by admission. `stage()` **blocks** here, because acknowledgements arrive on
  the client's IO threads whether or not the caller ever calls `collect()` — waiting makes progress.
- **Uncollected outcomes** are bounded by `maxStagedMessages`. `stage()` **throws** here, because the only
  thing that can drain that buffer is the caller's own `collect()`. Blocking would deadlock the one thread
  able to release it.
...
returned list, so the application can see exactly where the break is and what happened after it. The list must
...
**Allocation: retained bytes are capped per producer, with borrowing.** The client's memory budget — direct
and heap alike — is divided by a demand-aware max-min rule rather than by usage share. Every account competing
for capacity gets an equal share of the budget less a reserve; unused shares are lent to accounts that have
demand; and a lent share is reclaimed as the borrower's own sends complete. Two properties are what the
...
neighbours were idle. And the client can enforce a new ceiling immediately, because unlike the broker it has
exact local knowledge of every charge and release and no network delay to wait out.
...
- **Shrinking a ceiling never lowers usage.** When an account's ceiling falls below what it currently holds,
  ...
  retained.
- **Accounts follow the topic, not the producer object.** Partition producers and V5 segment producers inherit
  their logical topic's account instead of each opening a top-level share, or a client would enlarge its own
  entitlement by partitioning. A producer with no retained bytes and no pending demand holds no reservation,
  but keeps a lightweight membership for a cooldown interval so that short bursts do not thrash the shares.

An oversized message is the case a strict fit test cannot serve: a message larger than its account's ceiling
would never be admissible. It draws on the reserved slot, which is sized for one maximum-sized message
precisely so that this case terminates.
...
**Three clocks, where there was one.** `sendTimeoutMs` (default 30 s) conflates waiting for client capacity
with waiting for the broker, and its only implementable shape is a whole-window cliff:
`ProducerImpl.run(Timeout)` measures from the head message's `createdAt` and calls `failPendingMessages`
(`ProducerImpl:2316`, `:2380`), failing every in-flight message including one accepted milliseconds ago.

Two things make one number untenable once admission is dynamic, and it is worth being precise about which,
because an earlier draft argued this from the wrong one. It is **not** that a budget cut leaves accepted
messages draining slowly: admission is decided at acceptance and nothing re-gates a message afterwards, so
accepted work transmits as fast as the connection allows. What is true is that the clock starts before
transmission — `createdAt` is set when the operation is created, and for a batching producer the timer falls
back to the container's first-added timestamp — so a message can spend its budget **waiting to be sent**. The
proposal makes that worse deliberately, because adaptive batching widens the accretion window precisely when
the producer is budget-limited. A deadline that runs from acceptance is therefore measuring two different
waits added together, and it responds to the sum by failing the whole window. So the deadline is separated by
what it actually measures:

| Clock | Expiry means | Owned by |
|---|---|---|
| `maxSendBlockTime` (blocking paths), or the caller's own `orTimeout` on `whenSendable()` (asynchronous) | Permission did not arrive in time; **nothing was published and nothing is pending**, so the application may sample, drop or defer with no partial state | the caller |
...
application already owns and can bound with `orTimeout` on its own terms. That is a simplification worth
noting: the deadline that mattered most is the one the application can now express itself.
...
**`submitAsync()` — Promise:** the message is accepted. It is not refused for lack of capacity, under any
budget, so it cannot leave a gap that a later message fills — which is the whole reason the method exists. The
returned `SendSubmission` always carries the acknowledgement future, and carries a present `whenSendable()`
exactly when the producer will not accept another message until it completes. Whether it is present is fixed
when the submission is returned.
*Testable by:* publishing a numbered sequence through `submitAsync()` under a budget repeatedly cut to zero
and restored, then asserting from the topic that the sequence is complete and in order.
**Not promised:** that the message will be persisted — acceptance is not delivery, and `messageId()` can still
fail on a timeout, a fenced producer or a broker error. Nor that a second `submitAsync()` while a
`whenSendable()` is outstanding will be tolerated: that throws `SendNotReadyException` synchronously, because
ignoring the signal is a defect and the reserved slot that funds the promise has already been spent.

**`Producer.whenSendable()` — Promise:** the returned future completes when this producer has ordinary
admission capacity, so a caller can decide whether to acquire the next message from its source. Concurrent
callers receive the same future, and it completes at most once.
...
- `Producer.whenSendable()` returning `CompletableFuture<Void>` — advisory, for deciding whether to acquire
  the next message.
- `Producer.stage(TypedMessageBuilder<T>)` returning a `long` index, and `Producer.collect()` returning
  `List<SendOutcome>`; the new types `SendOutcome` and `PublicationStatus`.
...
handles them differently. Reconnecting to a broker that does not negotiate the feature means **no
broker-derived budget at all**, which the client treats as unbounded. Updates that stop arriving on a
connection that *did* negotiate it are a failure, not a permission, and follow the expiry rules — the burst
allowances lapse and the rate decays toward `adaptivePublishMinAdmissionFactor × reference`.
