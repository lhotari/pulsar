...
- Completed-operation latency stops producing samples exactly when storage stalls, so an
  ...
  It is a **stall detector**, with its own trigger and its own action. It fires when
...
- **The within-period statistic is the minimum, and that choice is the algorithm rather than a detail.** CoDel
  ...
  in the period waited longer than the target, the queue never drained — and a standing queue is the only thing
  the *delay* term should reduce a budget for. So `z_delay` is the *minimum* pre-initiation queueing delay
  observed over the control period, divided by the target. The occupancy terms remain independent grounds for
  a reduction, as the recurrence says; what the minimum decides is when *delay* has become one.

  Two properties follow, and they are conditional rather than absolute. A transient disturbance is invisible to the minimum **when the queue drains and further
