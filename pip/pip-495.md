# PIP-495: Produce backpressure: capacity grants and per-producer pacing
...
`setAutoRead(false)` transmits no information: the client sees latency and cannot distinguish throttling from a slow network, a slow bookie, or a GC pause. Neither does the pacing proposed here; only an explicit signal would (*Alternatives*). What can be fixed without one is who else the decision lands on, and whether the application sees the pressure as capacity instead of as latency.
...
public final class CapacityRequest {
    ...
    public interface Builder {
        ...
        Builder maxPayloadBytes(long max);  // required — see "What backs a grant"
...
**What backs a grant.** `payloadBytes()` is reserved from `ClientBuilder.memoryLimit` — the budget that actually enforces on stock defaults — so `maxPayloadBytes` is **required**, not defaulted to unbounded. A grant that reserved nothing would exempt its sends from the one live budget, which is the opposite of what it is for. `messages()` is reserved from the producer's `maxPendingMessages` semaphore where one exists, and `ProducerImpl:216-220` creates it only when `maxPendingMessages > 0`; on stock defaults there is none, so there the message dimension caps how many sends the grant admits without standing behind a second budget. `pendingSendHeapLimit` backs it once enabled. This is what makes "a granted send debits its grant and does not re-enter admission" safe: the reservation came from the same budgets admission would have charged.
...
- **`releaseUnused()`, not `AutoCloseable`.** `AutoCloseable` invites `try (grant) { …sendAsync(); }`, where the block exits long before the sends complete and a reader takes the grant to be finished with. A named method says what it does, and it is **idempotent** because an unspecified release reproduces [#26469](https://github.com/apache/pulsar/issues/26469).
...
**Where broker-side accounting completes.** A paced response is queued *after* its publish completes, not instead of completing it. `completedSendOperation` (`ServerCnx:4483-4489`) decrements the connection's pending-request count **and** the IO-thread-wide pending-byte total. Deferring those would let rate pacing trip `IOThreadMaxPendingPublishBytesExceeded` — which pauses every connection on that IO thread, the widest blast radius in the system and the one this proposal exists to avoid. Hence the requirement: **pacing must not extend the publish-accounting or payload-ownership lifetime.** Publish accounting completes as it does today, and what the pacing queue holds is detached response data — never a publish context, a payload reference, or anything whose release the managed ledger still owns. (`OpAddEntry.completeAdd` invokes the callback before releasing the entry buffer, so a queue that captured the context could outlive the buffer's intended lifetime.) Failed and non-persistent publishes complete on their own paths and are queued the same way.

That has a consequence which must be stated rather than assumed: **the pacing queues are outside every existing pending-publish limit.** They hold responses, not payloads, but a producer whose client does not slow down keeps filling one, and nothing already in the broker will stop it. Three bounds. The response-count and byte bounds are checked on every enqueue; the age bound cannot be, because in the state pacing is designed to produce the client has stopped sending and no enqueue arrives to trigger it. It rides the limiter's existing unthrottling tick (`scheduleUnthrottling`), which already runs per throttling period, rather than needing a timer of its own.

| Bound | Unit | Default | Crossing it |
|---|---|---|---|
| `producerResponsePacingMaxAgeMillis` | ms, oldest held response | 5000 | flush that producer, then pause its connection |
| `producerResponsePacingMaxQueuedResponses` | responses, per producer | 1000 | the same |
| `producerResponsePacingMaxTotalQueuedBytes` | bytes, broker-wide | 64 MiB | the same, for the producer whose reservation failed |

**What a queued response costs is known per entry, not estimated.** A queued entry is a small detached record — for a receipt, the five identifiers `newSendReceiptCommand` takes (`Commands:448`); for an error, its code and message — charged at its own known size, on the order of tens of bytes. It is deliberately **not** a pre-serialized frame: `Commands.serializeWithSize` (`Commands:2005`) allocates from `PulsarByteBufAllocator`, and this queue must not draw on the direct pool that the broker's existing publish-buffer limits are there to guard. So 64 MiB of heap is on the order of a million held responses; against a per-producer bound of 1000, roughly a thousand fully-paced producers before the broker-wide budget binds. That is the quantity an operator is really choosing. The charge is a per-entry constant plus, for an error, its message string — an implementation must include the queue node in it, and the constant is what makes this bound settable where the client-side per-send retention (*Layer 1b*), whose cost depends on application metadata and callbacks, is not. The cost of holding a record rather than a frame is that serialization moves to flush time, so an escalation serializes a producer's whole queue on the IO thread in one burst.

**Why the broker-wide bound is not optional.** The condition this proposal exists for is fleet-wide: thousands of producers paced at once would hold thousands of separately-bounded queues with nothing capping the sum. Every enqueue takes a reservation against the budget, including responses from publishes already in flight when it filled — those complete whether or not reads are paused, so "stop pacing new decisions" would leave them with nowhere to go, and letting one bypass a non-empty queue would break the ordering invariant above. A failed reservation therefore puts its producer into the same flush-then-pause state, covering the response that failed and everything completing behind it. Releasing queue bytes does not by itself clear a throttle hold. `pulsar.broker.producer.response.pacing.queued.bytes` reports the total, so the budget is watched rather than guessed at.

**What the age bound can actually bound.** `producerResponsePacingMaxAgeMillis` bounds **how long a producer's oldest response is deliberately held**, not the per-response increment — and deliberate holding is the most it can bound, since `sendSendReceiptResponse` (`PulsarCommandSenderImpl:113`) writes through `writeAndFlushWithVoidPromise` (`:427`), so the broker never learns when a client received one. The increment is the wrong quantity to bound in any case: at saturation head-of-queue age is `window / rate` regardless of it, so a 50 ms increment with 1000 in flight is 50 seconds of head age.

**Escalation is a state, not an event.** Beyond any of the three bounds the broker **flushes that producer's pacing queue and then pauses the connection**, and stays in that state until the limiter says otherwise. A drained queue is deliberately *not* a resumption condition: a producer could otherwise fill the queue, be flushed, regain client capacity and refill it immediately, turning sustained overload into a burst/pause cycle that repays no debt.

Resumption waits for the limiter's own release instead (*What holds a response*, above), so accumulated token debt is repaid first, and it remains subject to any other throttle reason holding the same connection. Pacing is re-entered only after resumption.

The honest consequence: under overload sustained beyond what pacing can absorb, the steady state is today's behaviour, a paused connection. Pacing buys the interval before that. Flushing first is what makes the bound mean anything: pausing reads stops new sends arriving but does not deliver responses already held, so a pause on its own would let the very age the bound exists to cap go on growing. Delivering them completes that acknowledged work and releases the capacity it held on the client; pausing reads is what keeps the producer throttled. It does not reset the client's send timer, which `ProducerImpl.run(Timeout)` measures from the remaining head's original `createdAt` — flushing removes the messages it acknowledges from that queue, nothing more.
...
- **Chunked messages are paced per chunk, not as a unit.** The tempting rule — hold every chunk's response until the last is due — was rejected twice over: the broker can see *that* a send is a chunk — `CommandSend.is_chunk` (`PulsarApi.proto:555`) reaches the publish path — but not where it sits in its run, since `chunk_id` and `num_chunks_from_msg` live in `MessageMetadata`, which the publish path parses only where encryption is required or deduplication is enabled — and neither reads the chunk fields — and the premise is wrong anyway, because `ackReceived` releases capacity per chunk (`releaseSemaphoreForSendOp`) even though the application's callback fires once on the last. What is true, and is enough, is that a chunked message completes no sooner than its slowest-paced chunk.
...
`connectionsPerBroker` is client-wide: raising it spreads everything across more connections and cannot isolate one producer. A per-producer isolation API ([#23403](https://github.com/apache/pulsar/issues/23403)) would address the residue and is independently shippable, but is not specified here: the client-side residue cannot be sized until Layer 2 ships, and specifying isolation first would make it the workaround people build on. Nor can isolation deliver what it appears to: `IOThreadMaxPendingPublishBytesExceeded` throttles every connection sharing an IO thread (`ServerCnx:305-312`), and a paced producer still occupies the client-wide memory budget. A dedicated socket isolates a socket, not client memory, an IO thread, or storage.
...
- `Producer` and `TypedMessageBuilder` are `@InterfaceStability.Stable` extensibility surfaces, so every added method ships as a `default`. `CODING.md:207-211` asks first for a default that delegates to an existing method, and failing that for a capability-query method. Neither fits: no existing method yields capacity, and a per-method `supportsX()` would multiply across the surface. So the default throws `UnsupportedOperationException`. Third-party implementations keep compiling, and a caller reaching `capacity()` on one gets a clear failure rather than a silently wrong answer.
...
- `producerResponsePacingMaxTotalQueuedBytes` — broker-wide bound on the **heap** held by all pacing queues together. **Default 64 MiB.** A failed reservation puts its producer into the flush-then-pause state, as the other two bounds do (*Layer 2*). Without this the per-producer bounds multiply by the producer count, in exactly the fleet-wide overload this proposal addresses.
...
- `pulsar.broker.producer.response.pacing.queued.bytes` (gauge) — total heap held by all pacing queues, against `producerResponsePacingMaxTotalQueuedBytes`. The saturation signal for the one budget with no other backstop; when it reaches the bound, producers are being escalated to connection pauses as their reservations fail.
...
**A blocking permit acquire.** That is `blockIfQueueFull(true)`, rejected under *The client offers no third option* (#26344).
