...
*Motivation* established what this section implements: a client that never refuses a message and then accepts
its successor cannot open a capacity gap, and an ordered application therefore needs no protocol for repairing
one. The surface splits in two by what the application does when there is no capacity — not because a refusing
...
- **Growth.** The reference is a high-water mark and it rises **multiplicatively**, not by adding the admission
  ...
  rises when `f = 1`, `z_delay` is below the headroom factor, and the scope's admitted rate in **that
  dimension** has reached the rate that was actually **allocatable** — the reference less the newcomer reserve
  and the committed fallbacks — rather than the raw reference. Testing against the raw reference would be a
  trap: allocations are issued from the reduced figure, so with no newcomers arriving and no grants expiring,
  steadily paced incumbents could never consume the withheld remainder and the estimator would sit at `f = 1`
  discovering nothing. Demand at the ceiling with no queue is the only evidence that more
...
- **Expiry.** When `valid_for_ms` elapses with no newer update, the burst allowances lapse at once. A producer
  ...
  given a formula whose inputs it cannot see. **The broker funds it from the scope's *rate* budget — the same place the newcomer
  reserve comes from, not the liability pool the bursts draw on — and not as a multiple of the producer's
  current allocation** — a producer cut to zero would otherwise
