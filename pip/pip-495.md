...
A Pulsar broker knows when it is struggling and has no way to say so. It has exactly two instruments for
regulating how fast a producer may publish — a rate an operator configured before the traffic arrived, and
`setAutoRead(false)`, which stops it reading from a socket and tells the client nothing. This proposal closes the loop: the broker measures its own queueing delay,
...
*What the broker measures* is the controller. *What should be voted on, and in what order*, at the end of
*Goals*, is the part to read first if your question is whether this is one proposal or seven — it is seven,
and it says which.
...
3. **Broker → storage.** For a persistent topic, the broker writes entries to BookKeeper and holds them until
   the write completes. A non-persistent topic acknowledges without storing, so this boundary does not exist
   for it and only the first two apply.
...
A Pulsar broker has two instruments for regulating the *rate* at which it is published to: a rate an operator
configured before the traffic arrived, and `setAutoRead(false)`, which stops the broker reading from a TCP
connection. Others exist for other purposes — a backlog quota can disconnect a producer outright, where the
slowest cursor is not a system cursor (`BacklogQuotaManager:133-135`) — but none of them tells a producer how fast it may go, which is the gap this
proposal is about. The first cannot change when conditions do. The second carries no information — a client cannot tell it from a
...
replay. It is recoverable in the sense that a protocol for recovering exists — but the client does not supply
that protocol, and the boundary is weaker than it looks: a timeout neither closes nor fences the producer, and
`failPendingMessages` deliberately preserves operations enqueued by callbacks that run during the failure
(`ProducerImpl:2384`). So the application still has to stop its own successors and coordinate the replay; what
it is given is a defined set to replay, not a barrier that holds while it does so.

One error is worth naming because it looks like a whole-window failure and is not. A broker
`PersistenceError` takes `ClientCnx.handleSendError`'s default branch, which drops the affected producer's
binding to its connection and schedules a reconnect, after which the producer **retransmits its pending
queue** — the application's sends are not failed at all. Retransmission, not failure, is the ordinary
response to a persistence problem, which is why deduplication exists.

The branch deliberately does **not** close the shared connection, and the distinction matters later. Its
comment is explicit — "don't close this ctx, otherwise it will close all consumers and producers which use
this ctx" (`ClientCnx:1219`) — and it calls `producer.connectionClosed(...)` (`:1220`), which releases the
reference and schedules reconnection (`ConnectionHandler:239-255`); the channel itself is closed only where
pooling is disabled (`ConnectionPool:469-475`). So the blast radius is one producer, and the "new connection"
the replay rules below speak of is a new producer *registration*, which may well arrive on the same socket.
...
That gives the rule the client API is designed around. **A gap the application can be prevented from creating
is worth more than any protocol for repairing one**, and admission is the one gap the client can simply decline
to open, because unlike a schema mismatch it has no per-message cause: capacity returns. Blocking `send()` with
`blockIfQueueFull(true)` is correct today for exactly that reason, not because parking is desirable.

**This is an argument about defaults, not about impossibility, and it is worth being precise about which.**
A refusing API *can* be used correctly by an ordered publisher: retain the refused record, admit no successor,
retry it, and only then advance. That discipline works, and the earlier part of this section says so. What it
costs is that the application must hold a message the client would not take, represent that state across
whatever asynchronous machinery it uses, and get a rule right that nothing in the API states or enforces —
while the natural shape, the one every example and every tutorial reaches for, is silently wrong.

So the case for two contracts is that an API which *may* refuse fits a source whose answer to refusal is to
**drop**, where the gap is deliberate and the application knows which records it gave up; and an API which
**accepts and separately says stop** makes the ordered case correct by construction, moves the one-message
buffer inside the client where it can be bounded and accounted, and removes a state the application would
otherwise have to invent. That is a strong argument for the design. It is not a proof that no other design
could work, and the document does not need one.
...
- **An application-visible admission signal that does not break ordering.** A way for an application to learn
  ...
  a refusing API can be used correctly by an ordered publisher and
  almost never is, which is the argument for two contracts rather than one. Pulsar's own geo-replicator has
  hand-built the first of them
...
- **Tenant entitlement.** Three things travel under the word "fairness" here and they are delivered to
  different degrees. *Proportionality* — every producer's share falls by roughly the same fraction — is
  delivered. *Isolation* is delivered at the granularities *Allocation* names and no finer: between
  logical-topic accounts for retained memory, and between producers for their turn at admission; not between
  partitions of one topic. *Entitlement* — a tenant is due some share regardless of what it has been using —
  is **not** delivered, because it requires choosing an allocation identity, and that is a behaviour contract
  the project cannot change once it ships.
- **Replacing the configured rate limiters.** `PublishRateLimiterImpl` and `ResourceGroupPublishLimiter` remain as configured ceilings; the adaptive budget is a second constraint below them **in the byte dimension, where the two are the same currency** — *Reference* caps growth against a configured byte rate. A configured *message* rate is not commensurable with the entry budget and can still bind first, which is a residue this proposal names rather than closes (*General Notes*).
...
- **An ordered-publishing guarantee for clients with very many producers.** `submitAsync()` funds its
  never-refuse promise from a reservation held per *logical* producer — one, whatever its partition count —
  and the number a client can hold is still bounded by the minimum over its direct, heap and pending-position
  budgets. A proxy, a Functions runtime or a connector writing to a topic per tenant is fanning out rather
  than publishing one ordered stream, and is served by `trySendAsync()` and `sendAsync()`, which reserve
  nothing. Making the guarantee available at that scale needs a different funding model and is not attempted
  here.
...
3. **Client-local admission.** The API — `submitAsync()`, `trySendAsync()`, the reservation, the allocator, the
   ...
   be this.
4. **`stage()`/`collect()`.** Independent of the wire protocol, the controller and the never-refuse guarantee,
   and arguably its own PIP. It is **not** independent of stage 3, and an earlier draft said it was: `stage()`
   is specified to fail on a producer with no finite `maxSendBlockTime`, and that deadline is stage 3's. The
   dependency is exactly one setting rather than the whole of stage 3, so a separate PIP would have to carry
   `maxSendBlockTime` with it if stage 3 did not land first — which is the honest statement of the coupling.
...
**Five things in this document are claims about behaviour under load and none of them is settled by prose.**
A reviewer is entitled to ask for each before the stage that depends on it, and the document names them rather
than waiting to be asked:

1. **Does the controller act before the occupancy guards do?** *What the broker measures* admits this is
   uncomfortable at the defaults — a free 64 MiB buffer fills in about 62 ms at 1 GiB/s, less than one
   observation. The experiment is a step from idle to line rate, measuring which mechanism engages first.
2. **How large is the adaptive-batching gain, really?** The entry-cost argument says framing, index and
   completion work amortise; the journal's group commit already amortises the fsync. The experiment is entries
   and bytes at fixed throughput with and without a widened window, measuring bookie CPU and IO rather than
   entry count.
3. **What does the allocator cost at realistic producer counts?** Per-account max-min with an epoch is not free,
   and the target is a client with thousands of producers.
4. **Does the loop degrade gracefully past its gain-delay bound, or oscillate?** The document says the first
   and cannot show it.
5. **Does the reservation hold adversarially?** Not the happy-path ordered-sequence test, but chunking against
   a renegotiated maximum message size, a consumer charging the same controller, a shared controller across
   clients, and reconnect storms.

**And one honest correction to *Upgrade*:** shadow calculation does not validate a closed loop. A controller computing a budget nobody enforces never sees clients react, never sees batching
...
**One organising idea: close the loop.** Today the broker knows it is overloaded and the application never
finds out. Every mechanism above ends at the socket — `setAutoRead(false)` stops the broker reading and says
nothing — so the application learns about congestion as latency, and eventually as a timeout.
...
flowchart LR
    ...
    Meas --> Ctl --> Alloc
    Alloc -- "4 · an absolute rate, valid for a while<br/>on the send receipt, or as a command" --> Cli
...
3. **Allocate.** What remains of the budget after the scope's standing obligations — a newcomer reserve and
   the fallback rates it has committed to — is divided among the producers actually publishing, in proportion
   to what each is already admitted to send.
...
**Budgets aim to act before read throttling is needed; read throttling stays as the last resort.** The
qualification is load-bearing and is argued where the numbers are: against a step change from idle to line
rate the occupancy guards can fill before the controller has taken one observation, so for that shape the
involuntary guard is the primary mechanism rather than the backstop (*The controller*). What the budget
changes is the sustained case. A configured rate
...
client that negotiated a budget and then exceeds it exhausts a bounded allowance and has reads paused on its
connection. A client that never negotiated one is neither given a budget nor charged an allowance; it meets
exactly the limits it meets today. The budget changes what happens *first*, and gives the application
...
*Motivation* established the rule this section implements: the client must not refuse a message for lack of
capacity and then accept its successor. Preventing that *sequence* is what removes the capacity gap, and with
it the ordered application's need to repair one. The surface splits in two by what the application does when there is no capacity — not because a refusing
API could not be used correctly, but because only one of the two shapes is correct by default.
...
The two readiness futures have **one predicate each, and they are different predicates.**

- `SendSubmission.whenSendable()` completes only when the reservation is restored **and** the accumulated
  budget reaches the resume threshold. That threshold is
  `min(burst capacity, max(one entry, rate × sendResumeThreshold))`, and both bounds in it are load-bearing:
  without the floor, a rate low enough that the duration is worth less than one entry waits forever, and
  without the clamp, so does a producer whose configured burst is smaller than the threshold. Pulsar's own
  bucket has the same floor — `calculateThrottlingDuration()` targets `Math.max(1, …)` tokens
  (`AsyncTokenBucket:274-275`) — so "never on the first token" is the rule and one token is the floor beneath
  it. Where the floor binds, the producer resumes one entry at a time, which is what a bucket that cannot
  dispense a fraction of a message can do.
...
- **Therefore the number of admission-capable producers on a client is bounded**, and the bound is a
  *minimum over budgets* rather than a division: with a reservation vector of `v_direct` bytes, `v_heap` bytes
  ...
  `min(⌊D_available / v_direct⌋, ⌊H_available / v_heap⌋, ⌊P_available / v_positions⌋)`. Writing it as `budget / envelope`
  would **overstate** the number of producers a client can support, because the envelope counts payload bytes
  while the funding also includes metadata, batch growth, retained heap and one position per chunk — dividing
  by the smaller number gives a larger and wrong answer. This is a real limit: a client that opens ten thousand
  producers cannot fund ten thousand reservations out of a 64 MiB default budget. It does not need to. `submitAsync()` exists for a producer publishing **one ordered stream**,
...
public interface Producer<T> extends Closeable {
     ...
     *
     * <p>An implementation that does not support admission control returns a future completed
     * exceptionally with UnsupportedOperationException — never throws, since a caller that has
     * attached exceptionally() would never observe a synchronous throw. Ask
     * supportsAdmissionControl() to find out without catching.
     */
...
|---|---|---|
| Ordered, can be slowed | `whenSendable()` to decide whether to pull; `submitAsync()` to publish; await the submission's `whenSendable()` when present | Slows. Capacity never causes a gap; other failures still can |
| Can be dropped | `trySendAsync()`, and on empty apply its overflow strategy | Loses deliberately, by the application's own rule |
| Blocking | `send()` with `blockIfQueueFull(true)`, or `stage()` + `collect()` when batching matters | Waits, bounded by `maxSendBlockTime`, which `stage()` requires |
...
has, with sends still live marked `UNRESOLVED` and retained for the next collection, rather than waiting
forever. `UNRESOLVED` is deliberately not `UNKNOWN`: the first says the client has not found out yet, the
second says it never will.
...
send is admitted only if its **incremental** cost fits all of them — which matters for the entry dimension,
where a message joining an existing batch container costs zero entries and must not be refused merely because
the entry balance is empty. So `trySendAsync()` refuses when a dimension the message actually consumes is
exhausted, `submitAsync()` returns a present `whenSendable()` in the same condition, and that
...
Two rules keep the reservation coherent with the per-account ceilings. Reservations are **separately accounted and non-borrowable**: they are not drawn
...
**A routed producer reserves once at the parent, and lends it.** The reason one reservation suffices is worth
following precisely, because the obvious version of it is wrong. Many ordinary submissions can be outstanding
at once — the pump example pipelines them, each returning an empty `whenSendable()` — so it is *not* true that
only one submission is in flight. What is true is that **at most one of them is reservation-backed**: the
reservation is spent only by the submission that finds the ordinary budget exhausted, and that same submission
closes the gate, so no second can be accepted until the signal completes. One reservation per parent is
therefore exactly enough, lent to whichever underlying producer that submission routes to and returned when it
is accounted for.

Reserving once per *underlying* producer instead would be both wasteful and unfundable: a partitioned producer
would reserve a hundred envelopes to make one guarantee, and a V5 producer routing to a segment created on
demand (`ScalableTopicProducer:374`), or a topic gaining partitions, could need a reservation the client can no
longer fund long after creation succeeded. A new partition or segment therefore needs no new funding, and the number of
...
#### The reservation and the heap budget
...
#### Sharing the client's memory between producers
...
(`pip/pip-322.md:167-169`). Turn-taking under a shared limit is an established pattern here with a shipped
...
Five definitions have to be pinned down or none of this is implementable: what the accounting boundary is, who
holds accounts, what an account is, what demand means, and when allocation runs.

- **The accounting boundary is the controller instance, not the client**, and that is not a detail. A
  `MemoryLimitController` is normally per client (`PulsarClientImpl:353-356`) but can be shared across clients
  through `PulsarClientSharedResources`, so a client-level opt-in has to mean "opt this controller's accounts
  in" and every client sharing it must agree — mixing an opted-in and an opted-out client on one controller is
  a configuration error and is rejected rather than silently resolved.
- **Consumers charge the same controller, hold accounts too — and cannot be admission-gated, which bounds
  what the invariant below can claim.** With `autoScaledReceiverQueueSizeEnabled` a consumer force-reserves its
  incoming buffers against the same counter (`ConsumerBase:1329`, `:1348`), so an allocator that only knows
  about producers would let a consumer consume capacity the design believes is reserved. Consumer queues
  therefore get accounts on the same terms, keyed to their own topic.

  An account is not admission, though, and it would be dishonest to imply otherwise. The consumer charge is
  taken **after** the message is already in the queue — `forceReserveMemory` runs after
  `incomingMessages.offer(...)` (`ConsumerBase:994-1000`) and tests no capacity — and the permits that caused
  the arrival are a message count issued earlier, with no byte estimate attached. Refusing at that point is
  impossible: the bytes are retained, and blocking the receive path would block an IO thread. So a consumer
  account **measures and attributes** consumption; it does not gate it, and a burst of large messages arriving
  against outstanding permits can push the controller past what producer reservations assumed. The invariant
  below therefore holds over producer-side admission, with consumer arrivals as an excursion the allocator
  cannot refuse — and the size of that excursion has to be stated rather than called "bounded", because it is
  finite in the way a cliff is finite. Permits are a message count with no byte term, so the bytes a consumer
  can force in are its outstanding permits times the negotiated maximum message size: at stock defaults
  `receiverQueueSize` 1000 (`ConsumerConfigurationData:134`) against a 5 MiB maximum
  (`Commands.DEFAULT_MAX_MESSAGE_SIZE:129`) is about **5 GiB**, against a 64 MiB default `memoryLimit`. That
  is roughly eighty times the whole budget, and no producer-side rule can prevent it.

  So the honest statement is that the invariant below is a **producer-side** invariant: it governs what the
  allocator admits, and consumer arrivals can carry the controller's total past the budget by up to the
  excursion without any party having misbehaved. That excursion is measured and attributed — it is why
  consumers hold accounts at all — and it is not gated. Subtracting it from the budget instead would be worse
  than useless: at the numbers above the remainder is negative, so no producer reservation could ever be
  funded on a client that both produces and consumes, which is the outcome this bullet exists to avoid.

  The consequence for the contract has to be stated where the contract is: **`submitAsync()`'s never-refuse
  guarantee is conditional on the co-resident consumer population**, not unconditional, on a client that
  shares one controller between producers and an auto-scaling consumer. It is unconditional on a client whose
  controller serves producers only, which is the configuration the guarantee was designed against. Closing the
  gap needs byte-aware consumer permits — consumer-side flow control, out of scope here — and until it exists
  the honest position is a stated condition rather than a promise that quietly depends on nobody consuming.

  A client that enables `adaptiveAdmission` while sharing a controller with a consumer using
  `autoScaledReceiverQueueSizeEnabled` is therefore accepting that excursion; the combination is reported
  through the heap and usage gauges rather than rejected, since rejecting it would make the feature unusable
  on any client that both produces and consumes.
...
- **Demand is an explicit registration, not an inference, and it is a quantity.** An account has demand if it
  holds retained bytes, has a blocking waiter, has an outstanding `whenSendable()`, or was refused by
  `trySendAsync()` within a bounded recent window — that last case recorded deliberately, because a refusing
  call leaves no enqueued send from which demand could otherwise be observed. The amount is the account's
  outstanding request in bytes and positions, not the boolean the controller uses for its admission-limited
  test, and a unit that is reserved is not also counted as demanded.
...
**The ledger invariant, and the three rules that hold it.** Over everything the allocator admits — which is
producer-side admission, and not consumer arrivals, for the reason the bullet above gives — at every point,
including mid-transition:

> `retained + unspent issued credit + reserved capacity ≤ budget`

1. **Revoked credit stays owned and charged until acknowledged.** Reducing an account's ceiling does not move
   its unspent credit anywhere; it remains charged to that account under its old epoch until the account
   returns it.
2. **Only acknowledged credit is reissued.** The allocator may not grant to B what it revoked from A until A
   has returned it, which is what stops the sum exceeding the budget during a transition.
3. **Acquisition validates and transfers atomically.** One step checks the epoch and moves credit from unspent
   to retained. An acquisition that commits under a superseded epoch is valid — the credit was genuinely held
   — and simply reduces what A has left to return; one that has not committed is retried against the new epoch.

These are ordinary ledger mechanics rather than a novel protocol, which is the point: the allocator needs no
consensus and no coordination beyond one owner per account.

**Three further rules make the allocation implementable rather than aspirational:**
...
Today's `MemoryLimitController` cannot support this and is replaced — for every path that charges it, not only
the blocking one, since the bypasses above are on the non-blocking path. The reason is worth stating precisely,
because the obvious diagnosis is the wrong one. Its gate tests
...
The two dimensions are charged at different moments, and the whole of this subsection follows from that. A
send's **size** is known when it is accepted; the **entry** it will become does not exist until a batch is
closed. So the lifecycle has four points, and each is stated below in the order it happens.

**1. Acceptance decides admission, and nothing after it re-gates the send** — on the connection it was admitted
on. (Replay onto a *new* connection is the single exception, for the reason given under the lifecycle rules:
the grant it was admitted under belonged to a broker relationship that no longer exists.) This is why the
budget is a rate with a burst rather than a window: a debt that settles later is meaningless in a window.

**2. At acceptance the client charges an estimated byte cost and reserves an entry liability.**

The estimate is needed because client and broker do not measure the same bytes. The client knows the message's
uncompressed payload plus metadata; the broker pays for the `headersAndPayload` it hands to storage, after
compression and batching, and the two differ — sometimes by a lot. The budget is denominated in **broker-side
bytes**, so the client charges its estimate now and reconciles later.

The entry liability is **incremental**, and the difference between per-message and per-container is the
difference between this working and it defeating the adaptive batching beside it. Reserving one entry for every
accepted message would exhaust a budget-limited producer's entry credit long before those messages could share
an entry — the opposite of what the batch denomination exists to encourage. So: **one unit when a message opens
a new batch container, zero when it joins an existing one**, and the bounded chunk count for a message that
chunks. Key-based batching adds a container per new key and none for a key already present
(`BatchMessageKeyBasedContainer:49-52`), so the liability tracks the key count as it grows rather than assuming
it. Without this, "at most one message of overshoot" is not a bound on entries at all.

**3. When the `CommandSend` is formed, both are settled.** The batch token is charged as a **post-paid debt** —
the bucket may go negative — and the byte estimate is reconciled against the actual serialized size, returning
or charging the difference in the same step. A client that systematically under-estimates is corrected by the
reconciliation rather than by the broker, which is what keeps the two ends agreeing about what was spent. Any
operation that splits or replaces a container settles the old reservation and opens a new one.

**4. Settlement refunds what produced no entry — on whichever response actually arrives.** Three cases produce
no entry: a message the broker deduplicates, a publish to a non-persistent topic, and a publish the broker
rejects before storage. They do **not** share a carrier, and an earlier draft's "the refund travels on the
receipt" was wrong for the third: a rejected publish returns `CommandSendError`, never a `CommandSendReceipt`
(`Producer:220-278`, `:509-529`), so an implementation that waited for a receipt would leak that entry credit
permanently. Nor is the admission snapshot the right vehicle — it is an absolute *rate*, deduplicated by
`update_sequence`, and a per-send settlement is neither.

So settlement is its own field, present on both response commands and correlated to the send rather than to a
budget:

- **The client settles locally where it already knows the answer.** A publish to a non-persistent topic is
  known to be entry-free from the topic name — a test the client already makes elsewhere for its own reasons
  (`ProducerImpl:777`) — so it is charged zero
  entries at acceptance and needs no refund at all.
- **A `CommandSendReceipt` carries an `entries_charged` count** — the entries the broker actually created for
  that send, zero for a deduplicated message. The client returns the difference against what it reserved.
  Deduplication is already reported this way in substance, as a receipt whose ledger and entry ids are both
  `-1` (`PersistentTopic:702-705`, read at `ClientCnx:847-878`), so this makes an existing signal countable
  rather than adding a new one.
- **A `CommandSendError` carries the same field**, and only where the broker can prove the send never reached
  storage — the pre-storage rejections above. An **ambiguous** failure carries no settlement: a persistence
  error that may or may not have written an entry is not evidence of a refund, and treating it as one would
  let a failing storage layer manufacture credit.

Settlement is applied **at most once per `CommandSend`**, and the identity it is keyed on is the client's own
pending operation — the `OpSendMsg` a response is already matched against — and explicitly **not** the
message's sequence id. A sequence id will not serve as the key, for two reasons the client already knows
about. Chunking reuses it: `updateMessageMetadataSequenceId` returns the *same* id for every chunk of a
logical message once the first has set it (`ProducerImpl:745-754`, called per chunk at `:699`), so an
*n*-chunk message is *n* sends sharing one id, and since a chunked message produces one entry per chunk there
are *n* settlements to apply — keying on the id would apply one and leak the other *n* − 1 permanently, which
is the leak this rule exists to prevent. And sequence ids are caller-supplied through
`TypedMessageBuilder.sequenceId` and `ProducerBuilder.initialSequenceId`, with nothing requiring them to be
distinct, so the key would be under the application's control. A chunked logical message therefore settles
**per chunk**, against each chunk's own operation. A retransmission after reconnect resolves to the same
operation and so cannot refund twice. What is unsettled at terminal
cleanup — a send failed with no response at all — is released with the rest of that operation's charge.

A scope whose traffic is entirely non-persistent has no entry denominator at
all and its batch dimension is inert; the byte dimension still binds.

Charging an estimate and reconciling it is not unusual: Amazon describes the same pattern for requests whose
cost is unknown until they are served, charging every request as the cheapest possible and correcting once the
true cost is known.

#### Admission waits, delivery deadlines and connection recovery
...
What it covers and what it does not both matter here, and both are commonly misread.
...
acceptance and nothing re-gates a message afterwards on the connection it was admitted on, so accepted work
transmits as fast as that connection allows.
...
producer reconnecting repeatedly would otherwise never reach it. For a chunked message it starts at the first
chunk and covers the whole logical message, since a partial message is not a delivery. On expiry it fails
**that operation and the ones behind it on the same producer**, because a gap in the middle is the outcome
*Motivation* rules out — the blast radius is the same as today's, and what changes is when the clock starts.

**Accepted work that has not yet been transmitted has no clock running at all**, and this is worth stating
because it is the gap between the first two rows of the table. Admission is over, so `maxSendBlockTime` does
not apply — it expires with nothing pending by construction and cannot fail work already accepted — and
transmission has not begun, so the delivery deadline has not started. What bounds such work is the client's own
windows, and they bound how much can *accumulate* rather than how long any of it sits there: a bound on
quantity is not a deadline. An application that wants one supplies it itself, on the readiness future.

The **connection watchdog** is a property of the connection, not of a producer, and it is important not to
promise more than that. It observes whether the broker is responding at all, which the existing keep-alive
already measures — and, as *Security Considerations* explains, that state is cleared by **any** command
arriving, so it is satisfied by another producer's traffic on the same socket. It therefore cannot detect one
producer making no progress while the connection is otherwise busy; that is what the delivery deadline is for.
It fails the connection and lets the reconnect path replay, rather than failing sends.
...
The new API's own deadlines are opted into by using it: `maxSendBlockTime` bounds the blocking waits in `send()` and `stage()`,
...
One implementation rule goes with it: **each caller gets its own future over one shared readiness event**, and
the producer's gate state never depends on what a caller does to the future it was given. The client hands out
a `copy()`, not its internal object. The reason is that `orTimeout` completes the future it is called on and
returns that same object, so handing out the internal one would let a single caller's deadline fail every other
caller's wait — and leave a future that could never afterwards complete normally.
...
- **It cannot grow the client's memory without bound.** A wider window holds more messages in the batch
  ...
  absolutely as it first appears: once the charge is in serialized broker-side bytes, repacking *can* reduce
  it, by amortising per-entry framing and by compressing a larger window better. What it cannot do is
...
no adaptation available. That is a real consequence of pricing entries rather than
messages. Nor does a larger batch come free downstream: it is one dispatch unit and one
...
**What the delay term actually is.** `z_delay` is the **minimum** pre-initiation queueing delay observed over
the control period, divided by the target — and the choice of the minimum is the algorithm rather than a
detail. CoDel is explicit that an average "would indicate a persistent queue where there is none": a burst that
arrives and drains raises the mean while the queue is genuinely empty part of the time. The minimum over an
interval is what distinguishes a **standing** queue from a transient one, because if even the least-delayed
publish in the period waited longer than the target then the queue never drained. A standing queue is the only
thing the delay term should reduce a budget for; the occupancy terms remain independent grounds for a
reduction, as the recurrence below says.

Two consequences follow, and they are conditional rather than absolute. A transient disturbance is invisible to
the minimum **when the queue drains and further publishes arrive within the same window**, because those
publishes wait nothing — a GC pause with traffic continuing behind it is the usual case and is handled. What is
not handled by that alone is a finite burst in which every publish waited longer than the target and none
arrives after the drain: emptying a queue does not manufacture a sample. So a window explicitly records a
**queue-empty event** and treats it as a zero sample, which is how CoDel avoids the same problem.

**Three facts about the available instrumentation constrain any implementation:**

- `PendingBytesPerThreadTracker` already counts pending publish bytes per IO thread, but it is charged before
  the producer publish call and released by the IO-thread completion task, so it spans the whole path —
  retained buffers, storage, and the return hop. It is a good occupancy safeguard and a poor proxy for storage
  queue depth.
- Managed-ledger add latency **misses its own initial executor wait**: `asyncAddEntry` submits a task, and
  `OpAddEntry`'s `startTime` is set after that task is dequeued, so time spent waiting for the dequeue is
  invisible. Submission-to-start timing has to be added.
- Completed-operation latency stops producing samples exactly when storage stalls, so an **oldest-outstanding
  age** has to be measured alongside it. Note what this measures: *outstanding age*, not absence of
  initiation, which is a different condition — BookKeeper completions can stop while the broker happily keeps
  initiating writes (`ManagedLedgerImpl:908-923`).

That last measurement feeds a **stall detector** rather than the delay term, and the distinction matters. An
oldest-outstanding age is a maximum over outstanding work, and `z` is already a maximum over pressures, so
feeding one into the other would make the controller a max-of-max that any single slow operation pins. The
detector instead has its own threshold, `adaptivePublishStallMillis`, and its own action, and that action has
to be written as its own law rather than as "the ordinary multiplicative step". The reason is that the
ordinary step is a function of `z`, and **the state the detector exists to catch is one in which `z ≤ 1`**:
BookKeeper can stop completing writes while the broker happily keeps initiating them (`ManagedLedgerImpl:908-923`),
so pre-initiation delay stays low, pending bytes can sit under their watermark, and
`adaptivePublishOutstandingOpsWatermark` ships unset. Feeding `z ≤ 1` into `1 − β(z − 1)` yields a multiplier
of **at least 1** — the detector would raise the budget during a storage stall, which is exactly backwards. So
the rule is stated directly: while the scope is stalled,

```
f ← max(f_min, f · adaptivePublishMinBudgetFactor^(Δt/period))     regardless of z
```

— the per-step floor applied as the step itself, normalised for the elapsed interval like every other
actuation. The reduction **bypasses the minimum-sample gate and the sustained-periods gate** — a stall is
precisely the state that produces no samples, and would otherwise leave the controller sitting still while
nothing completes. It bypasses nothing else: the actuation interval still applies, the increase branch is
suppressed for as long as the detector holds, and it never raises `f`. A period with no publishes at all
likewise leaves `f` unchanged unless the detector fires. Where a `z > 1` decrease would apply in the same
actuation, the stall step **replaces** it rather than compounding with it — the two are alternative reasons to
take one step, and applying both would put 0.25 into a single nominal period and defeat the `max(0.5, …)`
clamp that exists to stop exactly that.

**The trigger has to be a property of the scope, not of its unluckiest member**, or this reintroduces the
noisy-neighbour behaviour *Allocation* promises to prevent. An oldest-outstanding age is a maximum, and a
scope can be an executor or the broker as well as a single managed ledger — so one wedged operation anywhere
on a broker would otherwise pin the broker scope, drive it to `f_min` in about five actuations, and throttle
every producer on the machine for one stuck write. That is the same max-of-max hazard that kept this
measurement out of `z` in the first place, arriving by the back door. The detector therefore fires only when
the oldest outstanding operation exceeds `adaptivePublishStallMillis` **and at least half of the scope's
outstanding operations do too**. The second clause turns a maximum into a property of the scope, which is what
makes it safe to apply to an aggregate one: one wedged write among a hundred completing normally moves
nothing, while a scope whose work is mostly stuck fires whatever its oldest straggler happens to be.

**"Half" is a constant rather than a setting, and the near-miss is worth naming.** The obvious second clause —
that the scope completed *nothing* in the window — is too strong, and it fails on the realistic case: a single
slow bookie in an ensemble leaves the fast completions arriving while most operations wedge behind it, so a
no-completion test would stay silent through exactly the degradation this detector exists to catch. A
fraction catches that and a maximum does not. It is fixed at a half rather than exposed because an operator
has no basis on which to choose a different one, and this document has already argued that the answer to
twenty settings is fewer knobs an operator must touch rather than more.
...
### The controller: turning a measurement into a budget
...
healthy rate".** Leaving it at that phrase is not a cosmetic omission: with
...
- **Growth.** The reference is a high-water mark and it rises **multiplicatively**, not by adding the admission
  ...
  units of `f` and adding `0.01` to a quantity measured in entries per second is not a defined operation. In each dimension, the reference rises only when `f = 1`, `z_delay` is below the
  headroom factor, and admitted traffic has reached that dimension's **allocatable** rate. The allocatable rate
  is the reference less the newcomer reserve and the committed fallbacks; the growth test deliberately does not
  use the raw reference. Testing against the raw figure would be a trap, because allocations are issued from
  the reduced one: with no newcomers arriving and no grants expiring, steadily paced incumbents could never
  consume the withheld remainder, and the estimator would sit at `f = 1` discovering nothing. Demand at the ceiling with no queue is the only evidence that more
...
- **Decay.** A reference not reached for `adaptivePublishReferenceDecayPeriods` decays toward the observed
  ...
  against it. It is **suspended whenever `f < 1`** — not merely during a reduction, which would leave a scope
  parked at `f_min` decaying onto the very traffic its own throttling produced — so a throttled scope's
  reduced traffic never becomes its new idea of capacity. Decay is floored at the bootstrap rate, so a scope
  that goes quiet does not have to rediscover its capacity from nothing.
...
- **Ceiling.** Growth is capped by any configured publish rate limit applying to the scope, and without this
  cap the claim that the adaptive budget sits *below* the configured limiters is not true — the reference
  would rise multiplicatively past a configured ceiling and the broker would then be telling a cooperative
  client to publish at a rate its own limiter will refuse, which is worse than telling it nothing. The cap
  applies cleanly in the **byte** dimension, where a configured byte rate and the reference are the same
  currency. It does **not** apply in the entry dimension, because the configured limiters count messages and a
  message rate is not an entry rate — so an entry budget is bounded by the byte budget beside it and by
  nothing else, and where a configured *message* rate binds first the client will be paced by a limiter it was
  not told about. That residue is the reason the configured-limiter half of PIP-385 is worth doing and is
  named as follow-up work rather than claimed here.
...
act on.** The reference moves slowly, and *upward* only on positive evidence — decay and reset move it down
without any, which is the safe direction. `f` moves quickly and mostly on negative evidence.
...
That inequality is uncomfortable at the defaults. Halving takes two control periods only under *severe* pressure: at `z = 1.1` the multiplier is
...
**Two worked timings, and why they are illustrative.** At a fixed 100 ms cadence with `a = 0.01`, `f` climbs
0.1 per second, so a scope at `f_min = 0.05` takes about 9.5 seconds to return to its reference. A fall to the
floor takes five reductions — which, with the sustained-periods gate at 2, is at least six consecutive bad
observations and 600 ms.

Both figures assume a fixed cadence and continuous eligibility, and neither holds exactly: the actuation
interval is a floor rather than a period, a volume-triggered evaluation can bring it forward, the confirmation
counter resets on any observation with `z ≤ 1` so an intermittent overload takes longer, and the increase only
runs while producers are admission-limited. They are the right order of magnitude and not a schedule.
...
The recurrence above is written per **nominal** control period; when an evaluation lands at a different
interval the multiplier is normalised as `m^(Δt/period)`, so a decrease at half the period is `0.707` rather
than `0.5`. Both clamps are load-bearing and they are different bounds. `max(0.5, …)` floors a single step
**at the nominal period**, so one bad interval cannot collapse the budget; `f_min` floors the **sequence**, so a sustained overload
cannot drive a scope to zero through repeated decreases. Only the sequence floor binds in the end: from
`f = 1`, five maximum reductions give 0.5, 0.25, 0.125,
...
reference anywhere; this proposal emits rates because a publish's cost is not one unit. That reason stands on
its own, and an earlier draft propped it up with a second one that was simply false — that the client already
has a token bucket to reuse. It does not: `AsyncTokenBucket` is broker machinery, every bound the producer
client owns today is a window, and the rate shape therefore *adds* a client-side bucket that the window
alternative would not need. That is a cost of this choice, not a saving. Breakwater does not freeze its increase on demand-limited clients, has no
...
admission factor over a frozen reference. The orders of magnitude agree and the operation does not, so only the
former is claimed here.

**When the budget may grow.** The increase is frozen while producers are **demand**-limited — publishing less
than the budget already allows — because otherwise the controller accumulates an imaginary capacity estimate
over an idle period and then admits a burst against it. It grows only while producers are
**admission**-limited, where a larger budget would actually be drawn on and the next measurement therefore
tests it.
...
costing two varints on a frame that is already on the hot path. **It does influence capacity, and the temptation is to claim otherwise in the same breath as describing how.**
...
The hint has two limits. It measures the client's **queued backlog**,
...
**The stability constraint is on the gain, not on the period.** The intuitive rule — that the loop period must
exceed the time for a decision to reach clients and change what arrives — is wrong, and Breakwater refutes it
directly: it runs its loop *at* one network RTT — exactly the feedback delay — and gets
...
comfortably for any feedback delay under a period. What happens *beyond* a period is where this document's
claim has to stop: the expectation is graceful degradation rather than oscillation, and that expectation is
one of the five things *Goals* lists as unsettled by prose. It is an acceptance criterion for enabling the
controller, not a property established here.
...
### Allocation: dividing the budget among producers
...
- **Freeze the allocation weights during reductions.** Recomputing each producer's share from
  ...
  from the scope's capacity *reference*, whose decay is suspended for the whole time `f < 1`, for its own
  reason — the weights decide how a budget is divided, the reference decides how large a budget there is to
  divide.
...
**The client enforces it locally.** The budget is spent against a token bucket in the client whose **rate and
capacity an update replaces**, and which accrues locally at that rate — an update is a new statement of the
rate, not a grant of credit, which is why supersession clamps the accumulated level and why a producer can
recover on expiry without receiving anything. That is what keeps the hot path free of round trips, and it is why the two things
the broker cannot verify are structured so that misreporting them is self-limiting: the admitting sequence can
only restrict what the broker enforces against, and the demand hint cannot enlarge a producer's own weight —
though, as *What the broker measures* says, it can make the scope probe upward, and the probe is then tested by
the delay it produces.
...
A producer accumulates tokens up to its burst and no further. What it does *not* accumulate is entitlement:
there is no store that grows with idle time, because a budget is a rate with a **bounded** burst, established
at producer creation from the scope's current admission factor and recomputed from measurement thereafter, and
a new update clamps whatever has accrued to the new capacity. A producer that has been idle for an hour is not *penalised* for it: it
starts from the newcomer path with a usable grant rather than from zero, which is what the first half of the
objection asks for. What it does not do is accumulate — an idle producer's unused share is lent to its
neighbours by the surplus rule, so it holds no stored entitlement to draw down later, which is why the second
half of the objection does not follow.

What does need bounding is the aggregate of the burst allowances, because those *are* simultaneous by
definition, and the bound has to be a **pool rather than a formula**. Breakwater's per-client overcommitment,
`max((C_total − C_issued) / n, 1)`, is the obvious formula to borrow and does not bound the aggregate at the
unallocated remainder: summed over `n` producers it is `max(U, n)`, so a fully allocated scope with a thousand
producers grants a thousand units rather than none. Breakwater can
...
are not the same currency**. A burst is a quantity of work that may
...
- **The liability pool** bounds the burst grants and newcomer grants the scope has outstanding — the capacity
  it has *committed* without it having been spent. A burst grant is one minimum viable unit, sized to a single
  maximum-sized batch, made only when the pool can fund it. The floor of one unit is what keeps a producer on
  a tiny budget able to send a single batch at all, and is the same floor the overspend allowance needs.

  **It does not bound accepted-but-untransmitted work, and an earlier draft claimed it did.** That work is a
  client-side queue the broker cannot see: a compliant producer charges its bytes at acceptance, holds the
  messages behind a slow transport, and admits more as its bucket refills — a rate bucket replenishes with
  time, so nothing in it caps a standing backlog. What actually bounds that backlog is client-side and is
  specified under *What the client bounds*: the account's retained-byte ceiling, the pending-position and heap
  budgets, and — for the never-refuse path — the single funded reservation, which is why the overshoot there is
  one message per producer rather than a queue. The broker's only view of it is the demand hint, which is
  untrusted evidence by construction. Stating the pool as a bound on all three would have promised an aggregate
  guarantee no party is in a position to enforce.
- **The rate budget** is what the ordinary allocation divides, and it is divided **after** the standing
  ...
  rates the scope has committed to. The proportional formula applies to what remains — allocating 100% of
  `R_batch` while separately reserving 5% of it does not add up.
...
received an update has no fallback to take and is governed by the ordinary path — and a scope whose
committed fallbacks would exceed `adaptivePublishFallbackReserveShare` of the minimum budget reachable while
those commitments stand stops granting them and
...
**A fallback outlives the budget it was sized against, so it must be funded against the smallest budget it
could ever have to be honoured from.** This is the rule an earlier draft was missing, and the arithmetic shows
why a grant-time test alone is not enough. Take a scope with a reference of 1,000 entries a second at `f = 1`:
5% commits 50 entries a second of fallbacks, which is within the share. Now let the controller do its job and
reduce `f` to its `f_min` default of 0.05. The scope's rate budget is 50 entries a second; the standing
fallbacks still require 50, the newcomer reserve requires 2.5, and `A_batch` — the budget less both — is
**−2.5**. Ordinary allocation has gone negative, and the clients holding those fallbacks are entitled to
exercise them at exactly the moment the scope is least able to serve them, because expiry and congestion have
the same cause.

Three rules close it. The share is measured against `f_min · reference` — the least the budget can fall to
while the reference itself holds — rather than against the current budget.
`A = max(0, R − newcomerReserve − committedFallbacks)` is clamped at zero rather than allowed to go negative,
and a scope that reaches the clamp publishes it through `pulsar.broker.publish.admission.allocatable`, which
exists for this state; it is deliberately **not** the enforcement counter, since reaching the clamp is a
funding condition and not a client failing to comply. And because the *reference* can itself move — it decays
toward observed traffic and resets outright on a ledger rollover, an ensemble change or a restart, while a
commitment stands until the producer's incarnation ends — **standing commitments are re-tested whenever the
reference falls**, and those that can no longer be funded are withdrawn by sending an update with the fallback
fields absent. A withdrawal is always possible at that moment, because a scope re-testing its reference is by
definition still in contact with its clients; what the funding rule has to survive is silence, and silence
cannot begin retroactively.

**The cost of this is that a fallback is scarce, and the document should say how scarce rather than leave it
pleasant-sounding.** At the defaults the fallback pool is `0.05 × f_min × reference` = 0.25% of a scope's
reference, and each commitment costs at least the one-unit floor — one entry per three-second validity, a
third of an entry a second. A scope with a reference of 1,000 entries a second therefore funds about **seven**
standing fallbacks, not the ten thousand the paragraph above uses to illustrate the cost of granting them
universally; that figure needs a reference near 1.3 million entries a second. So on an ordinary topic most
producers hold no fallback, and their expiry path is the ordinary one — client-side windows, configured limits
where an operator set any, and the occupancy guards. That is the status quo rather than a regression, but it
is not the recovery story the fallback exists to tell, and it is worst in the correlated case where a
congested broker stops refreshing everyone at once. Which producers get the scarce grants is therefore part
of the contract: they go to the scope's largest admitted-traffic producers first, because those are the ones
whose uncontrolled return would hurt most. Raising `adaptivePublishFallbackReserveShare` is the lever, and the
metric that says whether it needs raising is the count of expiries that found no fallback.

The newcomer reserve is `adaptivePublishNewProducerReservePercent` of the scope's **rate** budget, and what
the FIFO changes is how that reserve is *handed out*: one complete minimum viable quantum at a time, in arrival order,
...
opposite of the intuitive one, which treats the piggyback as a later optimisation. WeChat's DAGOR spends
...
than as prose because it is easy to describe an expiry behaviour that is not self-consistent.
...
- **Expiry.** When `valid_for_ms` elapses with no newer update, the granted burst allowances lapse at once,
  down to the one-unit floor the transition below specifies. A producer
  whose last update carried a fallback rate **takes it, in one step**; one carrying none — because the scope had
  committed its whole `adaptivePublishFallbackReserveShare`, or because the producer never held a budget at
  all — has no broker-derived rate and falls back to the ordinary path, bounded by its own client-side windows
  and by whatever configured limits and occupancy guards apply. Which case a producer is in is visible on the
  wire, since the fallback fields are either present or not.

  Where a fallback exists, the transition is to it and deliberately **not** to zero or to whatever was granted
  at creation. A creation-time budget is a healthy-cluster number that would be restored precisely when silence
  most likely means the opposite, and a reconnect storm would re-arm it for every producer at the same moment.

  **The transition is a single step, stated exactly, because it is protocol rather than policy.** An earlier
  draft said the producer "converges" toward the fallback "a step per validity period" and never defined the
  step — which would have left two independent client implementations enforcing different rates after
  receiving the same update and the same silence. There is no second controller here. On expiry, in each
  dimension:

  1. the bucket's **rate** becomes the advertised fallback rate, at once;
  2. its **capacity** becomes one minimum viable unit — a single maximum-sized batch in the entry dimension,
     and the bytes that unit implies — which is the smallest capacity from which a whole entry can ever
     accumulate, and is why "the burst allowances lapse" cannot mean capacity zero;
  3. a **positive** accumulated level is **clamped** to that capacity — the same operation a superseding
     update performs, deliberately, so that expiry and supersession are one rule rather than two. Clamping
     already removes every banked advantage, because rule 2 has just set the capacity to a single unit; and
     discarding instead would leave the producer unable to send anything for a whole validity period, which
     would contradict the recovery-without-a-further-message property that justifies the TTL in the first
     place; and
  4. a **negative** level carries across unchanged, because a debt is not forgiven by an expiry any more than
     it is by a new budget.

  That is deliberately sharper than a glide, in both directions. A producer above the fallback drops to it in
  one step, which is the safe direction and the reason no smoothing is warranted. A producer *below* it —
  including one that was cut to zero — rises to it in one step, which is what stops a lost or withheld update
  muting a producer permanently, and it is bounded by construction because the fallback is a probe rate the
  broker already funded. The broker retains its
  involuntary instrument for the case where that recovery is not warranted, which is why letting it recover
  is safe. A gradual policy remains addable later, additively and under negotiation, if measurement shows the
  step is too sharp. The tradeoff against the alternatives is worth stating without overstating it: a
  ...
  given a formula whose inputs it cannot see. **The broker funds the fallback from the scope's *rate* budget**, which also funds the
  newcomer reserve; bursts draw on the separate liability pool. **And it is not calculated as a multiple of the
  producer's current allocation**, because a producer cut to zero would then receive a fallback of zero and
  could never recover — precisely the mute this rule exists to prevent.
  ...
  never accumulate. The aggregate of the outstanding fallbacks is charged against the scope's **rate** budget —
  not against the liability pool, which funds bursts — because a fallback is a rate that continues indefinitely
  and a scope has to survive every producer's updates stopping at once. The one-unit burst capacity each
  expired producer retains is the pool's side of the same event, and is funded there.
...
- **Replay after a reconnect is paced — and it is the one explicit exception to "acceptance only".** This is
  ...
  It is a real exception: work admitted on a *previous* connection is re-gated on the new one, because the
  grant it was admitted under belonged to a broker relationship that no longer exists. Two consequences. The messages keep their order and their sequence
...
**To assess overspend, the broker needs each send to identify the budget the client admitted it under**, and
that requires one more field.
...
carrying the latter instead would defeat it. A client may legitimately accept work under update 10,
...
and the rule that catches it is the volume bound stated immediately below rather than a timer — without some
such rule, withholding the field is a way to opt out, since the configured rate limits default to off and the
occupancy guards bound outstanding memory rather than compliance. And the
...
**The exemption for work admitted under an older grant is bounded by volume, not by elapsed time**, and this
is the rule an earlier draft got wrong in both directions. A pure time bound punishes a compliant producer:
accepted work has no clock before transmission, adaptive batching deliberately widens the window in which it
waits, and such a producer can legitimately still be draining sequence-10 work more than a validity period
after update 11 arrived. But a pure exemption is an opt-out, and one the broker cannot see through — admission
is a client-side act that leaves no evidence, so "I was still draining" and "I am ignoring you" arrive as the
same frames.

What the broker *can* size, without trusting the client at all, is **how much work grant 10 could ever have
legitimately funded**, and that has to be measured over the whole time grant 10 was the client's operative
budget rather than over one validity period. Four terms, all of them the broker's own arithmetic:

1. grant 10's rate from when update 10 was sent until it **expired at the client** — its send time plus
   `valid_for_ms` — or until update 11 was sent, whichever came first;
2. plus, over the **remainder** of the interval up to update 11 — the part in which grant 10 had expired —
   the fallback rate the broker itself granted, because a producer whose updates stopped goes on admitting at
   that rate and has no newer sequence to stamp;
3. plus grant 10's burst allowance;
4. plus the one submission the reservation entitles the producer to.

The two windows are **disjoint**, and that is what makes the figure bite. Term 1 is capped at one validity
period rather than running for as long as the broker stays silent; if it ran at the full grant rate through a
silence, a client could stamp sequence 10 for ever and keep publishing at its old rate while its allowance
grew to meet it — which is exactly the opt-out this rule exists to close, and a congested broker is precisely
the one that goes silent. After expiry the client is entitled to the fallback rate and no more, so that is
what term 2 grants it.

So the rule is a budget on the exemption rather than a deadline on it: work arriving stamped
`admitted_under_sequence = 10` is honoured against grant 10 until the total so stamped reaches that figure,
and every further arrival under that sequence is enforced against the current grant. A producer that merely
drains is covered by construction, because its backlog cannot exceed what it was permitted to admit — and
term 2 is what makes that true through a silence, which is the common case rather than an edge, since a
congested broker is exactly the one that fails to refresh. A client that stamps 10 on everything for ever is
caught the moment it exceeds the figure, which it must if it is publishing more than the broker ever funded.
Elapsed time appears only inside those terms, never as a deadline of its own, so a slow drainer is never
punished for being slow.

**Old clients keep working, and are not trusted to.** The command is sent only after feature
negotiation. A client that understands it and then ignores it gets a bounded transition
allowance and then the existing involuntary instrument, `setAutoRead(false)`, on its connection. A client that
never negotiated the feature gets neither — no budget, no allowance, no `OVERSPEND` update — and is throttled
exactly as it is today, which is the only compatible answer. An advisory percentage cannot protect a broker on its own; the signal makes
...
dividing a budget by `f` recovers the reference and not what was offered. What a balancer would need is the set, kept
separate rather than collapsed into one figure: `admission.factor` for controller state,
`admission.reference` for what it is a fraction of, `admission.allocated` for what producers were given,
the existing usage score for what they used, and `publish.demand` for what they wanted. All five are published
here; making the balancer consume them is out of scope, and saying that they exist and why is not.
...
Raising `connectionsPerBroker` spreads a client's producers over more connections but cannot isolate one, since
it is client-wide. A per-producer isolation API ([#23403](https://github.com/apache/pulsar/issues/23403)) would
address what remains and is independently shippable, but it is not specified here, and it delivers less than it
appears to: `IOThreadMaxPendingPublishBytesExceeded` pauses reads on every connection sharing an IO thread
(`ServerCnx:305-312`), and a producer still draws on its account's share of the client's memory whatever socket
it owns. A dedicated socket isolates a socket — not client memory, not an IO thread, and not storage.

What this proposal changes is that a dedicated socket is no longer the *only* isolation available. The
per-account memory ceilings and the per-producer admission arbiter isolate the two resources a socket cannot,
which reduces #23403 from a workaround for a general problem to a targeted fix for the one it actually
addresses: the shared read-pause blast radius.

## What this interface promises

Everything above describes mechanism *and* contract, mixed together as a design has to be. This section
collects the **contracts** in one place, in a form that can be turned into tests. It is a digest, not a
definition: a rule stated normatively above — the scheduler's ordering and liveness properties, say, or the
allocator's ledger invariant — is binding whether or not it is repeated here, and omission from this list
does not make it freely changeable. Each entry is written so that it can be turned into a test, and each is followed by what is explicitly
...
**`submitAsync()` — Promise:** while the producer's reservation is funded, a message within its declared
submission envelope is accepted. It is not refused for lack of capacity under any budget, so **capacity can
never be the reason a gap appears in the stream.** The guarantee is conditional on that funding, and there are
**two** ways to lose it after creation. The first: a reconnect that negotiates a smaller maximum message size
raises the chunk count the envelope implies, and so the reservation. If the client cannot fund the larger
vector the guarantee is **suspended** — `submitAsync()` then fails loudly rather than degrading into a
refusing call — and resumes when it can. An application that must not see that should pin its envelope below
the smallest maximum message size it will meet. The second: on a client that shares one `MemoryLimitController`
with a consumer running `autoScaledReceiverQueueSizeEnabled`, that consumer's arrivals are charged *after* the
bytes are already retained and cannot be refused, so they can carry the controller past the budget the
reservation was funded from — see *Sharing the client's memory between producers*, which sizes the excursion.
On such a client the guarantee is conditional on the consumer population; on a controller serving producers
only it is unconditional. The returned `SendSubmission` always carries the acknowledgement future, and carries a present
...
- **Not unconditional on a client that also consumes.** A consumer with
  `autoScaledReceiverQueueSizeEnabled` sharing the same `MemoryLimitController` charges its incoming buffers
  after they are already retained, so the allocator cannot refuse them and they can carry it past the budget —
  by up to outstanding permits times the maximum message size, which at stock defaults is far larger than the
  budget itself. The guarantee holds unconditionally only where the controller serves producers alone. This is
  a real limit, it is not small, and closing it needs byte-aware consumer permits, which are out of scope here.
...
**`Producer.whenSendable()` — Promise:** the returned future completes when this producer has **ordinary**
admission capacity — deliberately not counting its reservation, since a producer whose only remaining capacity
is the reservation should not be encouraged to fetch more work — so a caller can decide whether to acquire the
next message from its source. This is a different predicate from `SendSubmission.whenSendable()`,
which requires the reservation restored **and** the floored resume budget — and, deliberately, not ordinary
capacity, since the reservation is what backs the next submission.
...
**`pendingSendHeapLimit` — Promise, when enabled:** the heap the *client* retains for pending sends is bounded
by this number. It ships disabled (`0`) in its first release, where the gauge reports what *would* have been
charged — which is a measurement and explicitly not a bound, so the adaptive-batching safety argument that
rests on this limit requires it to be enabled. The accounting
...
index it returns identifies that message in the list `collect()` returns. The untimed `collect()` returns one outcome per staged
message, in staging order, each with a **terminal** status; the timed overload may additionally return
`UNRESOLVED` for sends still live, whose terminal outcomes are retained for the next collection rather than
lost. A `NOT_PUBLISHED` outcome means the client can
...
*Testable by:* asserting the two at their own granularities and not confusing them. Two producers on
*different* topics have separate accounts, so one stalled producer cannot push the other past its account's
ceiling — subject to borrowing, which means the neighbour's *lent* capacity is returned as sends complete
rather than instantly. Two producers on the *same* topic share one account and are isolated only in their turn
at admission.
...
binding, is stale the moment it is returned, and is **absent altogether** wherever no broker rate exists,
which is every deployment that has not yet enabled the controller. The same number reaches operators through
`PublisherStats`.
...
**And one promise is clarified rather than narrowed.** `sendTimeoutMs` is widely read as a promise that a send
either completes or fails within a bounded time of the application handing it over. It has never been that:
the admission wait precedes the operation whose clock it starts, so a producer parked in
`MemoryLimitController.reserveMemory` is outside it entirely. Applications that do not opt in keep exactly the
behaviour they have, absent admission bound included. What the proposal will not do is extend it to cover waiting for admission,
...
This list is the **authoritative declaration inventory**: every method, type and setting this proposal adds to
`pulsar-client-api` appears here, and a shape shown in an interface sketch earlier in the document but absent
here is not proposed.
...
- `Producer.whenSendable()` returning `CompletableFuture<Void>` — advisory, for deciding whether to acquire
  the next message.
- `Producer.currentBudget()` returning `Optional<ProducerBudget>`, a new value type carrying
  `long batchesPerSecond()` and `long bytesPerSecond()`. The `Optional` is empty — rather than the type
  carrying a sentinel — wherever there is no broker-derived rate to report: a client that has not negotiated
  admission control, a broker that has not enabled it, and a producer whose budget has expired with no
  fallback. That is the normal state for stage 3 against old brokers, so it is the case the type is shaped
  around rather than an edge. Advisory and immediately stale by construction; it is not a reservation and not
  a scheduling primitive.
- `TypedMessageBuilder.stage()` returning a `long` index — a terminal operation like `send()` — and
  `Producer.collect()`, `Producer.collect(Duration)` and `Producer.resumeStaging()`; the new types
  `SendOutcome` and `PublicationStatus`.
...
- `Producer.supportsAdmissionControl()` and `Producer.supportsStaging()`, each returning `boolean` over a
  frozen, enumerated method group, so an application can ask rather than catch.
...
**Compatibility.** `Producer` and `TypedMessageBuilder` are `@InterfaceStability.Stable`, so every addition
must be a `default` method. `CODING.md:207-211` states two options in order: prefer a default that delegates
to an existing method, and "if no sensible delegation exists, add a separate capability-query method". No safe
delegation exists here, so this proposal takes the second — which is following that guidance, not departing
from it. A `trySendAsync()` that delegated to `sendAsync()` would be wrong twice: `sendAsync()` can block on a
third-party implementation configured with `blockIfQueueFull(true)`, and it can fail with a capacity error,
neither of which the `trySendAsync()` contract permits. A `whenSendable()` that returned a completed future
would assert capacity that a third-party producer never promised. A default that compiles and lies is worse
than one that refuses.

**How an unsupported default refuses depends on the return type**, and getting this wrong would violate the
project's own asynchronous rule. `CODING.md:42-45` requires that a method returning `CompletableFuture` never
throw synchronously, because a caller that has attached `.exceptionally(...)` would never see the failure. So:
a default with a synchronous return type — `trySendAsync()`, `submitAsync()`, `stage()`, both `collect()`
overloads, `resumeStaging()` and `currentBudget()` — throws `UnsupportedOperationException`; a default
returning `CompletableFuture`, which
today is `Producer.whenSendable()`, returns `CompletableFuture.failedFuture(new UnsupportedOperationException())`.
Both are honest; only one of them is honest on the thread the caller is watching.

**Two capability queries, not one, and the membership of each is frozen.** `Producer` gains
`default boolean supportsAdmissionControl() { return false; }` and
`default boolean supportsStaging() { return false; }` so an application can ask rather than catch. They are
separate because the two groups ship separately — staged publishing is stage 4 and is independent of
admission — and a single boolean would make a third-party implementation that supported only the first
answer `true` to a question about the second. Each query covers a **fixed, enumerated** set of methods:
`supportsAdmissionControl()` covers `submitAsync()`, `trySendAsync()`, `Producer.whenSendable()` and
`currentBudget()`; `supportsStaging()` covers `stage()`, both `collect()` overloads and `resumeStaging()`.
A later revision that adds an operation does **not** widen either meaning — it brings its own query — because
widening one would silently break every implementation that answered `true` before the addition existed. The
built-in producers implement each group in full; a third-party producer inherits an honest "not supported"
until it opts in deliberately. This is a real cost of the design and the alternative was considered and
...
**This section is the authoritative inventory of wire changes.** Where a mechanism elsewhere in this document
implies a field, it is listed here or it is not proposed.

One new broker-to-client command, `ProducerAdmissionUpdate`, carrying an absolute budget for one producer
incarnation — the fields are those sketched under *The signal*: `producer_id` and `producer_generation` to
address the incarnation, `update_sequence` to order supersession, `valid_for_ms`, batch and byte rates, burst
allowances, the fallback rates the client takes in one step on expiry, and a reason. **The same payload is also an optional field on
...
**A second, unrelated optional field is added to `CommandSendReceipt` and to `CommandSendError`:
`entries_charged`**, the number of entries the broker actually created for that send. It is the settlement
described under *What is charged, and when*, and it is deliberately not part of the admission snapshot: the
snapshot is an absolute rate deduplicated by `update_sequence`, while a settlement belongs to one `CommandSend`
and is applied at most once against the **client's own pending operation** — not against the message's
sequence id, which chunking reuses across every chunk of one logical message. The wire carries only the
sequence id; it is the in-order match against the pending queue (`ProducerImpl.ackReceived`, which peeks the
head and uses the id only to validate) that tells the chunks apart. *What is charged, and when* has the rule.
It appears on the error command because a rejected
publish returns `CommandSendError` and never a receipt, and it is omitted — rather than set to zero — wherever
the broker cannot prove no entry was written, so an ambiguous persistence failure refunds nothing. A client
that negotiated the feature but receives no field settles at its reserved estimate.
...
**And the negotiation the rest of this document leans on is itself a wire change**, which an inventory
claiming to be authoritative has to list rather than assume. Pulsar negotiates capabilities through
`FeatureFlags`, carried on both `CommandConnect` and `CommandConnected` (`PulsarApi.proto:312`, `:318`,
`:334`), so this adds one boolean to each: the client's says it implements admission control, and the
broker's says it both supports **and has enabled** it. Two flags rather than one, because the broker's half
cannot be inferred from a protocol version — `adaptivePublishAdmissionEnabled` is a runtime setting, and
"has not enabled" is precisely the state *Downgrade / Rollback* means by a budget that was never established.
Every other field here is sent only when both flags are set, which is what lets a client that never
negotiated pay nothing on the hot path.
...
- `sendResumeThreshold` — how much budget must accumulate before a **submission's** `whenSendable()` completes, expressed as a duration's worth of the producer's current rate, and resolved as `min(burst capacity, max(one entry, rate × this))`. It does not govern `Producer.whenSendable()`, which is advisory and gates on ordinary capacity alone. **Default 10 ms**, which is the same 1% of a second that `AsyncTokenBucket` already uses on the broker (`DynamicRateAsyncTokenBucketBuilder:33`) and exists for the same reason: resuming on the first available token unthrottles a producer that can send one message and must then be stopped again. It is a client setting because the bucket it governs is the client's.
...
- `adaptiveBatchingReevaluation` — how often a budget-limited client re-evaluates its accretion window. **Default 1000 ms**, an order of magnitude above the controller's period so the coupling between the two loops stays slow — a period ratio is a mitigation and not a stability proof, and this interaction is one the staged rollout has to measure. Distinct from `valid_for_ms`, which is how long a budget remains good rather than how often one arrives.
...
- `submissionEnvelope` — the maximum logical-message size for which `submitAsync()` promises not to refuse, and the size the producer's reservation is drawn against. **Unset by default**, which means no reservation and no never-refuse guarantee for that producer. It is inert unless `adaptiveAdmission` is on; and on a client where that is on, a producer without an envelope still shares the new allocator. The number of admission-capable producers a client can support is the minimum over its direct, heap and pending-position budgets divided by the corresponding component of the reservation vector — counting routed *parents*, since a partitioned producer reserves once — so this is the setting that decides whether a fan-out client can use `submitAsync()` at all.

Broker:

- `adaptivePublishAdmissionEnabled` — **enforcement**, not calculation. When `false` the controller still runs and reports its budget through `pulsar.broker.publish.admission.budget`, and no `ProducerAdmissionUpdate` is sent; when `true` the budgets are allocated and sent. **Default `false`, and it stays `false` until a separately reviewed decision changes it** — approving this proposal is not that decision, and no release schedule substitutes for one. *Upgrade* states the evidence required, which is a parameter sweep on a loop clients are actually enforcing; shadow calculation validates the estimator and not the loop. Calculation is disabled only by disabling the instrumentation itself.
...
- `adaptivePublishUtilisationThreshold` — how much of its budget a scope must actually draw, as the period's peak, before `f` may increase. **Default 0.5**, which is this proposal's own choice: the adaptive-concurrency limiters use a guard of similar shape but gate on in-flight concurrency against a concurrency limit, which is a different quantity.
- `adaptivePublishStallMillis` — the age at which an outstanding operation counts as stuck. The scope is treated as stalled, and `f` reduced by the stall law bypassing the sample-count and sustained-period gates, only when the oldest outstanding operation exceeds this **and at least half of the scope's outstanding operations do too** — the fraction is what stops one wedged write on a broker-wide scope throttling every producer on the machine, and it is a fixed constant rather than a second setting. **Default 5000**, deliberately three orders of magnitude above the delay target because it detects a different condition. It is deliberately the one detector here that ships enabled without a measured basis: leaving it unset alongside `adaptivePublishOutstandingOpsWatermark`, which does ship unset, would mean a storage stall reaches the controller only through pending bytes — an occupancy signal that arrives late and is exactly what *Motivation* criticises the existing guards for.
...
- `adaptivePublishIncreaseStep` (a) — the additive increase applied to the admission factor each period, in units of `f`. **Default 0.01**, a 1% increase per period. The same *coefficient* raises the reference when `f = 1` and the scope is drawing its whole budget without queueing — but multiplicatively, `reference × (1 + a)`, since adding a number in units of `f` to a rate is not a defined operation. That is the only mechanism by which capacity above the current estimate is discovered.
...
- `adaptivePublishMinBudgetFactor` — the floor on the **nominal-period** multiplier. **Default 0.5.** It is not a floor on every applied multiplier: an evaluation covering more than one nominal period normalises as `m^(Δt/period)` and can therefore apply less than 0.5. What it floors is how far a single period's worth of overload can move `f`.
...
- `adaptivePublishOverspendAllowancePercent` — how far a client may exceed the budget its sends are **attributed to** before the broker pauses reads on its connection, as a percentage of the budget over one validity period. **Default 20**, sized to absorb a decision already in flight. **Two different things were conflated here and they need separating.** *Repayable debt* is what a compliant client legitimately incurs: the post-paid batch token, and the one submission the reservation entitles it to make past its budget. That is expected, is settled as the bucket refills, and must never consume a misconduct allowance — so a compliant client's floor is **one submission's worth of entries per producer per validity period**, computed from the same reservation vector rather than as a percentage. On a small budget a percentage alone is less than one entry, and the broker would be punishing a client for doing exactly what the API told it to.
  ...
  *Overspend* is what remains after the repayable debt is accounted for, and only that consumes the allowance. It is **consumed per producer incarnation and does not refill**, because an allowance that refilled every period would grant a permanently non-compliant client a permanent overspend, which is the opposite of its purpose.
- `adaptivePublishNewProducerReservePercent` — the share of each scope's rate budget held back for new and returning producers rather than allocated by usage. **Default 5.** It is subtracted before the proportional division rather than alongside it, and it is what funds the newcomer FIFO described under *Allocation*, which grants one complete minimum viable unit at a time rather than dividing the reserve among however many producers happen to be waiting.
- `adaptivePublishFallbackReserveShare` — the share of a scope's rate budget it will commit to expiry fallbacks, separate from and additional to the newcomer reserve. **Default 5.** The share is measured against `adaptivePublishMinAdmissionFactor × reference` — the least the budget can fall to while the reference holds — not against the current budget, because a fallback granted at `f = 1` still has to be honourable at `f_min`; and standing commitments are re-tested and withdrawn when the reference itself falls. Once committed fallbacks would exceed the share the scope grants no more. **This is the setting most likely to need raising**, and the arithmetic is why: at the defaults the pool is 0.25% of a scope's reference and each commitment costs at least one entry per validity period, so a 1,000-entries-a-second scope funds roughly seven standing fallbacks. `pulsar.broker.publish.admission.expiry` with `fallback=absent` is what says whether that is hurting.
...
**On the number of settings.** This section adds thirty-five — eight client and twenty-seven broker — and
Pulsar's own standard is against that:
...
intended to be correct, and the one without a principled default —
`adaptivePublishOutstandingOpsWatermark` — ships unset and inert rather than guessed. Two are load-bearing and an operator will genuinely tune them:
...
- `pulsar.broker.publish.admission.factor` (gauge; attributes: scope) — the admission factor `f` itself, and `pulsar.broker.publish.admission.pressure` (gauge; attributes: scope, term) — each normalised pressure separately, so an operator can see **which** term is binding. Exposing only the controller's output makes a misbehaving loop undiagnosable; the adaptive-concurrency implementations publish every term of their control law, and this follows them.
- `pulsar.broker.publish.admission.reference` (gauge; attributes: scope, dimension) — the scope's capacity estimate in each dimension. It is what `f` is a fraction *of*, so a reader without it cannot turn the factor into a rate, and it is the quantity that shows the estimator learning, decaying or being reset.
- `pulsar.broker.publish.demand` (gauge; attributes: scope) — the demand evidence the scope is acting on: reported backlog combined with admitted traffic, per the rule in *Allocation*. This is the one that distinguishes "the controller cut and there is no demand left" from "the controller cut and demand is queued behind it", which is exactly the distinction a load balancer reading a suppressed usage score is missing.
...
- `pulsar.broker.publish.admission.allocatable` (gauge; attributes: scope, dimension) — what remains of the scope's rate budget after the newcomer reserve and the committed fallbacks, which is what the proportional division actually divides. It is distinct from `.budget`, and the distinction is the point: a scope whose budget is healthy while this sits at zero is one whose standing obligations have swallowed it, which is a funding condition rather than a client failing to comply — so it is reported here and not on the enforcement counter.
- `pulsar.broker.publish.admission.expiry` (counter; attributes: scope, `fallback` ∈ present, absent) — budgets that reached `valid_for_ms` without a newer update, split by whether the producer had a funded fallback to take. The `absent` arm is the one an operator alerts on: it counts producers that lost their broker-derived rate entirely, and a rising rate on it is the signal that `adaptivePublishFallbackReserveShare` is too small for the producer population on that scope.
...
- Enforcement attributes each send to the grant its `admitted_under_sequence` names: work admitted under an
  older grant and transmitted after a reduction is not enforced against the reduction, which is asserted by
  delivering an update late and observing no enforcement for work already admitted. That exemption is bounded
  **by volume rather than by elapsed time**, and both halves are asserted separately: a producer that drains
  earlier-admitted work for longer than a validity period is still not enforced against the newer grant, while
  a client that keeps stamping a superseded sequence past what that grant could have funded — its rate until
  it expired, plus the broker's own fallback rate over the rest of the interval until the next update, plus
  burst, plus one submission — is enforced against the current one. The four terms are asserted with a silence
  long enough that the fallback term dominates, since that is the case a three-term reading gets wrong. The first test is what
  stops the rule punishing a compliant slow drainer; the second is what stops the field being an opt-out.
...
- The stall detector fires on **outstanding age, not on absence of initiation**: a scope whose broker keeps
  initiating writes while BookKeeper stops completing them is detected, which is the realistic stall and the
  one the delay statistic cannot see. Its reduction bypasses the sample-count and sustained-period gates and
  nothing else, and it **replaces** rather than compounds a `z > 1` decrease in the same actuation.
- **The stall detector's negative case is asserted, because it is a noisy-neighbour regression if it is
  wrong.** A broker-wide scope carrying one operation older than `adaptivePublishStallMillis` while the rest of
  its work completes normally does **not** have `f` reduced — the trigger needs half the scope's outstanding
  operations to be stuck, not merely its oldest. The positive case is the partial stall a no-completion test
  would miss: a scope in which the fast completions keep arriving while most operations wedge behind one slow
  bookie does fire.
- The control loop **observes** early on `adaptivePublishControlPeriodEntries`, so a burst arriving inside one
  period is seen within it. Actuation still obeys `adaptivePublishActuationMinIntervalMillis` and the
  sustained-periods gate: an early observation brings the next change forward to the minimum interval, it does
  not bypass the gates.
...
- `currentBudget()` reports what the client is enforcing and nothing else: it changes when an update is
  applied, is unaffected by which local window happens to be binding, and neither blocks nor reserves. It is
  **empty** — not zero, and not a sentinel rate — wherever there is no broker-derived rate: against an old
  broker, against one with enforcement off, and after an expiry that left no fallback. The against-old-broker
  case is asserted explicitly, since it is the normal state for a client that adopts the API early.
...
- For a producer the scope actually granted a fallback, expiry moves the rate to it in **one step from either**
  side — one cut to zero recovers, one above it comes down — with burst capacity at one minimum viable unit, a
  positive token level **clamped to that capacity** exactly as a superseding update clamps it, and a negative
  one retained. The discriminating assertion is that a producer whose bucket was full at expiry can send its
  next entry **immediately** rather than after a validity period, which is what clamping buys over discarding
  and is the recovery-without-a-further-message property the TTL is justified by. The step is asserted as exact
  rather than as a
  direction, because an unspecified convergence is what lets two client implementations diverge. A producer
  that never held a budget, or one the scope declined to fund because its committed fallbacks had reached
  `adaptivePublishFallbackReserveShare` against the minimum reachable budget, has no
  fallback and is governed by the ordinary path; the test asserts which producers are in which case rather
  than assuming all of them recover.
- Standing fallbacks stay fundable across a reduction: a scope that commits fallbacks at `f = 1` and is then
  driven to `f_min` still has a non-negative allocatable rate, because the commitment was sized against the
  minimum reachable budget rather than the one current when it was granted.
- Settlement is applied at most once per send and from whichever response arrives: a deduplicated message
  refunds its entry through its receipt, a pre-storage rejection through `CommandSendError`, and an ambiguous
  persistence failure refunds nothing. A retransmission after reconnect does not refund twice.
- The stall detector reduces `f` when every ordinary pressure term is below one — the case where the ordinary
  multiplier would be at least 1 — and does not raise it.
...
- `stage()` fails on a producer with no finite `maxSendBlockTime`, before consuming the builder, rather than
  accepting a message it may wait on forever — and a send timeout does not substitute, because `sendTimeoutMs`
  does not cover the admission wait. A staged producer whose connection is down therefore does not wait
  indefinitely.
...
- Third-party `Producer` and `TypedMessageBuilder` implementations that do not override the new methods still
  compile and run, report `supportsAdmissionControl() == false` and `supportsStaging() == false`, and refuse
  rather than silently mis-implementing a contract — refusing **by return type**: the synchronous additions
  throw `UnsupportedOperationException`, while `Producer.whenSendable()` returns a future completed
  exceptionally with it. The second half is asserted specifically, because a synchronous throw from a
  `CompletableFuture`-returning method is invisible to a caller that attached `.exceptionally(...)`.
- The two capability queries are independent: an implementation that supports admission control but not
  staging reports `true` and `false`, and an application checking only the query it needs is not misled.
...
traffic already justifies, and configured rate limits remain as ceilings above the adaptive budget wherever
the two are the same currency — which is the byte dimension; a configured message rate can still bind first.
...
absorb a decision in flight, and charged against **the grant each send says it was admitted under** rather
than against whatever the newest grant is, so a client transmitting work it legitimately accepted earlier is
not enforced against — and is then paused with `setAutoRead(false)`, exactly as today. A client that never
advances that field is not thereby exempt: work stamped with a superseded sequence is honoured only up to what
that grant could ever have funded, after which it is enforced against the current one. An advisory percentage alone cannot protect a broker,
and the proposal does not rely on one.

**What the backstop does to liveness is easy to get wrong in both directions.** A paused connection is one the broker has stopped reading from, so it does not read the client's
...
*What should be voted on, and in what order* gives the sequence and the reasoning; this section gives the
default-value mechanics within it. Every part is staged, and each stage is decidable on a metric the previous
stage shipped.
...
its own history before anything changes.

**Approving this proposal does not authorise the default flip, and an earlier draft promised one on a release
schedule.** Enforcement ships `false` and stays `false` until a separately reviewed decision says otherwise;
elapsed releases and shadow metrics do not make that decision on their own. The reason is stated in *Goals*
and has to bind here rather than being softened in restatement: a controller computing a budget nobody
enforces never sees clients react, so shadow running validates the estimator and nothing about the loop.

**The criterion is a parameter sweep on a closed loop, and the sweep is a floor rather than a sufficient
condition.** Breakwater's evidence for its own controller is not a stability proof but a demonstration that it
behaves across a range of its constants; the same shape applies here. Before the controller is enabled by
default the project should be able to show, on at least two workload shapes and with clients actually
enforcing, that varying the target delay by one step and each of the two gains by half and double leaves the
loop holding its target without hunting. A controller that works only at its shipped constants is a controller
nobody can tune in an incident. The five open behaviour questions in *Goals* belong to different decisions and
should be answered against the stage each one gates, not collected into this one.
...
restore: a reverted broker simply falls back to it sooner. Three cases must not be conflated, and the client
distinguishes them by **negotiation, never by silence**. A budget that was never established at all —
because the handshake said the peer does not support or has not enabled admission control — leaves the
broker-derived rate absent, and the client is bounded by its own windows and by whatever the broker enforces
independently. Once a budget *has* been established on a connection, updates that stop arriving are a failure
rather than a permission, and follow the expiry rules exactly as written there: burst capacity drops to one
minimum viable unit, and a producer whose last update carried a fallback takes that fallback in one step.

**A producer that held a budget and was granted no fallback is the third case, and it is not the first.** The
scope may decline to fund one — its committed fallbacks having reached
`adaptivePublishFallbackReserveShare` against the minimum reachable budget — and such a producer then has no
broker-derived rate after expiry. That is the same *outcome* as never having negotiated, and it must not be
reached by the same *inference*: it is arrived at through the expiry rules on a connection where the feature
was negotiated, and it is visible on the wire, since the fallback fields were either present in the last
update or not. Silence alone still tells a client nothing, and is never read as permission by any of the
three.
...
Replicator producers publish through the same path and are subject to the same **adaptive** budgets, which is
the right answer for a reason worth stating: the adaptive budget is not a quota an operator sold but the
destination broker saying what it can absorb, and a replicator that ignores it congests the destination exactly
as any other producer would.

PIP-82 recorded an intention to exempt replication from resource-group rate limiting. That intention is not
what the code does today — `AbstractTopic.incrementPublishCount` calls `handlePublishThrottling` for every
accepted publish with no test for a remote producer (`AbstractTopic:1081-1082`) — so this proposal is not
departing from a shipped exemption, and the historical intent is noted rather than relied on.

Two specifics follow. A replicator disables its send timeout and bounds its pending messages instead
(`AbstractReplicator:157-158`), which is the shape this proposal generalises. And the replicator is the first
in-tree consumer of the new client API (*The application boundary*): porting it off
`ProducerImpl.isWritable()` is what replaces its TCP-only signal with one that can see the destination's
storage.

**Whether a budget-limited replicator lags or loses depends on which replicator it is**, and the distinction
matters to an operator reading their dashboards. A persistent replicator reads from a cursor, so a budget cut
turns into replication backlog — the right symptom, and operators should expect the backlog metric to move
where latency previously did — but it is not lossless in the limit: `GeoPersistentReplicator` deletes messages
whose TTL has expired rather than replicating them (`GeoPersistentReplicator:228`), so a long enough lag
becomes loss by expiry. A **non-persistent** replicator has no backlog to build and drops when it cannot write
(`NonPersistentReplicator:143`), so for it a budget cut is loss immediately, which is the existing contract for
non-persistent topics rather than something this proposal introduces.

One thing this proposal does **not** settle: whether a replicator's budget pressure at the destination should
feed the *source* broker's own congestion scope for the origin topic, so that origin producers see it. Doing so
would propagate backpressure across the replication hop, which is either the natural extension of an end-to-end
design or a way to let a remote cluster throttle a local one, depending on who you ask. It is out of scope and
named because the loop is asymmetric without it.

The **persisted message format** is unchanged, so mixed-version clusters interoperate. The negotiated
protocol additions do cross inter-cluster connections, because replication uses the ordinary producer
transport — so a mixed-version pair is compatible through negotiation rather than untouched by the change,
and a replicator to an older destination simply gets no budget.
...
What is taken from that family is narrow. The per-step floor of 0.5 is common to it, and the warning about which within-window statistic to use is worth heeding. The utilisation
...
validity period, no fallback, and no expiry transition to specify. Rejected because a duration is not something a client
...
that delegates to an existing method, and falls back to a capability query where no sensible delegation
exists. This proposal is in the second case, and this is the place to show that it is the second case rather
than a preference. A `trySendAsync()` delegating to `sendAsync()` would block on an implementation configured
with `blockIfQueueFull(true)` and could fail with the very capacity error the contract says it never produces;
a `whenSendable()` returning a completed future would assert capacity on behalf of a producer that never
agreed to bound anything. Both compile, both pass a signature review, and both break the guarantee the caller
was given. Refusing — synchronously where the return type is synchronous, through a failed future where it is
not — plus the two capability queries costs two extra methods and makes the gap visible; the alternative hides
...
**Kept — configured rate limits.** The adaptive budget constrains below them in the byte dimension, where *Reference* caps its growth against the configured ceiling; in the entry dimension the two are not commensurable and a configured message rate can bind first, which is stated as a residue rather than claimed away (*General Notes*). An operator who has sold a rate keeps selling it; what changes is that the broker may allow less when it genuinely cannot serve more.
...
[#26469](https://github.com/apache/pulsar/issues/26469) (a double release that permanently *under-counts*
usage — with cumulative excess `D` the limiter reads roughly `actual − D`, so admission still refuses once real
usage passes `limit + D`, and what repeated drift removes is the limit's usefulness rather than its existence),
and a third found while writing this: every chunk of a chunked message carries the whole
...
the protocol stage is voted on**, and this document states a proposed disposition rather than leaving the whole
question open. PIP-385 proposed adding rate-limit semantics to the protocol, a client-side backoff and an
application-visible throttled indication; its pull request has been open since October 2024 and is not merged.
This proposal adopts its central position — an explicit, negotiated broker-to-client signal rather than an
inferred one — and adds what makes it act on something: a budget derived from measurement rather than from
configuration, and an application-facing surface to respond to it.

**The proposed disposition is that this is an alternative to PIP-385, not a functional superset**, and saying
so is more useful than asking the list to choose between words. The two differ in what the signal *is* — an
absolute rate with a validity period here, a throttling indication there — and, more importantly, in what they
change about the **configured** limiters. PIP-385's motivating case is a cooperative client avoiding the
connection-wide pause that a configured topic or resource-group quota triggers. This proposal explicitly keeps
those limiters and their existing enforcement: the adaptive budget is a second constraint *below* them, and a
producer that breaches a configured rate still has reads paused on its connection exactly as today, with the
blast radius *Motivation* complains about. That gap is real and this proposal does not close it. Extending
negotiated budgets to carry configured-quota enforcement as well is a natural follow-up and is not proposed
here, because it changes the behaviour of a shipped, operator-configured mechanism.

So superseding PIP-385 would leave its case unaddressed rather than absorb it — a distinction worth keeping,
since PIP-385 is unmerged and that half is unimplemented, so nothing shipped is lost either way. The question
for the list is therefore narrower than "supersede or reconcile": it is whether the configured-limiter half
should be folded into this proposal's protocol stage, kept as separate follow-up work, or pursued
independently. It gates stage 5 and nothing earlier; client-local admission does not depend on the answer.

There is one residue this proposal must not leave implicit, because it would surface as a bug report. The
adaptive budget is below the configured limiters only where the two are the same currency: *Reference* caps
growth against a configured **byte** rate, and nothing caps it against a configured **message** rate, because
the adaptive entry dimension and a configured message rate are not commensurable. A deployment that sets
`maxPublishRatePerTopicInMessages` can therefore still have a producer paced to an entry budget its message
limiter refuses. Closing that is the same work as folding in PIP-385's half.
...
retry — which is the retry spiral KIP-219 documents. A budget is schedulable; an error is not. And there is a
second reason specific to the shape: an error rides on a message, so it can only reach a producer that is still
sending — while the case that matters most is telling a producer that has *stopped* that it may resume.

The honest qualification is that DCTCP shows a *sequence* of one-bit marks can be turned into an estimate of
congestion extent, so a richer signal does not strictly require a number in every message. What it also shows
is how much machinery that reconstruction costs, and Pulsar can send the number instead.
