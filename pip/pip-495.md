...
Replicator producers publish through the same path and are subject to the same **adaptive** budgets. Note that
this is deliberately unlike the configured limiters: PIP-82 excluded replication from resource-group rate
limiting, on the reasoning that an operator's tenant quota should not throttle the cluster's own replication.
That reasoning does not transfer here, because the adaptive budget is not a quota an operator sold — it is the
destination broker saying what it can absorb, and a replicator that ignores it congests the destination
exactly as any other producer would.

Three specifics follow. A replicator disables its send timeout and bounds its pending messages instead
(`AbstractReplicator:157-158`), which is the shape this proposal generalises. A replicator that is
budget-limited **lags rather than fails**, so replication backlog becomes the visible symptom of a congested
destination cluster — which is the right symptom, and operators should expect the backlog metric to move where
previously latency did. And the replicator is the first in-tree consumer of the new client API (*The
application boundary*): porting it off `ProducerImpl.isWritable()` is what replaces its TCP-only signal with
one that can see the destination's storage.
...
Nothing crosses the replication wire format, so mixed-version clusters are unaffected.
