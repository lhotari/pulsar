...
**Proportionality holds over a band, not everywhere**, and the band's width is `1 + 0.5/β`. With `β = 0.2` the
...
producer is what stops one idle producer's slack from being read as headroom for the whole scope. The
constant is chosen rather than inherited: the adaptive-concurrency limiters use a utilisation guard of the
same shape, but they gate on in-flight concurrency against a concurrency limit, which is not the same
quantity, and this proposal should not borrow their number as though it were.
