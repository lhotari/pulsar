...
connection. Others exist for other purposes — a backlog quota can disconnect a producer outright
(`BacklogQuotaManager:131`) — but none of them tells a producer how fast it may go, which is the gap this
...
- **Consumers charge the same controller and must hold accounts too.** With
  `autoScaledReceiverQueueSizeEnabled` a consumer force-reserves its incoming buffers against the same counter
  (`ConsumerBase:1329`, `:1348`), so an allocator that only knows about producers would let a consumer consume
  capacity the design believes is reserved. Consumer queues get accounts on the same terms, keyed to their own
  topic; without that the invariant below is not an invariant.
...
**The ledger invariant, and the three rules that hold it.** At every point, including mid-transition:
...
**4. The receipt refunds what produced no entry.** Three cases do: a message the broker deduplicates, a publish
to a non-persistent topic, and a rejected publish. All three are known only at the broker, so the refund
travels on the receipt rather than being computed at the client — another reason the receipt is the natural
carrier for admission information. A scope whose traffic is entirely non-persistent has no entry denominator at
...
detector instead has its own threshold, `adaptivePublishStallMillis`, and its own action: when the oldest
outstanding operation exceeds it, `f` is reduced by the ordinary multiplicative step, and that reduction
**bypasses the minimum-sample gate and the sustained-periods gate** — a stall is precisely the state that
produces no samples, and would otherwise leave the controller sitting still while nothing completes. It
bypasses nothing else, and it never raises `f`. A period with no publishes at all likewise leaves `f`
unchanged unless the detector fires.
...
- **Expiry.** When `valid_for_ms` elapses with no newer update, the burst allowances lapse at once. A producer
  whose last update carried a fallback rate **converges to it**; one carrying none — because the scope had
  ...
  Where a fallback exists, convergence is to it and deliberately **not** to zero or to whatever was granted at
  creation. A creation-time budget is a healthy-cluster number that would be restored precisely when silence
...
never be the reason a gap appears in the stream.** The guarantee is conditional on that funding, and there is
exactly one way to lose it after creation: a reconnect that negotiates a smaller maximum message size raises
the chunk count the envelope implies, and so the reservation. If the client cannot fund the larger vector the
guarantee is **suspended** — `submitAsync()` then fails loudly rather than degrading into a refusing call —
and resumes when it can. An application that must not see that should pin its envelope below the smallest
maximum message size it will meet. The returned `SendSubmission` always carries the acknowledgement future, and carries a present
...
- `adaptivePublishFallbackReserveShare` — the share of a scope's rate budget it will commit to expiry fallbacks, separate from and additional to the newcomer reserve. **Default 5.** Once committed fallbacks would exceed it the scope grants no more, which bounds how many producers can hold a recovery guarantee and is the honest limit on it.
...
- Enforcement attributes each send to the grant its `admitted_under_sequence` names: work admitted under an
  ...
  delivering an update late and observing no enforcement for work already admitted. That exemption is
  **bounded** — a client that never advances the field is enforced under the adoption deadline, one validity
  period later, which is asserted separately.
...
- For a producer the scope actually granted a fallback, expiry converges the rate to it from **either** side —
  one cut to zero recovers, one above it comes down. A producer that never held a budget, or one the scope
  declined to fund because its committed fallbacks had reached `adaptivePublishFallbackReserveShare`, has no
...
- `stage()` waiting for admission is woken by a *failure* as well as by capacity, and does not accept the
  ...
  that point.
- `stage()` fails on a producer with no finite `maxSendBlockTime`, before consuming the builder, and a send
  timeout does not substitute — the check is on the admission deadline specifically, because `sendTimeoutMs`
...
advances that field is covered by the adoption deadline rather than exempt. An advisory percentage alone cannot protect a broker,
