...
`adaptivePublishResumeThresholdMillis` worth of budget, which is a low-water mark in the same sense as the
...
- `ProducerBuilder.maxSendBlockTime(Duration)`, `ProducerBuilder.adaptiveBatchingMaxDelay(Duration)`,
  ...
  `ClientBuilder.pendingSendHeapLimit(long)`.
...
- `adaptivePublishResumeThresholdMillis` — how much budget must accumulate before `whenSendable()` completes. **Default 16**, matching the equivalent already used by `AsyncTokenBucket` on the broker, which exists because resuming on the first available token flaps ([#23920](https://github.com/apache/pulsar/issues/23920), [#24001](https://github.com/apache/pulsar/issues/24001)).
