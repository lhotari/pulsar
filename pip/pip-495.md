...
`ServiceConfiguration:3239,3247,3281,3298`). No bookie latency, no pending-add depth, no journal state. **A
...
| **CoDel / FQ-CoDel** (RFC 8289, 8290) | Minimum sojourn over an interval, with a target and an interval | The statistic, the queue-empty handling, and the observation that a working queue controller erodes the delay signal a cooperative sender would infer from | CoDel drops packets; this controller cannot drop a publish, so its actuator is a budget rather than a discard |
...
| **Kafka** (KIP-219) | Retreated from throttling by delayed response; mutes the channel after answering | Acknowledge promptly and regulate future work explicitly; keep an involuntary backstop | Its signal is a duration, which cannot be scheduled against |
...
| **JDBC `executeBatch`** | Indexed outcomes for a staged group | `stage()`/`collect()`'s index-correlated outcomes | JDBC leaves post-failure continuation implementation-defined; here the successors have already been transmitted, so the outcome list must say what became of them |
| **Amazon Builders' Library** | Shed locally and cheaply; measure sojourn at dequeue | That shedding suppresses the signal a rebalancer needs, that the false-positive target is zero, and the objection to usage-proportional allocation | Its shed-order remedies — LIFO, TTL drops, sidelining — all reorder or discard, which per-producer publish order forbids |
| **Pulsar itself** | `PersistentReplicator` gates cursor reads on `isWritable()`; `AsyncTokenBucket` resumes on a threshold; PIP-322's producer turn queue; `AsyncSemaphore`'s head-served queue | All four, as existing implementations to follow rather than designs to invent | The replicator's signal is TCP writability alone, which is the narrowest available and cannot see the cluster |
...
bit, and the reason it is not enough is the reason DCTCP exists: a sender told only that there is congestion
can respond only with the same blunt reduction whatever the size of the problem. A rate is what lets a client
pace and an application plan.
...
- `adaptivePublishRefreshPeriodMillis` — the forced refresh interval regardless of suppression. **Default 1000**, one third of the default validity, because that is what actually makes a dropped update invisible: refreshing at *half* the validity means a single loss puts the next update at the expiry instant with no margin, and any delay at all expires the budget. Surviving `n` consecutive losses requires a refresh at `validity / (n + 2)`, so the default pair survives one loss with a full refresh period of margin and two only if delivery is instantaneous — which is the right place to sit, since a third consecutive loss is better treated as a failed control channel than as something to paper over.
...
(`PulsarHandler:116-121`) — but the **client** has no such exemption, because its own auto-read is on. At the
