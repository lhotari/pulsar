...
- **The within-period statistic has to be named, because the intuitive choice is wrong.** Netflix report from
  production that using the minimum observed latency in a window makes a gradient controller insensitive to
  exactly the tail the operator cares about, and a maximum makes it hunt on outliers. This proposal uses the
  **median** queueing delay over the control period, and skips the period entirely — leaving `f` unchanged —
  when fewer than `adaptivePublishMinSamplesPerPeriod` publishes were observed, rather than acting on a
  statistic drawn from three samples.
...
This is Breakwater's shape — central AIMD on measured queueing delay, decrease proportional to overload rather
than a blind halving, floored so one bad interval cannot collapse the budget — and Breakwater is the evidence
that AIMD is right *here*, where a server with complete local knowledge must nonetheless search for a capacity
it cannot compute. DAGOR is the evidence that the constants are survivable: it has run a 5% multiplicative
decrease and a 1% additive increase on queueing delay across WeChat's backend for more than five years, and the
increase step here is the same 1%. The increase is frozen while producers are
...
what stops one idle producer's slack from being read as cluster-wide headroom. Emergency protection is a separate path and is bound by neither floor.
...
QUIC is the direct authority for that choice, and it made it as an explicit correction to HTTP/2. HTTP/2's
...
- `adaptivePublishMinSamplesPerPeriod` — the minimum number of publishes in a control period for its median queueing delay to be acted on. **Default 20.** Below it the period is skipped and `f` is unchanged, rather than the controller acting on a statistic drawn from three samples.
