...
<!-- This doesn't sound very professional way to start. What does "change on evidence" even mean.-->
Pulsar does not lack limits. What it lacks is a limit that can change on evidence. **There is no path by
which a broker's measurement of its own load changes the rate a producer is allowed to publish at** —
that is the whole of it, it is checkable from the code cited below, and every problem in this section is
a consequence of it. Capacity has to be estimated before the traffic arrives, so it has to be bought
before the traffic arrives. When the estimate is wrong, the only response available is a connection-scoped
stop that transmits no information. The client's own bounds miss the resource that actually runs out, and
on the V5 client they do not engage at all.

### On a stock broker, nothing is limiting publishing

<!-- it's not that "nothing is limiting". The rate limits are off by default, but the other limits are enabled. -->
All four publish rate settings default to `0`, meaning off: `maxPublishRatePerTopicInMessages`/`InBytes`
(`ServiceConfiguration:1286`, `1293`) and `brokerPublisherThrottlingMaxMessageRate`/`MaxByteRate`
(`:1248`, `:1255`). What remains enabled is not rate limiting at all but two memory-occupancy guards: a
connection's pending publish requests (`maxPendingPublishRequestsPerConnection`, default 1000,
...
Those two do read the broker's condition, and they read the wrong end of it. They say nothing while
memory is filling, fire once it is already committed, and the second pauses **every connection sharing
the IO thread**. They are an out-of-memory guard, not capacity management. Between "nothing is wrong" and
"half of direct memory is pending", a stock broker holds no opinion at all about how much it ought to be
admitting — and no way to tell anyone.

So for the default deployment the argument is not that the limit is set badly. It is that there is no
feedback path of any kind, and therefore the only defence against a peak is to have bought enough
hardware that the peak is survivable. That is the overprovisioning this proposal is trying to remove.

<!-- mention that the current situation results in outages when there is very spiky traffic and broker & bookies are already loaded. The load balancer might try to react to the situation by shedding load, but this usually makes the situation worse for a period of time. -->

### Where a rate *is* configured, one number has to be right for every state the cluster is ever in

Where an operator sets a rate, it is a constant. `PublishRateLimiterImpl.updateTokenBuckets` builds its
token buckets from the configured rate and from nothing else; `ResourceGroupPublishLimiter` does the same
from its resource group's. Resource groups look like the exception and are not:
`ResourceQuotaCalculatorImpl.computeLocalQuota` divides the **configured** total across brokers in
proportion to measured usage — a distribution loop over a number an operator supplied, not a discovery
loop over what the cluster can serve.

That number then has to hold for every state the cluster is subsequently in: every workload mix, every
message size and batch shape, a bookie in recovery, a compaction running, a co-tenant arriving, a GC
pause. Those states do not have the same capacity, so one number is right for at most one of them.

- **Set near what a healthy cluster can serve**, and it does not fire in a degraded one — the only state
  in which it was needed.
- **Set at what a degraded cluster can serve**, and the difference is hardware standing idle in every
  normal hour.

Two honest qualifications. First, a configured rate is not only a capacity guess: elsewhere this document
calls it a ceiling a tenant has purchased, and both readings are true of real deployments — what follows
applies to it in its capacity-guess role, which is the role it plays when an operator is deciding how
much hardware to buy. Second, three of the four knobs are **per topic**, not broker-scoped, so N topics
each obediently under a conservative per-topic limit can still overload a broker together; only
`brokerPublisherThrottlingMax*` has broker scope, and it is the one most likely to be left at zero.

The project's own survey names the alternative and its bet: rate limiting "constrains tenant usage to
specified quotas, enabling accurate planning and **over-commitment of resources**" — the wager that the
quotas sold will not all be drawn at once. That cuts against the argument here as much as for it: quotas
are what let an operator *sell* more than the hardware, and this proposal should not pretend otherwise.
What the wager lacks is any way to notice itself going wrong in time to do something proportionate. The
same survey names the missing piece while arguing against PIP-310 — custom limiters "operate on a topic
or namespace, ignoring cluster-wide resource usage and contention" — and calls for "Pulsar cluster wide
capacity management".

<!-- this is very hard to understand and connect to what is being covered. -->
**No utilisation figure is offered here.** The project publishes none, and a single deployment's number
would prove nothing about anyone else's. The claim is narrower and checkable: the only two instruments in
existence are a constant an operator wrote down and an emergency stop. How much capacity a conservative
constant leaves unused is exactly what nobody can currently measure — which is why this proposal makes
that quantity observable before it makes it recoverable, shipping the queueing-delay and budget metrics,
enabled, one release before the controller that acts on them (*Upgrade*).

<!-- stop -> throttle reading. "pausing a connection" -> "pausing reading from a connection" -->
### When the estimate is wrong, all the broker can do is stop a connection

Overload arrives anyway — from a limit set too high, from a limit never set, from a co-tenant, from a
slow bookie. Every condition in *Background* then ends in the same place: `setAutoRead(false)` on a
connection. That instrument has connection resolution, and everything else about it follows.

**Its blast radius is a connection, and three of the five conditions are not.** `TopicPublishRate`,
`ResourceGroupPublishRate` and `BrokerPublishRate` describe a topic, a resource group and a broker; all
three are enforced by pausing a connection. With `connectionsPerBroker = 1`, an application producing to
twenty topics through one client has all twenty share one connection, so one topic breaching its publish
rate stops reads for the other nineteen and for every consumer on that client. A well-behaved tenant's
latency is decided by the worst-behaved topic it shares a connection with. This is
[#23403](https://github.com/apache/pulsar/issues/23403); [PIP-385](https://github.com/apache/pulsar/pull/23398)
documents the same problem. `IOThreadMaxPendingPublishBytesExceeded` is wider still, throttling every
connection on the IO thread (`ServerCnx:305-312`).

<!-- It -> The connection throttling by pausing reads -->
**It transmits no information.** The client sees latency and cannot distinguish throttling from a slow
network, a slow bookie, or a GC pause. Nothing the broker can do to a TCP connection changes that, which
is why this proposal adds a signal rather than a better way to stop bytes.

**And it cannot attribute cause, as its own metric demonstrates.** A per-topic publish-rate-limit counter
exists (`OpenTelemetryTopicStats:70`), fed by `ServerCnx.increasePublishLimitedTimesForTopics()`
(`ServerCnx:4471-4480`), which iterates **every producer on the connection** and increments the counter
on **every topic it finds** — culprit and victims alike. This is not a reporting bug that could be fixed
on its own: the metric cannot tell them apart because the mechanism it reports on cannot either.

This proposal does not remove `setAutoRead(false)`. It keeps it as the involuntary backstop that makes an
advisory signal safe to ship (*Security Considerations*). What changes is that it stops being the *first*
thing that happens, and that the decision preceding it has producer resolution.

### The client offers no third option

`blockIfQueueFull` forces a choice between discarding work and parking a thread. Neither suits an
asynchronous application: failing throws away work that could have been admitted a moment later; parking
defeats the point of an async API and can wedge a shared executor. There is no way to say *"tell me when
you have room; I will decide what to do meanwhile."* This is
[#26343](https://github.com/apache/pulsar/issues/26343). Parking has produced real deadlocks: a send
issued from a client IO thread parks that thread on memory only that same thread can release
([#26344](https://github.com/apache/pulsar/issues/26344)).

### Heap retention is neither bounded nor accounted

`memoryLimit` bounds direct memory. Nothing bounds the heap a pending send retains, and because the
direct charge counts payload only, a producer of small messages exhausts heap long before it approaches
its direct-memory limit. Measured on a heap dump of a client that died this way: the limiter read
**92,335,360 of 209,715,200 bytes — 44% of its configured budget — when a 2 GiB heap was exhausted**,
with 1,285,311 sends outstanding and **not one message acknowledged**. Even had every outstanding send
been charged, the total would have been 164 MiB against a 200 MiB limit: the direct-memory limit could
not have engaged, because direct memory was never the resource running out.

### On the V5 client there is no boundary-1 backpressure at all

`ScalableTopicProducer.sendInternalAsync` appends to a per-segment dispatch chain and returns, so a
caller that does not await the returned futures is not slowed while the chain is behind — whatever the
underlying producer's limit is. When that limit does engage, neither outcome is usable backpressure. With
the default `blockIfQueueFull(false)` the failure surfaces asynchronously on the returned future, long
after the caller has moved on — and on V5 it is `MemoryBufferIsFullError` rather than
`ProducerQueueIsFullError`, because `ProducerBuilderV5` never sets `maxPendingMessages`, so the
pending-message semaphore is never constructed (`ProducerImpl:216-220`) and the message-count error is
unreachable. With `blockIfQueueFull(true)` it is worse: once the chain has caught up,
`appendToDispatchChain` runs the dispatch inline on the calling thread **while holding the dispatch
lock**, so the caller parks holding a lock every other producer thread needs — which is #26344. The
absence of boundary-1 backpressure on V5 is
[#26470](https://github.com/apache/pulsar/issues/26470).

<!-- Mention something that the current MemoryBufferIsFullError and ProducerQueueIsFullError are hard to handle in an application in a correct way assuming that the application is performing strict ordered messaging where message order has to be kept. There might be multiple messages inflight. To retain order, the only sensible way for the application would be to retry sending, however due to race conditions, it's not possible to consistently get ordered messaging since the application might be performing sendAsync calls in a loop and the errors are delivered on another thread. A later delivery can already succeed before it would start retrying from the first failed message. Therefore the current sendAsync API and it's error model is problematic. 

Revisit the goals so that this is captured.
...
- **Client-side adaptation that reduces the load rather than merely rationing it.** A budget denominated in batches lets a client answer pressure by forming fewer, larger entries — spending latency the application has declared it can spare, to reduce the per-entry work the broker and the bookies actually pay.
<!-- the memory limit is fine, but the way it's distributed across multiple producers is a problem. 
A single (or few) producers might be delayed and take a large proportion of the limit and this memory will be kept used until the message have been sent out. This problem didn't exist with the maxOutstandingMessages solution since it was per-topic and would balance the amount of messages in flight. One possible solution would be to have a dynamic limit of the memory for each topic which gets shared across active producers. 
 -->
...
- **A fairness or entitlement policy** — meaning a rule about what a tenant is *owed*. Three different
  things travel under the word "fairness" in this document and only the first is delivered: *isolation*,
  that a badly-behaved producer cannot deny service to a well-behaved one, which the admission arbiter
  and proportional shedding do provide; *proportionality*, that everyone's share falls by roughly the
  same fraction, which they also provide; and *entitlement*, that a tenant is due some share regardless
  of what it has been using, which they do not and which needs an authenticated allocation identity. See
  *Fairness*.
...
<!-- the resource that needs to be measured is the broker and the bookies serving a topic (active ledger for a managed ledger). The bookkeeper API's LedgerHandle contains getLedgerMetadata() method which has getEnsembleAt(int entryId). With getEnsembleAt(Integer.MAX_VALUE-1), it's possible to get the most recent ensemble's BookieId instances 
There's already registerLedgerMetadataListener in LedgerManager. There's also org.apache.bookkeeper.proto.BookieClientImpl#lookupClient and then org.apache.bookkeeper.proto.PerChannelBookieClientPool#getNumPendingCompletionRequests to find out the queue depths. It looks like the key for org.apache.bookkeeper.proto.PerChannelBookieClientPool#isWritable is to check if the ledger is writable on this bookie.
These details don't necessarily go in in this part of the doc, but it's just as inspiration for calculating better load factors for each bookie and each topic (managed ledger's active ledger) for deciding how to proportionally slow down traffic when queues are long or growing.
-->
...
<!-- the throttling is kept as the last resort when limits are exceeded and clients don't follow the budgets -->
...
matter. Both outcomes are *chosen*, by the party able to choose well, and neither is an outage. A shed message has not succeeded — but it was given up deliberately, locally, and for a stated reason, which is a different thing from a cluster falling over.
...
z < H  →  f ← min(1, f + a)                             only while producers are demand-limited
else   →  f unchanged
```
...
against it. Emergency protection is a separate path and is bound by neither floor.
