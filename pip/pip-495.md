...
expires with nothing pending by construction and cannot fail work already accepted. What bounds that work's residence is the client's own
windows, which stop it accumulating. An application that also wants a bound on how long it *waits* to add more
...
recover on expiry without receiving anything. That is what keeps the hot path free of round trips, and it is
why the two things the broker cannot verify — the demand hint and the admitting sequence — are structured so
that lying about them gains a client nothing.
...
recomputed from measurement thereafter. A producer that has been idle for an hour is not *penalised* for it: it
...
next message from its source. This is a different predicate from `SendSubmission.whenSendable()`, which
additionally requires the reservation restored.
