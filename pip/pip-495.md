...
is reset on retransmission after a reconnect — a message that has been re-sent is being delivered afresh. For a
chunked message it starts at the first chunk and covers the whole logical message, since a partial message is
not a delivery. Accepted work that has not yet been transmitted has no delivery clock running; it is covered by
the admission deadline until it is sent, which is precisely the separation this table exists to make. On expiry
it fails **that operation and the ones behind it on the same producer**, because a gap in the middle is the
outcome *Motivation* rules out — the scope is the same as today's, and what changes is when the clock starts.
