...
So it completes on `sendResumeThreshold` worth of budget. That is a low-water mark in the same sense as the
replicator's 90%-of-window threshold and Netty's low watermark, and the interesting observation is that Pulsar
has independently grown three of them. (One nearby mechanism is worth not confusing with it: tokens are
recomputed only every 16 ms by default, `AsyncTokenBucket.DEFAULT_ADD_TOKENS_RESOLUTION_NANOS`, which is a
measure against CAS contention rather than a resume threshold.)
...
This proposal therefore does not specify a new admission primitive; it specifies using that one on the client,
and the work is to move the producer path onto it rather than to design it.
...
- Completed-operation latency stops producing samples exactly when storage stalls, so an
  ...
  makes the controller a max-of-max that any single slow operation pins. It is a **stall detector** with its
  own, much larger threshold (`adaptivePublishStallMillis`), and it exists to keep the controller reducing
  during a stall that produces no completed samples at all, not to modulate `f` in normal operation.
- **The within-period statistic is the minimum, and that choice is the algorithm rather than a detail.** CoDel
  ...
  Two properties fall out of that choice rather than needing their own machinery. A GC pause or one slow bookie
  response raises the maximum and the mean and leaves the minimum alone, because the drain that follows
  produces a near-zero sample — so the controller is immune to them without smoothing. And the burst
  allowances are safe for the same reason: a producer spending its burst creates a transient queue that drains,
  which the minimum does not see, whereas a producer that has outgrown its budget creates one that does not.
  A period in which no write was initiated at all has no sample; `f` is unchanged unless the stall detector
  fires.
...
survivable at the scope's involuntary limits, and Breakwater supplies the right shape for it. Its
overcommitment per client is `max((C_total − C_issued) / n, 1)`: the burst is a share of what the scope has
**not** allocated, divided over the producers present, and floored at one unit. That has the property a fixed
fraction of each producer's own rate does not — the aggregate cannot exceed the unallocated remainder no matter
how many producers arrive, because it is that remainder being divided — and it self-tightens as a scope fills
up, which is exactly when a correlated burst would hurt. The floor of one unit is what keeps a producer on a
tiny budget able to send a single maximum-sized batch, and is the same floor the overspend allowance needs.

The newcomer reserve is handled the same way rather than as a static percentage. A producer with no budget
sits in a FIFO, and is granted one minimum viable unit — one maximum-sized batch's worth — whenever the scope
has anything unallocated. Breakwater's own evaluation is the argument for the FIFO: a scheme that leaves
zero-window clients to be rediscovered by chance starves them under sustained load, and the queue is what
bounds how long a new producer waits by the number ahead of it rather than by luck.
...
So the update is carried as an optional field on `SendReceipt` whenever a receipt is due, and sent as a
standalone `ProducerAdmissionUpdate` only in the cases a receipt cannot cover: a producer with no outstanding
sends to acknowledge, a producer that has been cut to a rate at which its own receipts are too sparse to carry
a timely change, and permission to resume for a producer that has stopped entirely. Those cases are real —
...
- **Expiry.** When `valid_for_ms` elapses with no newer update, the burst allowances lapse at once and the
  ...
  given a formula whose inputs it cannot see. **The broker sets it per producer, to
  `adaptivePublishMinAdmissionFactor × that producer's allocated rate`, never to a fraction of the scope's
  reference** — the aggregate of the fallbacks is what a scope has to survive if every producer's updates stop
  at once, so it must be bounded by the same argument that bounds the aggregate of the bursts.
...
into effect — one varint on a frame already on the hot path, alongside the demand hint. The broker charges the
overspend allowance only against sends whose `applied_update_sequence` is at least the update being enforced.
A client that never acknowledges makes no progress toward compliance and is throttled on the ordinary path; a
client that acknowledges and then exceeds is unambiguously non-compliant. This is AMQP's `echo` in the same
role: a receiver that needs to know its flow state has actually been adopted has to ask the sender to say so.
...
opted into at producer creation**, by setting `submissionEnvelope`. A producer that does not set it reserves
...
- `adaptivePublishControlPeriodEntries` — a volume trigger that evaluates the loop early when this many entries have been admitted since the last evaluation, whichever comes first. **Default 2000.** DAGOR uses exactly this compound window — "every second or every 2000 requests, whenever either criteria is met" — because a time-only window cannot catch a burst that arrives inside one period, and a volume-only window never fires on an idle scope.
...
- `adaptivePublishStallMillis` — the age of the oldest outstanding operation at which the scope is treated as stalled, independently of the queueing-delay target. **Unset by default**, and deliberately much larger than the target: this is a stall detector, not a second pressure term.
...
- `adaptivePublishBudgetValidityMillis` — the `valid_for_ms` carried on each update, measured from the client's reception. **Default 3000.** The binding constraint is not that it be an order of magnitude above the control period but that it exceed the control period plus the broker's worst-case outbound queueing plus one round trip: expiry must mean "the control channel has failed", never "the broker was busy". A single dropped update being invisible follows from that rather than motivating it.
- `adaptivePublishReportSuppressionPercent` — how far a producer's allocated budget must move before an update is sent at all. **Default 10**, with a forced refresh at half the validity period. PIP-82 uses the same shape for resource-group usage reports, and it is what keeps the control traffic proportional to how much the answer is changing rather than to the number of producers.
- `adaptivePublishOverspendAllowancePercent` — how far a client may exceed its budget **after acknowledging it** before the broker pauses reads on its connection, as a percentage of the budget over one validity period. **Default 20**, sized to absorb a decision already in flight. It is **consumed per producer incarnation and does not refill**: an allowance that refilled every period would grant a permanently non-compliant client a permanent 20% overspend, which is the opposite of its purpose. It also has a **floor of one entry per producer per validity period**, and that floor is not optional: a compliant client is contractually permitted to accept one message past its budget, so on a small budget — one batch per second over a three-second validity is three batches, of which 20% is less than one — a percentage alone would have the broker punish a client for doing exactly what the API told it to.
...
- **The statistic is the minimum, and the tests that distinguish it from an average are the point.** A burst
  that arrives and fully drains within a control period does not reduce the budget, because some publish in
  that period waited nothing; a load that keeps the queue permanently above target does, even where the mean
  is identical. A GC pause on the broker does not reduce the budget for the same reason.
...
- The stall detector fires when no write is initiated for `adaptivePublishStallMillis`, in a period that
  produces no completed samples at all — the case the delay statistic cannot see.
...
- **The budget arrives on the receipt.** A producer with receipts in flight receives its updates as fields on
  `CommandSendReceipt` and no standalone command; a producer with nothing outstanding, and one being told it
  may resume, receive the command. An update delivered by both carriers is applied once, by `update_sequence`.
- `whenSendable()` completes on `sendResumeThreshold` worth of budget rather than the first token: a producer
  released at the instant one token appears does not immediately pause again. The test that matters is the
  transition count — a producer held just below its budget should pause and resume a bounded number of times,
