...
**The gate reopens on a threshold, never on the first token**, and Pulsar has already learned this the hard
way on the broker side. `AsyncTokenBucket.calculateThrottlingDuration()` deliberately waits until the bucket
holds a configured minimum — 16 ms worth of tokens at the configured rate — before it resumes a throttled
producer, and issues [#23920](https://github.com/apache/pulsar/issues/23920) and
[#24001](https://github.com/apache/pulsar/issues/24001) are what happens without it: resuming at the first
token lets a burst straight through and then throttles again, flapping. A `whenSendable()` that completed on
one available token would reproduce that on the client, one message at a time. So it completes on
`sendResumeThreshold` worth of budget, which is a low-water mark in the same sense as the
replicator's 90% queue threshold and Netty's low watermark — the pattern every implementation of this
eventually acquires.
...
One half of this is not new to Pulsar either. PIP-322 already gives the broker a queue "to track which
producer is given a turn to produce while the rate limit has been exceeded", so turn-taking under a shared
limit is an established pattern here with a shipped implementation to follow. What is missing is the client
...
- `sendResumeThreshold` — how much budget must accumulate before `whenSendable()` completes, expressed as a duration's worth of the producer's current rate. **Default 16 ms**, matching the equivalent `AsyncTokenBucket` already uses on the broker, which exists because resuming on the first available token flaps ([#23920](https://github.com/apache/pulsar/issues/23920), [#24001](https://github.com/apache/pulsar/issues/24001)). It is a client setting because the bucket it governs is the client's.
...
- `adaptivePublishControlPeriodMillis` — the control loop period, and a **ceiling** rather than a fixed cadence. **Default 100.** It must exceed the time for a decision to reach clients and change what arrives, or each adjustment acts on stale evidence.
...
that can work under a wide set of traffic patterns", and the project has a shipped example of what happens
otherwise — `autoScaledReceiverQueueSizeEnabled` is an adaptive client-side loop that has defaulted to `false`
since PIP-74, years ago. A feature nobody enables is worth nothing.
...
- `whenSendable()` completes on `sendResumeThreshold` worth of budget rather than the first token: a producer
  released at the instant one token appears does not immediately pause again, which is the flap
  `AsyncTokenBucket` avoids on the broker for the same reason.
