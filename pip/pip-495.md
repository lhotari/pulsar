...
**Should a rate-limited client batch harder?** Under a batch budget the rational client response to
being cut is to put more messages in each batch — fewer, larger sends — which is genuinely the adaptation
the broker wants, because it reduces cost per message. The byte dimension is what stops it running away:
bytes are charged per message and are invariant under repacking, so useful batch growth caps out where
the byte bucket binds, and a producer cannot manufacture throughput by repacking. **This proposal does
not enable it by default**, for three reasons that are easy to miss:

- **It can destabilise the loop.** Widening the batch window is client-side accretion whose duration is a
  function of the budget, and the budget is set by a controller whose period must exceed the time for a
  decision to change what arrives. Sparser arrivals lower measured delay, which raises the budget, which
  narrows the window again.
- **It worsens the resource this proposal exists to bound.** A wider window holds more messages in the
  batch container, charged against `memoryLimit` and the semaphore — except that on stock defaults there
  *is* no semaphore, so the only bound on the widened window is the direct-memory limit that *Motivation*
  shows is blind to heap.
- **Chunked producers cannot do it at all.** Chunks bypass batching and each becomes its own entry, so a
  producer whose messages chunk into `k` pieces is hard-capped at `B/k` and has no adaptation available.
  That is a consequence of choosing batches as the unit, and it is stated rather than hidden.
