...
*What the broker measures* is the controller. Everything else is consequence.
...
One argument for making the budget explicit is easy to miss and is the strongest of them. **A queue controller
that works destroys the evidence a cooperative client would otherwise have used.** This is FQ-CoDel's
observation (RFC 8290 §6.3): the whole purpose of the controller is to keep the queue short, so a client
watching latency to infer congestion sees a healthy system right up until it is refused. A well-behaved
application cannot back off on a signal the broker has successfully eliminated. Either the broker states the
budget, or cooperation is not possible at all.
...
- `SendSubmission.whenSendable()` completes when the producer can accept another submission, which needs
  **both** conditions: the reservation restored, *and* `sendResumeThreshold` worth of budget accumulated, or
  at minimum one entry's worth. The floor of one entry is not optional — at a low enough rate 10 ms is less
  than a single entry, and a threshold with no floor is a permanent wait. Pulsar's own bucket has exactly this
...
A routed producer reserves **once per underlying producer**, not once at the parent: a partitioned producer's
guarantee is per partition, and the reservation has to sit where the admission decision does.
...
- **Allocation runs off the per-message path.** Recomputing a max-min division on every acquisition would be
  ...
  Naming an epoch is not a protocol, so what is specified is the invariant it must maintain at every point
  including mid-transition: **`retained + unspent issued credit + reserved capacity ≤ budget`**. Holding that
  requires the credit ledger to define who owns unspent credit, how a revocation is acknowledged, and what
  happens to an acquisition that races the transition — such an acquisition is retried against the new epoch
  rather than admitted under the old one or failed. Demand here is a **quantity**, not the boolean used for
  the controller's admission-limited test: a max-min division needs to know how much an account wants, not
  merely that it wants something.
...
**The primitive to replace it with already exists in the tree.** PIP-442 added
`org.apache.pulsar.common.semaphore.AsyncSemaphore` and `AsyncDualMemoryLimiter` to `pulsar-common` for the
broker's topic-listing commands, and they have precisely the properties this section has been arguing for:
`acquire` returns a `CompletableFuture` rather than parking a thread, waiters are held in a queue served from
the head so a late arrival cannot overtake one already waiting, the queue is bounded and returns a distinct
`PermitAcquireQueueFullException`, acquisition takes a timeout and a cancellation predicate, and `update`
changes the size of a permit already held — which is what a shrinking ceiling needs and what a plain semaphore
cannot express. `AsyncDualMemoryLimiter` carries two limit types, which is the direct-plus-heap shape.

What it is **not** is the whole arbiter, and the difference should be stated so nobody plans on the wrong
thing. `update()` adjusts a charge and releases usage immediately when it shrinks
(`AsyncSemaphoreImpl:167-189`), which is the right operation for reconciling an allocation that really did get
smaller and the wrong one for reducing a *ceiling* while the bytes are still retained — the rule above forbids
exactly that. Its maximum is `final`. Its queue processing is scheduled rather than inline, so it is not an
immediate answer of the kind `trySendAsync()` needs. And `AsyncDualMemoryLimiter` selects one of its two limit
types per acquisition (`AsyncDualMemoryLimiterImpl:88-103`) rather than acquiring direct bytes, heap and a
pending position together, which is what a reservation vector requires.

So this proposal reuses that machinery for the queued, waiting half — which is the half with the subtle
concurrency — and still specifies a new arbiter above it for immediate admission, joint multi-resource
acquisition, per-account ceilings and the epoch. That is a smaller piece of new code than designing the queue
from scratch, and pretending otherwise would understate the work.
...
becomes one entry per chunk. So admission reserves an upper bound on how many entries the accepted work can
become — the key count for a key-based producer, the chunk count for a chunking one, one otherwise — and
closing a batch reconciles the reservation against what it actually produced, returning the difference. Without
that, "at most one message of overshoot" is not a bound on entries at all. That is why the budget is a rate with a burst rather than a
...
wrong place. Queueing delay is local by construction. On a service saturating at 750 QPS, DAGOR driven by a
20 ms queueing-delay threshold sustained the saturation throughput, while the same controller driven by a
250 ms response-time threshold settled at about 630 QPS (§4.1, §5.2). The practical consequence is the one
...
- **Bootstrap.** A scope with no history admits at its configured ceiling if one exists, and otherwise at an
  operator-set starting rate, with `f = 1`. The controller's first job is to find out whether that hurts.
- **Growth.** The reference is a high-water mark, and it rises on evidence: whenever `f = 1` and the scope's
  admitted rate reaches the reference while `z_delay` stays below the headroom factor, the reference is raised
  by `adaptivePublishIncreaseStep`. Demand at the ceiling with no queue is the only evidence that more is
  available, and this is the only path by which capacity is discovered.
...
or the involuntary guards fire first. The responsiveness requirement is the one that binds: **the time to halve
the budget must be well under the time for the broker's pending-publish buffer to fill at the maximum offered
rate**, because past that point the memory guard takes over and the voluntary mechanism has failed. At the
defaults that is two control periods against a buffer measured in tens of MiB, which holds with room to spare
— but it is the inequality an operator raising the control period must recheck.

**Proportionality holds over a band, not everywhere**, and the document should say which. With `β = 0.2`,
`1 − β(z − 1)` reaches the per-step floor at `z = 3.5` — a queueing delay of 17.5 ms against a 5 ms target —
above which every decrease is the same halving regardless of how far over the scope is. Inside that band the
response really is proportional to the overload; outside it the controller is Breakwater's floor doing the
work. Raising `β` widens the band at the cost of reacting harder to a small excursion, which is the trade an
operator tunes.

**Recovery is slow by design and the number should be visible.** At `a = 0.01` per 100 ms period, `f` climbs
0.1 per second, so a scope driven to `f_min = 0.05` takes about **9.5 seconds** to return to its full
reference — and only while producers remain admission-limited, so a scope whose demand has gone away stays
where it is. That is deliberate: the decrease direction is the one that protects the cluster and the increase
direction is the one that risks re-entering overload, so the asymmetry favours caution. An operator who finds
recovery too slow raises `adaptivePublishIncreaseStep`, and the metric that tells them is the budget gauge
against the queueing-delay histogram.
...
additive increase on queueing delay, across WeChat's backend, for more than five years. The increase step here
is the same 1%. The increase is frozen while producers are
...
default **0.5**, measured as the period's peak rather than its mean. That is the same guard and the same
constant every comparable adaptive-concurrency limiter uses, and measuring at scope rather than per producer is
what stops one idle producer's slack from being read as cluster-wide headroom.
...
costing two varints on a frame that is already on the hot path. It is advisory and unverifiable, so it is used
only where over-reporting cannot help the reporter: to distinguish "this producer has nothing to send" from
"this producer is being held back", which decides whether `f` may grow and whether an idle producer's share
should be lent out. A client that inflates it gains no capacity — allocation still follows admitted traffic —
it merely stops the scope from growing on its behalf. Where the hint is absent, the utilisation threshold above
is the fallback. Emergency protection is a separate path and is bound by neither floor.
...
The compounding is real and the mitigation is cheap — the control loop's phase is jittered per scope, so
brokers sharing an ensemble do not evaluate in lockstep, and the jitter is a **deterministic function of the
scope's identity rather than a random draw**, so a broker's phase is stable across restarts and two brokers
that happen to collide do not re-collide differently every minute — but the honest statement is that per-scope control
under-reacts to a shared bottleneck relative to a coordinator, and this proposal accepts that in exchange for
needing no coordinator.
...
DCTCP supplies the argument for why the signal carries a *degree* at all, and it is worth having because the
cheap counter-proposal to this whole design is a single "slow down" bit. Its diagnosis of TCP is exactly the
diagnosis of `setAutoRead(false)`: the sender "reacts to presence of congestion, not to its extent", so it can
only respond with the same blunt reduction whatever the size of the problem — and DCTCP's contribution was to
reconstruct a multi-bit estimate of the extent out of a stream of single-bit marks, because the extent is what
you actually need. Pulsar is in the fortunate position of being able to send the number directly rather than
reconstructing it, and the design should not throw that away.

QUIC is the direct authority for the absolute form, and it made it as an explicit correction to HTTP/2. HTTP/2's
`WINDOW_UPDATE` carries an *increment*, so every frame must be applied exactly once and a lost or reordered one
corrupts the window; QUIC's `MAX_DATA` and `MAX_STREAM_DATA` carry absolute cumulative limits, which makes
duplication and reordering harmless by construction — a receiver simply keeps the largest it has seen. That is
also the reason `update_sequence` is on the wire: with an absolute value the only thing a client needs in order
to be reorder-safe is a way to know which of two updates is newer.
...
So the burst is drawn from a finite scope-wide pool. A producer receives a burst grant — one minimum viable
unit, sized to a single maximum-sized batch — only when the pool can fund it, and waits otherwise. The pool is
what the scope's involuntary limits can absorb, and it is charged for outstanding burst grants, newcomer
grants and already-accepted-but-untransmitted work together, because those are the three ways capacity can be
committed without having been spent. The floor of one unit per grant is what keeps a producer on a tiny budget
able to send a single batch at all, and is the same floor the overspend allowance needs.

The newcomer reserve draws on the same pool through a FIFO rather than a static percentage. A producer with no
budget queues, and is granted one minimum viable unit when the pool can fund one. Breakwater's evaluation is
the argument for the queue rather than for the formula: a scheme that leaves zero-budget participants to be
rediscovered by chance starves them under sustained load, and a FIFO bounds how long a new producer waits by
the number ahead of it rather than by luck.
...
- **Incarnation.** Updates are scoped to the producer incarnation, and stale generations are discarded.
  ...
  **The broker's own state is keyed differently, and deliberately so.** A producer incarnation is something the
  ...
  those to the durable identity it already has, `(topic, producer name)`, and the incarnation addresses only
  *which connection state* an update applies to.
...
- **Expiry.** When `valid_for_ms` elapses with no newer update, the burst allowances lapse at once and the
  ...
  is safe. It is worth noting that every credit protocol whose grant does **not** expire — HTTP/2, AMQP,
  gRPC — has a documented permanent-stall failure mode in which a lost or ungenerated grant mutes the sender
  forever, recoverable only by tearing down the connection. A TTL with a defined fallback is what buys
  recovery without a further message from the broker, and that is the argument for it.
...
- **Replay after a reconnect is admitted, not exempt.** This is the largest un-admitted burst in the system
  and the draft previously said nothing about it. A producer that reconnects resends its whole pending queue
  (`ProducerImpl:2042`, `:2218-2249`), and "admission is decided at acceptance" would let all of it onto a
  broker that may be the reason the connection dropped. Replay is therefore paced by the producer's current
  budget on the new connection, or by the fallback rate until the first update arrives on it. The messages
  keep their order and their sequence ids; what they lose is the right to arrive all at once.
...
- **Surplus returns when demand falls.** The converse rule matters as much and is easy to omit. A producer
  whose demand hint drops below its allocation has the difference returned to the scope immediately, rather
  than holding an allocation it is not using until the next control period. Breakwater does this on every
  arriving request; here it happens whenever a hint arrives that is lower than the current allocation. Without
  it, a scope's budget is pinned by producers that have stopped needing it.
...
`CommandSend` therefore carries `applied_update_sequence`, the sequence of the newest update the client has put
into effect — one varint on a frame already on the hot path, alongside the demand hint. This is AMQP's `echo`
in the same role: a receiver that needs to know its flow state has actually been adopted has to ask the sender
to say so.

It is necessary and **not sufficient**, and the gap has to be closed explicitly rather than assumed away. A
client may legitimately accept work under update 10, receive a reduction as update 11, and transmit that
already-accepted work tagged 11 — the proposal forbids re-gating accepted work, so this is compliance, not
overspend. Enforcement therefore accounts against **the grant the work was admitted under**, carrying the
bounded liability of accepted-but-untransmitted work across a reduction, rather than against whatever sequence
the frame happens to carry. And a client can simply never acknowledge: for that, the broker applies an
**adoption deadline** of one validity period after which an unacknowledged update is enforced as though it had
been applied. Without the deadline, withholding the acknowledgement is a way to opt out, since the configured
rate limits default to off and the occupancy guards bound outstanding memory rather than compliance.
...
The fix is cheap and belongs with the metrics rather than in the balancer: **a broker must report its
suppressed demand alongside its usage.** `pulsar.broker.publish.admission.factor` is exactly that signal — a
scope sitting below 1 is a broker saying "this is what I am admitting, not what was offered" — and a balancer
that reads the score without it is comparing a throttled broker to an unthrottled one as though they were the
same measurement. Making the balancer consume it is out of scope here; publishing it, and saying why it
matters, is not.
...
- `ProducerBuilder.submissionEnvelope(int maxMessageBytes)` — the declared maximum logical-message size for
  which `submitAsync()`'s never-refuse guarantee holds. **This is what opts a producer into admission control**,
  and it is what the reservation is sized from; a producer that does not set it reserves nothing and behaves
  exactly as today.
...
The opt-in is therefore **client-wide as well as per producer**: `submissionEnvelope` opts a producer into the
guarantee, and the client's new accounting is enabled by a client-level setting that defaults off in the
release that introduces it. A client that enables neither behaves exactly as today. Claiming that a
producer-level opt-in preserves a shared pool would be wrong, and it is the kind of wrong that surfaces as a
regression report rather than as a review comment. Without that rule, subtracting a reservation
...
One new broker-to-client command, `ProducerAdmissionUpdate`, carrying an absolute budget for one producer incarnation: batch and byte rates, burst allowances, the fallback rates to converge to if it expires, a validity period, a monotonic update sequence and a reason. The same payload is added as an **optional field on `CommandSendReceipt`**, which is the carrier used whenever a receipt is due; the standalone command covers the cases a receipt cannot. Adding an optional field to an existing command is backward compatible by protobuf's own rules, and a client that does not negotiate the feature is never sent either form. It is sent only after feature negotiation, and it is idempotent — receiving the same update twice, or receiving one while idle, is defined and harmless.
...
measures*, and `applied_update_sequence`, which is what lets the broker charge its overspend allowance only
against traffic the client had already adopted the budget for. The first two are advisory and cannot increase
an allocation; the third can only ever restrict what the broker enforces against. All are sent only by a client
that negotiated the feature. **No new client-to-broker command is added.**

A client that does not negotiate the feature sends no hint, receives no update in either form, and is throttled
exactly as today.
...
- `adaptivePublishActuationMinIntervalMillis` — the floor on the interval between two changes to `f`. **Default 50.** The gains are defined per unit time and normalised to the actual interval when an observation triggers early, so a busy scope does not get a larger effective gain than an idle one purely because it produced more samples.
...
- `adaptivePublishStallMillis` — the age of the oldest outstanding operation at which the scope is treated as stalled and `f` is reduced, bypassing the sample-count and sustained-period gates. **Default 5000**, deliberately three orders of magnitude above the delay target because it detects a different condition. It is one of only two settings here that ships enabled without a measured basis, and the reason is that leaving it unset together with `adaptivePublishOutstandingOpsWatermark` would mean a storage stall is visible to the controller only through pending bytes — which is an occupancy signal that arrives late and is exactly what *Motivation* criticises the existing guards for.
...
- `adaptivePublishReferenceDecayPeriods` — how long a reference may go unreached before it decays toward the observed rate. **Default 600**, one minute at the default control period, long enough that a normal traffic trough does not discard a valid estimate.
- `adaptivePublishBootstrapRate` — the rate a scope with no history starts from, in batches and bytes, used only where no configured ceiling gives a better answer. **Unset by default**, in which case a scope bootstraps from its first observed healthy period rather than from a guess.
...
- `adaptivePublishNewProducerReservePercent` — the share of each scope's budget held back for new and returning producers rather than allocated by usage. **Default 5.** It funds the newcomer FIFO described under *Allocation*, which grants one minimum viable unit at a time rather than dividing the reserve among however many producers happen to be waiting.
- `adaptivePublishDemandHintEnabled` — whether the broker acts on the client's optional `pending_batches`/`pending_bytes` hint on `CommandSend` when deciding whether a scope is admission-limited. **Default `true` where the client negotiated it**, falling back to `adaptivePublishUtilisationThreshold` where it did not. The hint cannot increase an allocation, so a client that inflates it gains nothing.
...
- `pulsar.broker.publish.admission.false_positive` (counter; attributes: scope) — producer-periods in which a producer was budget-limited while the binding scope's own measured queueing delay was below target. Amazon's stated operational target for an admission-control system is a false-positive rate of zero, on the grounds that a system which sheds work it could have served will be turned off by the team that owns it. This is the metric that makes that target checkable here, and a non-zero rate is a bug in the controller rather than a fact about the workload.
...
- `sendResumeThreshold` is floored at one entry and clamped to the configured burst, so a producer on a very
  low rate or a small burst still resumes. The test that matters is the transition count — a producer held just
  below its budget pauses and resumes a bounded number of times, not once per message — and it must still pass
  at a rate low enough that the threshold's duration is shorter than one entry.
- A producer without `submissionEnvelope` reserves nothing, keeps exactly today's admission capacity, and
  throws from `submitAsync()` rather than degrading it to a refusing call.
- The demand hint cannot enlarge an allocation: a client reporting an inflated `pending_batches` receives the
  same share as one reporting honestly, and the hint changes only whether the scope treats itself as
  admission-limited.
...
**And the backstop has a time limit, which is not widely appreciated and bounds how much it can be relied on.**
A paused connection is one the broker has stopped reading from, so it does not read the client's keep-alive
pings and never answers them. The broker protects itself from its own pause — `PulsarHandler` skips the
keep-alive timeout when `isAutoRead()` is false, with a comment saying exactly why
(`PulsarHandler:115-120`) — but the **client** has no such exemption, because its own auto-read is on. At the
next tick of `keepAliveIntervalSeconds`, 30 s by default on both sides (`ClientConfigurationData:315`), a
client waiting for a pong closes the connection.

So a read pause held for much more than one keep-alive interval does not throttle a producer; it disconnects
one. And a disconnect is worse than the pause was, because the reconnected producer **retransmits its entire
pending queue** — which is why the two rules below are stated together rather than separately. Read throttling
is a backstop measured in seconds, not a state a broker can sit in, and any design that treats it as an
indefinite hold is relying on something that is not there.
...
**A latency gradient against a measured baseline, instead of an absolute queueing-delay target.** This is what
the adaptive-concurrency family does — Netflix's `Gradient`/`Gradient2` and Envoy's adaptive concurrency filter
all compare current latency to a minimum observed under no load, and move the limit on the ratio rather than on
a threshold. The attraction is real: a ratio needs no operator to know what "slow" means on this hardware, and
it tracks hardware and workload drift for free. It is rejected here for three reasons.
...
exists to detect. Second, a baseline has to be re-probed or it goes stale, and probing means deliberately
driving the system to a low limit: Envoy pins concurrency to `min_concurrency`, default 3, for 50 requests
every 60 seconds. A proxy can afford that because another proxy is serving; a broker that owns a topic cannot
stop admitting to it in order to recalibrate. Third, Pulsar has something the general case does not — a
publish SLO the operator already reasons about, and a queueing delay that is a component of it, so the target
is derivable rather than guessed.

What is taken from that family instead is its guard rails, which apply whatever the signal: the 50%
utilisation gate before increasing, the per-step floor of 0.5, and the warning about which within-window
statistic to use.
...
means one unit of work, and a publish does not: a `CommandSend` can be one entry or, chunked, several, and its
cost in bytes varies by four orders of magnitude across real workloads. Breakwater's requests are homogeneous
...
windows are missing but that nothing tells the application what rate it may sustain. Recorded here because a
reviewer who prefers the window should be answered with a reason and not with silence, and because if the
per-credit sizing problem is ever solved the window is the better mechanism.
...
**This proposal is not new work; it is a sub-PIP of an accepted one.** PIP-460 lists in scope "improve the
**producer protocol** to support application-level flow control, removing the dependency on TCP/IP flow
control" (`pip/pip-460.md:54`), and is explicitly a parent PIP whose areas are to be covered by dedicated
sub-PIPs. That is the commitment this document discharges for the publish path, and it should be read against
that mandate rather than as an independent proposal.
...
`ServerError.Throttled` on the send, and the reason is the same reason DCTCP needed more than a mark: an error
on a send says *that* the producer was refused, not *at what rate* it may proceed, so a client can only respond
with an undirected backoff and a retry — which is the retry spiral KIP-219 documents. A budget is schedulable;
an error is not. There is a second reason specific to the shape here: an error is attached to a message, so it
