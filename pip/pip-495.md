...
**A routed producer reserves once at the parent, and lends it.** Reserving once per *underlying* producer
would be both wasteful and unfundable: a partitioned producer would reserve a hundred envelopes to make one
guarantee, and a V5 producer routing to a segment created on demand (`ScalableTopicProducer:374`) or a topic
gaining partitions could need a reservation the client can no longer fund, long after creation succeeded.

The contract makes the cheaper answer available, for a reason worth stating carefully. Many
ordinary submissions can be outstanding at once — the pump example pipelines them, each returning an empty
`whenSendable()` — so it is not true that only one submission is in flight. What is true is that **at most one
of them is reservation-backed**: the reservation is spent only by the submission that finds the ordinary budget
exhausted, and that submission also closes the gate, so no second one can be accepted until the signal
completes. One reservation per parent is therefore exactly enough, lent to whichever underlying producer that
one submission routes to and returned when it is accounted for. A new partition or segment therefore needs no new funding, and the number of
...
- **Allocation runs off the per-message path.** Recomputing a max-min division on every acquisition would be
  ...
  **`retained + unspent issued credit + reserved capacity ≤ budget`**, and four rules establish it:
