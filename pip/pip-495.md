...
re-gates a send **on the connection it was admitted on**. Replay onto a *new* connection is the one exception,
for the reason given under the lifecycle rules: the grant it was admitted under belonged to a broker
relationship that no longer exists. The technique is not unusual: Amazon describes the same pattern for requests whose cost is
...
over. The admission deadline does not cover it either: `maxSendBlockTime`
expires with nothing pending by construction and cannot fail work already accepted. The client's own windows bound how much such work can
*accumulate*, not how long any of it sits there — a distinction worth keeping, because a bound on quantity is
not a deadline. An application that also wants a bound on how long it *waits* to add more
supplies one itself, on the readiness future; `maxSendBlockTime` does that job for the blocking paths and does
not apply to `submitAsync()`. On expiry the delivery deadline fails **that operation
