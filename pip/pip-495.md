...
cheap counter-proposal to this whole design is a single "slow down" bit. Its diagnosis of TCP is the diagnosis
of `setAutoRead(false)`: a sender that learns only the presence of congestion and not its extent can only
respond with the same blunt reduction whatever the size of the problem. DCTCP's own answer was to *reconstruct*
the extent from a stream of one-bit marks — which proves the extent is what matters, and equally that a
numerical field per message is not the only way to convey it. The argument for sending the number is that
Pulsar can, cheaply, on a frame it was already sending, and so does not have to build the estimator.

QUIC illustrates the value of the absolute form, and the comparison needs stating more carefully than an
earlier draft managed. HTTP/2's `WINDOW_UPDATE` carries an *increment* and is perfectly safe on the reliable,
ordered transport it runs over — increments are not lost, and their sum does not depend on their order — so
the objection to incremental credit is not that it is broken but that every frame becomes load-bearing, which
constrains what may carry it and how it may be resent. QUIC's `MAX_DATA` and `MAX_STREAM_DATA` carry absolute
cumulative limits, and duplication or reordering is harmless by construction: a receiver keeps the largest it
has seen.

**This proposal needs one thing QUIC does not, and that is why `update_sequence` exists.** QUIC's limits are
monotonically increasing, so "keep the largest" is a complete rule. A publish budget must be able to go *down*,
so largest-wins is unavailable and supersession has to be carried explicitly. That is an addition to the
design rather than something inherited from it.
...
The distinction between that and "a number" is worth keeping honest. The application-facing API deliberately
exposes admission and readiness, not a rate: an application that branched on a numeric budget would be
reimplementing the client's pacing badly and would couple itself to a value that changes every few seconds.
The number exists — it is what the client enforces and what `PublisherStats` reports for operators — and
`Producer.currentBudget()` returns a snapshot of it for the applications that genuinely need to plan, such as
one sizing a read batch from its source. It is documented as advisory and immediately stale, for the same
reason `whenSendable()` is.
