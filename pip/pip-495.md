...
**One optional client-to-broker field is added**, and it is a field rather than a command: `pending_batches`
and `pending_bytes` on `CommandSend`, the demand hint described under *What the broker measures*. It is
advisory, it cannot increase an allocation, and it is sent only by a client that negotiated the feature. No new
client-to-broker command is added.
