...
**`AuthenticationFactory.tls(certFilePath, keyFilePath)`.** The v5 `AuthenticationFactory` keeps a material-carrying convenience entry point — `static Authentication tls(String certFilePath, String keyFilePath)` — for source-compatibility with the v4 `AuthenticationFactory.TLS(cert, key)` idiom. It is **not** an exception to "the auth plugin carries no TLS material": when the client also configures a `tlsPolicy(...)`, the supplied PEM cert/key are **folded into the `CLIENT_DEFAULT` `TlsPolicy` at build time** — per-field and **plugin-wins** (the cert and key override the policy's, while the policy's trust material and flags — hostname verification, protocols, ciphers — are preserved), the same fold the v4 `AuthenticationTls` bridge performs (see the [TLS override hook](#legacyv4authenticationadapter-v4--v5)); the transport then reads its material from the client TLS factory, not from the plugin. Only PEM file paths are accepted here (a keystore identity goes through `tlsPolicy(...)`). Without a `tlsPolicy(...)`, the legacy behaviour applies and the plugin supplies the transport material directly. *(Whether this overload earns its place alongside `builder.tlsPolicy(...)` — it partly re-opens the mTLS-material-in-an-auth-entry-point shape this PIP otherwise removes — is a candidate for the minimalism review.)*
...
The tier-1 case is one expression — `builder.tlsPolicy(TlsPolicy.pem(trust, cert, key))` — which binds the policy to `CLIENT_DEFAULT`, the purpose every client purpose except `CLIENT_OAUTH2` resolves to via fallback (`CLIENT_OAUTH2`'s empty fallback resolves to the system default instead, unless the OAuth2 plugin contributes its own IdP material). The tier-2 case binds a policy to a specific purpose — `builder.tlsPolicy(TlsPurpose.CLIENT_OAUTH2, idpPolicy)` — and unconfigured purposes fall back to `CLIENT_DEFAULT` (or, for `CLIENT_OAUTH2`, to the system default). `TlsPolicy` describes material *locations*, not the loaded material, so it stays a small serializable value; the loading, caching, and rotation are the internal `TlsMaterialSource`'s job.
...
**v5 client builder.** Gains `tlsFactory(PulsarTlsFactory)` to supply a custom factory instance (adopted — closed with the client), and `tlsPolicy(TlsPolicy)` / `tlsPolicy(TlsPurpose, TlsPolicy)` to configure one or more `TlsPolicy` values — each covering cert/key/trust paths or a keystore, ciphers/protocols, and hostname-verification / insecure-connection flags — bound to a `TlsPurpose` (the single-argument form binds to `CLIENT_DEFAULT`, which every client purpose except `CLIENT_OAUTH2` resolves to via fallback). The simple file-based case is a single builder-level value, subsuming the PIP-466-era experimental `TlsPolicy` (see the Detailed Design).
...
**Server/Client symmetry.** `SslContextFactory.Client` was initially left out of the well-known set — a special-case asymmetry with the Server class, justified by its single in-tree consumer (the proxy's `AdminProxyHandler`) always being served by framework synthesis. Making it first-class removes that asymmetry: both Jetty classes now follow the identical ask-factory-first-then-synthesize path, so a custom factory can customize proxy→broker admin TLS exactly as it can the web-server TLS. The tradeoff is a marginally larger SPI surface (one more well-known class a factory *may* implement) weighed against a consistent, symmetric contract plus that added customizability; the symmetry won, and it costs the default factory nothing — it returns `empty()` for both and the synthesized fallback (including the reloading rotation of the admin `HttpClient`'s material) is unchanged.
