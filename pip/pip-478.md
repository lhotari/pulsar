...
6. **Compatibility bridge for v4 plugins** in `pulsar-client` (package `org.apache.pulsar.client.impl.auth.v5`), one direction only — v4 → v5, because the client drives the v5 model natively and resolves exactly one v5 `Authentication` per client:
   - `LegacyV4AuthenticationAdapter` — wraps an arbitrary v4 `Authentication` instance as a v5 `Authentication` declaring the right capability for its style. v4 calls are always off-loaded to a dedicated blocking executor — `ctx.blockingExecutor()` when a client bound one, otherwise the framework's shared fallback pool; the Netty event loop never runs the v4 plugin's I/O.
   - `V5AuthenticationProvider` (in `org.apache.pulsar.client.api.v5.internal`) — the hand-over seam for a built-in v4 shim that already owns a v5-native body: the client drives that body directly instead of bridging the v4 surface.
...
`LegacyV4AuthenticationAdapter` wraps an arbitrary v4 `Authentication` as a v5 `Authentication`, declaring the capability interfaces that match the v4 plugin's style. All v4 calls are off-loaded to a separate, dedicated executor — `ctx.blockingExecutor()` when the owning client bound one, otherwise the framework's shared fallback pool — so the Netty event loop never runs v4 plugin I/O. v4 plugins that supply TLS material (`hasDataForTls() == true`, e.g., `AuthenticationTls`) are represented by the built-in `TlsAuthentication` plugin; their material reaches the client's `PulsarTlsFactory` through the owning component rather than through the bridge (see the TLS override hook below).

## `ClientCnx` drives the v5 authentication model

`ClientCnx` no longer drives a v4 `Authentication` at all. The client resolves exactly one v5 `Authentication` — configured through the v5 builder, handed over by a built-in v4 shim's `V5AuthenticationProvider`, or produced by bridging an arbitrary v4 plugin with `LegacyV4AuthenticationAdapter` — and wraps it in a per-client binary driver; `ClientCnx` opens one exchange against that driver per connection attempt and routes connect, refresh and challenge handling through it. There is no separate synchronous path: a plain v4 plugin reaches the connection only through the bridge, whose calls are off-loaded. This is generic across all challenge types.
...
public interface Authentication extends AutoCloseable {
     ...
     * implementation is ready to serve credentials. MUST NOT block the calling
     * thread: the framework may call this from an I/O thread and never waits
     * for the future there. Start the I/O and return promptly, off-loading
     * blocking work to ctx.blockingExecutor(). A failed future is not cached —
     * the next use retries.
     */
...
public interface AuthenticationInitContext {
     ...
     * per PulsarClient: a small bounded pool (max 16 threads, created on demand
     * and reaped after 60s idle) — a fixed framework default, not a
     * client-builder knob. Work beyond the thread ceiling QUEUES rather than
     * being rejected: every caller is a connection attempt or an authenticated
     * request, so a saturated pool must slow them down, not fail them (a
     * rejecting pool would turn a reconnect storm against a slow identity
     * provider into failed connections). Because the queue is unbounded, a task
     * running here MUST NOT block waiting on another task submitted here; no
     * framework path nests submissions — the v4 HTTP-lookup branch composes the
     * v5 future rather than joining it.
     */
...
**Fallback when no client services are bound.** Not every connection is opened by a `PulsarClient`: the proxy builds broker connections straight from a configuration, a `PulsarAdmin` binds only an HTTP client factory, and embedders and tests bind nothing. Those callers get a **process-wide shared blocking pool** (8 daemon threads, the same queue-don't-reject policy, threads reaped after 60s idle, not tied to any one client's `close()`) rather than running the credential call on the caller thread — which on those paths is a Netty event loop. A body that serves its credential from memory still resolves inline there, with no thread hop per connect; only work that can actually block is handed to the pool.

The lifecycle is: the framework binds one set of runtime services per `PulsarClient` and hands each authentication driver an `AuthenticationInitContext` over them, calling `initializeAsync(ctx)` once per driven plugin, and the implementation may retain references for the lifetime of the client. When the plugin is configured through the v5 builder it is initialized **eagerly at client build**, on the application thread, so a misconfiguration fails the build. The framework's own drivers — which also carry a v4 built-in plugin that delegates to a v5 body, where there is no such build-time hook — initialize **on first use** instead, and there the call may land on a Netty event loop: the framework does not wait for the future, it chains the connection's authentication rounds onto it. Hence the "must not block the calling thread" clause above; a body that does its I/O inline before returning the future would stall every connection sharing that event loop. Initialization failures are not cached, so a transient failure recovers on reconnect rather than poisoning the plugin for the client's lifetime. `Authentication.close()` releases any resources the implementation acquired; the framework owns and closes the shared services (HTTP clients, scheduler, blocking executor); the shared fallback pool used when nothing is bound is process-wide and outlives any one client, at no idle cost. Per-call `AuthenticationCallContext` instances are cheap to allocate.
...
**The admin client follows the same rule, for the same reason.** `PulsarAdmin` selects transport TLS by an `https://` service URL rather than a `useTls` flag, but composition is again not the same question as enablement: an admin whose OAuth2 plugin carries IdP TLS material (folded into `CLIENT_OAUTH2`, see the OAuth2 note) composes a factory even behind a plaintext `http://` admin URL, because otherwise the framework HTTP client that fetches the token has nothing to resolve `CLIENT_OAUTH2` against and falls back to the platform trust store — the same silent downgrade the client-side rule prevents. The admin transport itself stays plaintext.

**One factory per client, on both paths.** `initialize(...)` is called exactly once per `PulsarClient` / `PulsarAdmin`, and `close()` exactly once, however many internal transports share it — a `PulsarAdmin` in particular drives more than one HTTP connector. Custom factories may therefore hold per-instance resources (HSM sessions, rotation tasks) without defending against repeated initialization, and whoever hands a factory *in* — the broker's admin attach is the one in-tree case — transfers ownership with it and must not close it itself.
...
**Provider-name resolution (both axes, one mechanism).** A non-blank provider name is resolved in three steps: (1) a provider already statically registered in the JVM (`Security.getProvider(name)`); (2) the **`ServiceLoader`** mechanism (`META-INF/services/java.security.Provider`) on the **thread-context** class loader — falling back to `JcaProviders`' own class loader when the thread has none — matching on `Provider.getName()`; (3) a loud `IllegalArgumentException` when the name resolves to nothing, rather than a silent revert to the JVM default. The registered provider is preferred deliberately, because the two steps differ in *which instance* answers a name: `ServiceLoader` constructs a fresh instance through the provider's **no-arg** constructor, whereas `Security.getProvider` returns the instance the operator installed — which may carry provider-level configuration and is the object actually in the JVM's provider list. This is reachable for any provider that ships a `META-INF/services/java.security.Provider` entry; `bcprov` ships one, so a pinned `jcaProvider=BC` would otherwise resolve to a fresh provider rather than the registered one. BouncyCastle's **JSSE** provider needs a third step, because neither of the first two can reach it: `bctls` ships no services entry, so `BCJSSE` is invisible to `ServiceLoader`, and it cannot simply be default-constructed either — its FIPS mode is a constructor argument fixed at construction (`new BouncyCastleJsseProvider("fips:BCFIPS")`) while the provider registers under the plain name `BCJSSE` either way, so a no-arg instance would quietly be the non-FIPS one. When `BCJSSE` is the pinned name and nothing has registered it, Pulsar therefore constructs and registers it from the class path: in **FIPS mode bound to the FIPS JCA provider** when the resolved BouncyCastle JCA provider is `BCFIPS`, and in the default mode otherwise — so the TLS stack follows whichever BouncyCastle artifact the deployment actually ships, and a FIPS JSSE provider is never paired with a non-validated crypto provider. An instance the operator registered themselves always wins, whatever mode they built it in. `bctls` is not shipped in the distribution; pinning `jsseProvider=BCJSSE` without it on the class path fails loudly. The walk is defensive: a broken `META-INF/services/java.security.Provider` entry belonging to an unrelated provider is skipped rather than aborting resolution. The resolved provider is handed straight to the TLS layer and is never itself installed process-wide via `Security.addProvider`. Invoking the resolver initializes the `JcaProviders` class, which installs Conscrypt process-wide when it is present; Bouncy Castle is **not** resolved by class initialization — it is looked up lazily, on first use by a caller that needs it, so a deployment without Bouncy Castle on the class path can still resolve named providers. Resolution happens where the provider is first needed — when a purpose's material source is constructed and when its context is built — so an unresolvable name surfaces at client build / server start for the eagerly probed purposes (the fail-fast `CLIENT_DEFAULT` probe, a server listener at startup) and at first use for a purpose resolved lazily.
...
public abstract class LegacyV4AuthenticationAdapter implements Authentication {
     ...
     * A plugin advertising the {@code "tls"} auth-method name is represented by
     * the built-in {@link TlsAuthentication} plugin; its TLS material reaches the
     * client's {@link PulsarTlsFactory} through the owning component, never
     * through this bridge (see "TLS override hook" below).
     */
...
1. **Always offload to a separate executor.** Even calls to "fast" v4 implementations like `AuthenticationToken.getAuthData()` go through a dedicated blocking executor rather than the scheduler — `ctx.blockingExecutor()` when the owning client bound one, otherwise the framework's shared fallback pool — so a slow or blocking v4 plugin cannot tie up the scheduler's threads or the Netty event loop. A capability call made *before* `initializeAsync` fails fast with an `IllegalStateException` rather than running inline. The cost is a thread hop, but the simplicity is worth it: the adapter has no class-name allow-list, no per-impl heuristics, and no production hazard from a misclassified plugin.
...
### `ClientCnx` drives the v5 model (generic, not SASL-specific)

The v5 client wraps the v4 transport (per PIP-466), so the practical change is twofold:

1. `ClientCnx` is inverted: it no longer holds a v4 `Authentication` for the credential exchange but a driver over the single v5 `Authentication` the client resolved.
2. That driver — `org.apache.pulsar.client.impl.auth.v5.BinaryAuthenticationDriver`, an implementation type in `pulsar-client` rather than a public API type, since nothing outside the client implements or observes it — is **exchange-scoped**: it hands `ClientCnx` an `AuthenticationExchange` for one connection attempt, and every round of that connect is driven through the single exchange object:
...
public interface BinaryAuthenticationDriver {
    ...
    interface AuthenticationExchange {
        ...
        String authMethodName();                                           // valid once a round has completed
...
The driver type is an implementation type in `pulsar-client`; the only "stable internal" seam an integrator can encounter is `org.apache.pulsar.client.api.v5.internal.V5AuthenticationProvider`, which signals "framework-observed — application code should not implement this." `ClientCnx` calls `newAuthenticationExchange(brokerHostName)` once per connection attempt and drives that connect through the returned exchange — the initial credential via `getAuthDataAsync()` and every ordinary `CommandAuthChallenge` round via `authenticateAsync(...)`. The broker's REFRESH sentinel is **not** driven through the current exchange: `ClientCnx` terminates it and opens a **fresh** exchange whose `getAuthDataAsync()` re-produces the credential (binary routing rule 2), so conversation state does not survive a REFRESH. **Exchange scoping matches the state slot's lifetime.** An authentication's per-connection conversation state — for a bridged v4 plugin, one `AuthenticationCallContext` and its state slot — lives exactly one *exchange*, so the driver hands `ClientCnx` an object that *owns* that state for the duration of the exchange, rather than re-deriving it from a host argument on each call. (An alternative two-method, host-keyed shape — `getAuthDataAsync(String)` / `authenticateAsync(AuthData, String)` — was rejected: the bridge would allocate a fresh call context per call and so lose challenge-response state across rounds.) `ClientCnx.newConnectCommand()` and `ClientCnx.handleAuthChallenge(...)` both drive the current exchange unconditionally; a fresh exchange is created per connection attempt (and when `ClientCnx` restarts authentication on the REFRESH sentinel), so concurrent handshakes never share state. **Generic across all challenge types** — connect, REFRESH, SASL multi-round, custom challenge-response.

The async and sync handling are **one state machine, not two parallel paths**: credential resolution produces a `CompletableFuture<AuthData>` — already completed when the resolved authentication serves its credential from memory (`NoAuthentication`, `TlsAuthentication`, a warm token body), pending when it must fetch one or when a bridged v4 plugin's call is off-loaded — feeding a single continuation that performs every side effect the sync path performs today — assigning the connection's `authenticationDataProvider` (on connect *and* on the REFRESH sentinel), building the command, transitioning connection state, writing, and completing the connection future. The continuation runs inline when the future is already done, so a credential needing no I/O costs no thread hop, and hops to the channel's event executor only when it was pending; in that case it is additionally bounded by the client's operation timeout, so a hung plugin fails the connection instead of wedging the handshake. This single-assignment-site structure is a hard requirement: splitting into a *duplicated* async path risks silently dropping the `authenticationDataProvider` assignment. Two invariants are acceptance criteria, enforced by pre-existing tests on master: the connection's `authenticationDataProvider` observable (`getCommandData()`) must reflect the refreshed credential, and a broker-pushed refresh must not disconnect (`getLastDisconnectedTimestamp()` unchanged — see `TokenOauth2AuthenticatedProducerConsumerTest.testOAuth2TokenRefreshedWithoutReconnect`). Failures from the async path are unwrapped (`CompletionException` never leaks) to the matching v4 `PulsarClientException` subtype before failing the connection future.

Every connection is driven through one exchange; there is no separate synchronous path. A plain v4 plugin is bridged, so its credential call runs on the blocking executor and the continuation hops to the channel's event executor. The observable side effects are unchanged — the published `authenticationDataProvider`, the command build, the state transition, the write, the connection-future completion — but the v4 plugin's I/O no longer runs on the event loop, which is the point of Motivation #1.

After the v4 internal migration (in-scope item #7) completes, the **credential-fetching** built-in v4 classes (Token, Basic, OAuth2, Athenz, SASL) keep their v4 synchronous surface verbatim and **additively** implement `org.apache.pulsar.client.api.v5.internal.V5AuthenticationProvider`, handing the client their v5-native body. The client — not the shim — wraps whatever body it resolves in one `V5BinaryAuthenticationDriver` (in `pulsar-client`, `org.apache.pulsar.client.impl.auth.v5`) and drives every connection from it, so even users of the v4 `PulsarClient` get the off-loaded path automatically for the plugins where it matters (Motivation #1). The **no-credential-I/O** built-ins own no v5 body: `AuthenticationDisabled` resolves to the built-in `NoAuthentication`, and `AuthenticationTls` / `AuthenticationKeyStoreTls` are routed by their `"tls"` method name onto the built-in `TlsAuthentication`. Both serve an empty credential from an already-completed future, so the default no-auth and mTLS hot paths still run inline on the event loop with no executor hop and no gating wrapper — async buys them nothing and they pay nothing for it. The shared `BinaryAuthenticationExchange` implements the binary routing rules and the v5 → v4 exception mapping in one place, so v4 exception types — part of the public API — are preserved for v4 callers (see the Error model).
...
**The budget covers the v4 branch too.** Both HTTP transports hand the driver an explicit budget — the lookup client's `lookupTimeoutMs` and the admin client's `requestTimeoutMs`, each falling back to 60 s — and the driver bounds each stage against the remaining budget, surfacing exhaustion as an authentication failure naming the round. The v4 branch, taken when the plugin exposes no HTTP challenge capability, is not driven inline either: the whole v4 header composition is off-loaded to the blocking auth executor and composed with `thenCompose`, never joined, so a plugin whose `getAuthData()` refreshes a token cannot deadlock against the event loop that would serve that refresh.
...
- **After the v4 internal migration (in-scope item #7)**, each of those classes keeps its v4 synchronous surface **verbatim** and *additively* implements `V5AuthenticationProvider`; it does **not** route its existing sync methods through a v5 delegate. The v5-native body is produced once per client, during client construction, and its `initializeAsync(...)` is memoized and run lazily on first use inside the client's driver. The migrated `AuthenticationToken` illustrates the shape:
  ```java
  public class AuthenticationToken
          implements org.apache.pulsar.client.api.Authentication,
                     EncodedAuthenticationParameterSupport, V5AuthenticationProvider {
      ...
      // PIP-478, additive: hand the client the v5-native body it drives.
      @Override public org.apache.pulsar.client.api.v5.auth.Authentication v5Authentication() {
          // Read the supplier live, so a token rotated via configure(...) is picked up next connect.
          return new TokenAuthenticationV5(() -> tokenSupplier.get());
      }
  }
  ```
  So the v5-native body (`TokenAuthenticationV5`) is what the client actually drives, through the one `V5BinaryAuthenticationDriver` it owns, whose `initializedAsync()` calls the body's `initializeAsync(...)` lazily on first use and memoizes it; the v4 sync methods that predate this PIP are untouched, and the framework services are bound into the client's own services object rather than into each shim — `ClientAuthenticationServicesAware` is implemented only by the two plugins that also need a framework HTTP client on their v4 surface (OAuth2 and SASL). For the HTTP-capable built-ins (SASL over HTTP), the v5-native body additionally drives the framework's HTTP multi-round loop through the `AsyncHttpAuthenticationProvider` seam and the shared `HttpAuthenticationDriver`, rather than a per-class `authenticationStage(...)` hook.
- The v4 `AuthenticationTls` / `AuthenticationKeyStoreTls` are routed by their `"tls"` method name onto the built-in `TlsAuthentication` body (through a thin adapter that still forwards their `configure` / `start` / `close`, so a custom `"tls"`-named plugin keeps its lifecycle), while their configured paths/keystore are surfaced to the client builder, which merges them into the `FileBasedTlsFactory`'s purpose→policy map at client construction (the build-time composition above), and use the built-in `TlsAuthentication` plugin so the binary protocol sends `auth_method_name=tls`.
...
- The built-in → v5-native hand-over is keyed by the `V5AuthenticationProvider` interface: a v4 plugin that implements it owns a v5-native body, and the client drives that body directly. Everything else — every third-party plugin, and every v4 class without a v5-native equivalent — goes through `LegacyV4AuthenticationAdapter` unchanged, which selects its style from the plugin's advertised auth-method name (never by probing a credential method, which would run v4 I/O on the caller thread). Keeping the interface in an `.internal.` package is what keeps this a framework-observed seam rather than an open extension point; the single concrete-class special case is `AuthenticationDisabled`, matched on the class rather than on a `"none"` method name so a third-party plugin reporting `"none"` is never silently downgraded to anonymous.
...
- **Programmatic**: `new AuthenticationToken(jwt)` — the constructor stores the JWT exactly as before, and `getAuthData()` still serves it synchronously. The v5-native `TokenAuthenticationV5` is created once when the client is built; its initialization is memoized and runs lazily on first use inside `V5BinaryAuthenticationDriver`, and `start()` remains a no-op (no eager I/O).
...
- **v5 → v4** — `BinaryAuthenticationExchange` maps v5 auth exceptions back to the matching v4 `PulsarClientException` subtypes before they reach `ClientCnx`, so v4 callers see the exceptions they always have. This is a straightforward one-to-one mapping (v5 `AuthenticationException` → v4 `AuthenticationException`, and so on) that introduces no behavioural change; the acceptance criterion is that the existing v4 authentication tests pass unmodified after the migration.
...
- `org.apache.pulsar.common.tls.impl` (in `pulsar-common`) — `FileBasedTlsFactory`, its `FileBasedTlsFactorySettings` value (factory-wide engine provider / client-auth / refresh interval), the `TlsContexts` build-support helper, and the framework-internal acquisition helpers `TlsContextAcquisition`, `TlsFactoryProbe` and `TlsSynthesisSpec` (public only so the server modules can call them — not part of the SPI contract); `pulsar-common` already carries the Netty engine dependencies these need. The `TlsMaterialSource` that owns per-purpose load/watch/cache is **package-private** to `FileBasedTlsFactory` — an internal implementation detail, not public API. `DefaultBrokerTlsFactory` lives in `pulsar-broker-common`.
- `org.apache.pulsar.client.api.v5.internal` (in `pulsar-client-api-v5`) — `ClientAuthenticationServices` and `ClientAuthenticationServicesAware`, the "stable internal" seam through which the framework late-binds its runtime services (HTTP client factory, scheduler, bounded blocking executor, clock, OpenTelemetry handle, client instance id) into a plugin after the `PulsarClient` is constructed; and `V5AuthenticationProvider`, the seam through which a built-in v4 plugin hands the client the v5-native body it drives. The `.internal.` subpackage marks them framework-driven — application code neither implements nor consumes them.
- `pulsar-client` (`org.apache.pulsar.client.impl.auth.v5`) — the built-in `TlsAuthentication` mTLS plugin, the built-in `NoAuthentication` (`none`) plugin, and the v4 → v5 compatibility adapter `LegacyV4AuthenticationAdapter`.
...
Removed: `org.apache.pulsar.common.util.PulsarSslFactory`, `PulsarSslConfiguration`, `org.apache.pulsar.common.util.DefaultPulsarSslFactory` and `org.apache.pulsar.common.util.keystoretls.KeyStoreSSLContext` (intentional breaking change — see Backward & Forward Compatibility); the public `org.apache.pulsar.jetty.tls.JettySslContextFactory` (in `pulsar-broker-common`), replaced by the framework's `JettyTlsFactory` integration (see the Jetty section); the public `org.apache.pulsar.client.util.PulsarHttpAsyncSslEngineFactory` (in `pulsar-client`), superseded by the rotating `SslEngineFactory` backed by a `PulsarTlsFactory` subscription; the PIP-466 `org.apache.pulsar.client.api.v5.auth.AuthenticationData` stub and the PIP-466 `org.apache.pulsar.client.api.v5.config.TlsPolicy` sketch, both subsumed by the types this PIP introduces; and the CN-matching helpers `org.apache.pulsar.common.tls.TlsHostnameVerifier` / `SubjectName` / `PublicSuffixMatcher` / `PublicSuffixList` / `DomainType` (see Security Considerations); the PIP-337 v4 builder methods `ClientBuilder.sslFactoryPlugin(String)` / `sslFactoryPluginParams(String)` and `PulsarAdminBuilder.sslFactoryPlugin(String)` / `sslFactoryPluginParams(String)` (a v4 source-compatibility break); and — deprecated rather than removed — the `ClusterData.brokerClientSslFactoryPlugin` / `...Params` accessors, whose successors are the new `ClusterData.brokerClientTlsFactoryClassName` / `brokerClientTlsFactoryConfig` (see [PIP-337 removal impact](#pip-337-removal-impact)). Changed (not removed), and worth stating explicitly because each is an extension point someone may have taken:

- **`ClientCnx.newConnectCommand()` → `buildConnectCommand(AuthData)`, plus two further `protected`-surface changes.** The `protected` hook that built the `CommandConnect` frame also *fetched* the credential (`authentication.getAuthData(...)` then `authenticate(...)`), which cannot work once the credential is a future. In the same move the `protected final Authentication authentication` field is replaced by `protected final BinaryAuthenticationDriver authDriver` (an implementation type, not a public-API one), and a new `protected String authMethodName()` appears — so an out-of-tree subclass that read the old field no longer compiles, and one that already declared a method of that name silently starts overriding framework behaviour. Credential resolution moves out to `resolveConnectAuthData()`, and the overridable hook now receives the already-resolved `AuthData`, so an override composes with both the sync and async paths. `ClientCnx` carries no `@InterfaceStability` annotation and lives in `...client.impl`, but it *is* subclassed in-tree (`ProxyClientCnx`) and the method was `protected`, so an out-of-tree subclass overriding the old name silently stops being called — a rename is the loud form of that (an `@Override` on the old signature fails to compile), which is why it is renamed rather than kept with a changed body.
- **`PulsarClientProvider.authenticationTls(String, String)` → `authenticationTls()`.** The mTLS marker plugin no longer carries cert/key paths: material goes through `tlsPolicy(...)`, so `AuthenticationFactory.tls()` is the no-arg form and the provider seam matches it. `PulsarClientProvider` is in the `.internal.` subpackage (framework-driven, "application code neither implements nor consumes"), and both it and the v5 auth API ship first in 5.0, so nothing released depends on the two-argument form.

Also removed, but **internal-only** (no `@InterfaceStability`, no client-API importer — not a compatibility break): the `org.apache.pulsar.common.util.SecurityUtility` grab-bag (decomposed into the cohesive `PemReader` / `JcaProviders` / `JcaKeyStores` / `JdkSslContexts` containers), the obsolete `TrustManagerProxy` / `KeyManagerProxy` delegate-swap rotation helpers, and the unused `SSLContextValidatorEngine` — see *Removal* in the Detailed Design.
...
- Broker `ServiceConfiguration`: `tlsFactoryClassName` (default: blank — a blank value, the reserved literal `default`, or the built-in factory's own class name all have the framework build the built-in `DefaultBrokerTlsFactory`) + `tlsFactoryConfig` for the listener/web purposes, and `brokerClientTlsFactoryClassName` + `brokerClientTlsFactoryConfig` for the broker's own outbound clients (the `BROKER_CLIENT` purpose).
...
- **Defaults.** Both provider keys are unset on every surface, and unset means today's behaviour. Under this
  PIP the `WEB` purpose honours the web-listener keys — `webServiceTlsProvider` / `webServiceTlsProtocols` /
  `webServiceTlsCiphers` take precedence over the binary-listener keys and fall back to them when unset — and
  a configured provider name is genuinely pinned, failing loudly when it cannot be resolved. That makes the
  legacy `Conscrypt` defaults, which were unconditional, untenable as written — so this PIP keeps the
  default but makes it **conditional on availability**: `webServiceTlsProvider` on `ServiceConfiguration` /
  `ProxyConfiguration`, the functions worker's `tlsProvider` and the WebSocket proxy's web-listener
  `tlsProvider` all resolve, when unset, to Conscrypt where Conscrypt actually loads and to the JVM default
  everywhere else. Under PIP-337 those defaults were inert — the broker passed `webServiceTlsProvider` to
  Jetty's `SslContextFactory`, which never reaches the built `SSLContext` that `DefaultPulsarSslFactory`
  supplies with no provider — so no deployment has actually been running on Conscrypt, and honouring the
  value here is what makes the setting real. An *unconditional* default would then break the web listener
  out of the box wherever Conscrypt cannot load: `conscrypt-openjdk-uber` ships native libraries for x86_64
  and, since 2.6.1, aarch64 — but not for every platform Pulsar runs on (s390x, for one), and it need not be
  on the class path at all. Availability is therefore tested rather than assumed — Conscrypt's own
  `checkAvailability()`, which is what fails where the native library is missing.
  The conditionality applies to the **default only**: an operator who names a provider explicitly still gets
  it pinned, and still gets the loud failure when it cannot be resolved, which is the point of pinning. It
  applies to **server listeners only**, so it cannot interact with client-side hostname verification — a
  server does not verify hostnames.
...
| `ClusterData.brokerClientSslFactoryPlugin` / `...Params` | admin API + **serialized cluster metadata** | **Deprecated and ignored** (with a WARN), not removed: the field stays in the metadata schema so a cluster written by 4.x still deserializes and reserializes unchanged. Its successors are the new `ClusterData.brokerClientTlsFactoryClassName` / `brokerClientTlsFactoryConfig`, which select the factory for the two outbound legs whose client configuration is built from a cluster entry — the binary replication client and the cross-cluster admin (HTTPS) client (not the peer-cluster lookup client, which is broker-level throughout, as it was in 4.x) — with the class name falling back to the broker-level `brokerClientTlsFactoryClassName` when blank; the config follows the class name rather than inheriting on its own, so one factory's init parameters are never handed to another. Per-cluster TLS *material* (the existing `brokerClientTls*` fields) keeps working as before. This is the one place a stale PIP-337 value **cannot** fail loud — rejecting it on a metadata read would make the cluster unloadable — so it is ignored rather than rejected; the remedy is to set the successor field. Documented in the [geo-replication note](#pulsar-geo-replication-upgrade--downgraderollback-considerations) |
| `pulsar-admin clusters` `--tls-factory-plugin` / `--tls-factory-plugin-params` options | `CmdClusters` CLI | **Replaced** by `--tls-factory-class-name` / `--tls-factory-config`, which write the successor `ClusterData` fields. The old flags are removed, so a script passing them fails loudly with an unknown-option error rather than silently configuring nothing |
...
- **Security-relevant behavior changes (5.0), surfaced here so they aren't buried in the removal inventory:**
  ...
  - **`ClusterData` per-cluster factory field deprecated — the one silently-ignored case.** The `brokerClientSslFactoryPlugin` / `...Params` fields stay on the `ClusterData` metadata model but are `@Deprecated` and **ignored**, with a WARN naming the cluster so the stale value is visible in the broker log. Their successors are `brokerClientTlsFactoryClassName` / `brokerClientTlsFactoryConfig`, which select the factory for the two outbound legs whose client configuration is built from a cluster entry — the binary replication client and the cross-cluster admin (HTTPS) client (not the peer-cluster lookup client, which is broker-level throughout, as it was in 4.x) — with the class name inheriting the broker-level setting when blank and the config following the class name. This is the single PIP-337 key whose stale value **cannot** fail loud: a metadata read can't reject without making the cluster unloadable, so it is logged rather than rejected. The remedy is to set the successor field. Per-cluster TLS *material* (`brokerClientTls*`) keeps working. See [PIP-337 removal impact](#pip-337-removal-impact) and the [geo-replication note](#pulsar-geo-replication-upgrade--downgraderollback-considerations).
  - **The `Conscrypt` web-listener provider defaults become conditional.** `webServiceTlsProvider` (broker
    and proxy), the functions worker's `tlsProvider` and the WebSocket proxy's web-listener `tlsProvider`
    shipped a default of `Conscrypt` that was **inert** under PIP-337: it reached only Jetty's
    `SslContextFactory.setProvider(...)`, which does not affect a factory whose `SSLContext` is supplied
    pre-built, so no deployment has actually been running on Conscrypt. This PIP honours the value, which
    makes the setting real — and therefore makes an unconditional default a startup failure wherever
    Conscrypt has no native library (`conscrypt-openjdk-uber` covers x86_64 and, since 2.6.1, aarch64, but
    not every platform — s390x, for one). The default is kept and made conditional: unset resolves to
    Conscrypt where it is available and to the JVM default elsewhere. **Operationally this means web
    listeners now genuinely run on Conscrypt wherever it loads, where they previously ran on the JVM
    default**, which is the behaviour the shipped configuration always described. An operator who prefers
    the JDK provider sets it explicitly.
...
- Existing applications using `pulsar-client-api` (v4) need **no source changes** — except the rare application that called the removed PIP-337 `sslFactoryPlugin(...)` / `sslFactoryPluginParams(...)` builder methods, which no longer compile and must move to `tlsFactoryClassName(...)` / `tlsFactoryConfig(...)` (see below) — but 5.0 carries several **runtime behavior deltas** they should review before upgrading:
  ...
  - **Three TLS material configurations that 4.x accepted now fail the load**, all of them fail-closed
    tightenings that surface at broker/proxy start (and again on every rotation reload), not on the wire:
    a configured keystore **truststore holding zero certificates**; a configured **keystore with no usable
    key entry** (a private key with its certificate chain); and a **PEM certificate configured without its
    private key**. The first is the security-relevant one: downstream, an empty trust list was
    indistinguishable from "no truststore configured", so both context builders installed the platform
    default trust manager and the deployment trusted every public CA while believing it had pinned a
    private one. The other two would have silently presented no TLS identity. The PEM *trust* axis is
    deliberately **not** tightened — 4.x fell back to the platform trust store for an empty trust file and
    deployments rely on that.

  The v4 *API* surface is otherwise untouched apart from the PIP-337 changes above; the deprecated v4 methods (`getAuthData()` no-arg, `configure(Map)`) are retained per PIP-466's stability promise. Beyond the PIP-337 method swap (removing `sslFactoryPlugin(...)`, adding `tlsFactoryClassName(...)` / `tlsFactoryConfig(...)`), the v4 module's **public** API gains nothing: the seam through which `ClientCnx` obtains credentials is an implementation type in `pulsar-client`, and the only "stable internal" type an integrator can encounter is `org.apache.pulsar.client.api.v5.internal.V5AuthenticationProvider` — framework-observed, not application-implemented. The `protected` `ClientCnx` surface does change, though: see the extension-point list under [Public API](#public-api).
...
Authentication is a per-client concern: each cluster authenticates the replicator client independently using whichever auth plugin is configured, and no auth-related metadata changes. TLS *is* affected, because `ClusterData` carried per-cluster `brokerClientSslFactoryPlugin`/`...Params` fields in 4.x. Those fields are **deprecated and ignored** in 5.0, superseded by `ClusterData.brokerClientTlsFactoryClassName` / `brokerClientTlsFactoryConfig`, which keep factory-class selection per-cluster (the class name falling back to the broker-level `ServiceConfiguration.brokerClientTlsFactoryClassName` when blank, the config following the class name rather than inheriting separately) and apply to the two outbound legs whose client configuration is built from a cluster entry — the binary replication client and the cross-cluster admin client. They do not reach the peer-cluster lookup client (`NamespaceService.getNamespaceClient`), which takes only the service URL from the cluster entry and its TLS material and factory from the broker-level `brokerClient*` settings; that leg read the broker-level `brokerClientSslFactoryPlugin` in 4.x too, so it is unchanged rather than narrowed. The per-cluster TLS *material* fields (`brokerClientTls*`) keep working: each outbound replication client owns a factory built from its own configuration (which already carries that cluster's material), resolved under the fixed `BROKER_CLIENT` purpose — no per-cluster purpose is minted. Retaining the field is what keeps this off the metadata-format break list: it stays in the schema, so a mixed-version fleet (4.x and 5.0 brokers reading the same cluster metadata) round-trips it unchanged in both directions — a 5.0 broker ignores a value a 4.x admin still writes, and preserves it rather than erasing it on rewrite. The one consequence to call out: a cluster that had a **custom** `brokerClientSslFactoryPlugin` value set has it **ignored** on upgrade, with a WARN naming the cluster (the removed-key validation can't run on a metadata read without making the cluster unloadable — this is the single PIP-337 key whose stale value cannot fail loud). Such deployments are rare; the remedy is to set `ClusterData.brokerClientTlsFactoryClassName` (per-cluster) or the broker-level key of the same name.
...
**Implementation approach.** The change is naturally ordered so each step keeps the codebase releasable: the new SPI types (the `pulsar-tls-factory-api` and `pulsar-http-client-api` types and the `org.apache.pulsar.client.api.v5.auth` types) land first with no consumers; then the default `FileBasedTlsFactory` / `DefaultBrokerTlsFactory` and the server-side migration (broker, proxy, websocket, functions-worker); then the client-side migration (including the new `tlsFactoryClassName` / `tlsFactoryConfig` by-name selector on `ClientConfigurationData` and the v4 client + admin builders) and the v4 → v5 bridge (`LegacyV4AuthenticationAdapter`, the `ClientCnx` inversion onto the v5-driven exchange, and the built-in v4 plugin shims' `V5AuthenticationProvider` hand-over); and finally the removal of `PulsarSslFactory` / `PulsarSslConfiguration` / `DefaultPulsarSslFactory` / `KeyStoreSSLContext` **together with the PIP-337 config keys themselves** — the `sslFactoryPlugin` / `sslFactoryPluginParams` / `brokerClientSslFactoryPlugin` / `brokerClientSslFactoryPluginParams` fields on the server configs and `ClientConfigurationData` (on `ClusterData` they are deprecated and ignored rather than removed), the v4 `sslFactoryPlugin(...)` builder methods, and the `CmdClusters` `--tls-factory-plugin` flags — once nothing references them, adding in the same step the **removed-key validation** that rejects any of those keys left in a config file (broker.conf / proxy.conf / client `loadConf`) with a non-default value. A stale PIP-337 factory key set to a non-default value is thus **rejected loudly at startup / build** rather than silently adapted — there is no compatibility shim keeping the old keys working, and builder-method removal is inherently loud (a compile error). The one exception is a stale *custom* `ClusterData.brokerClientSslFactoryPlugin` value in already-written metadata, which is ignored with a WARN rather than rejected (see the [geo-replication note](#pulsar-geo-replication-upgrade--downgraderollback-considerations)).
