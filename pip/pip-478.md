...
- **Security-relevant behavior changes (5.0), surfaced here so they aren't buried in the removal inventory:**
  ...
  - **`ClusterData` per-cluster factory value fails loudly.** A *custom* `brokerClientSslFactoryPlugin` value carried in serialized cluster metadata now **fails loudly** at cluster-client / cluster-admin creation rather than silently building file-based TLS (which would downgrade that cluster's broker-client identity/trust on upgrade); a blank value or the removed-default's FQCN is tolerated, and per-cluster TLS *material* keeps working. See [PIP-337 removal impact](#pip-337-removal-impact) and the [geo-replication note](#pulsar-geo-replication-upgrade--downgraderollback-considerations).
...
- Existing applications using `pulsar-client-api` (v4) need **no source changes**, but 5.0 carries several **runtime behavior deltas** they should review before upgrading:
  ...
  - **A custom PIP-337 factory via `sslFactoryPlugin`** (rare) must be ported to `PulsarTlsFactory`; the deprecated v4 builder methods fail loudly rather than silently ignore a security-relevant setting (see [PIP-337 removal impact](#pip-337-removal-impact)).

  The v4 *API* surface is otherwise untouched; the deprecated v4 methods (`getAuthData()` no-arg, `configure(Map)`) are retained per PIP-466's stability promise. The only v4 module addition is a new public interface `org.apache.pulsar.client.api.internal.AsyncAuthenticationDriver` (in a `.internal.` subpackage) — application code should not implement it; it is observed by `ClientCnx` to opt into the async path.
