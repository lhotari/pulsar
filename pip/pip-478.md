...
**v5 client builder.** Gains `tlsFactory(PulsarTlsFactory)` to supply a custom factory instance (adopted — closed with the client), and `tlsPolicy(TlsPolicy)` / `tlsPolicy(TlsPurpose, TlsPolicy)` to configure one or more `TlsPolicy` values — each covering cert/key/trust paths or a keystore, ciphers/protocols, and hostname-verification / insecure-connection flags — bound to a `TlsPurpose` (the single-argument form binds to `CLIENT_DEFAULT`, the purpose all Pulsar-cluster client traffic resolves against directly; an unconfigured client purpose resolves terminally to the system default). The simple file-based case is a single builder-level value, subsuming the PIP-466-era experimental `TlsPolicy` (see the Detailed Design).
