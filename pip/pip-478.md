...
public interface AuthenticationInitContext {
     ...
     * plugin call here. Never the Netty event loop.
     */
...
- **`PulsarTlsFactory`** (interface, in `org.apache.pulsar.common.tls`) — the v5 TLS SPI. The framework requests an instance of a well-known class for a `TlsPurposeContext` (described next); the factory returns a configured instance when it supports the combination, or `Optional.empty()` when it does not:
  ...
  public interface PulsarTlsFactory extends AutoCloseable {
       ...
       * delivery.
       */
      <T> CompletableFuture<Optional<TlsSubscription<T>>> createInstance(
  ...
  public interface TlsSubscription<T> {
      /**
       * The current instance for this subscription — the value most recently
       * delivered to {@code onLoadOrReload} (the initial load, or the latest
       * rebuild). Lets a consumer read the live instance on demand without
       * having to cache what the callback handed it.
       */
      T get();
...
**Rotation model: rebuild the context, don't mutate behind a stable one.** The reload callback hands consumers a freshly built `SslContext` / `SSLContext` on each rotation, rather than keeping one immutable context alive and swapping new material in behind a mutable delegate — the approach of `org.apache.pulsar.common.util.TrustManagerProxy` / `KeyManagerProxy`, which wrap an `X509ExtendedTrustManager` / `X509ExtendedKeyManager` whose backing instance a scheduled task replaces. The swap pattern works, but it interacts awkwardly with the Netty **OpenSSL** provider:
...
**Issue.** The design makes `javax.net.ssl.SSLContext` required only as a *fallback* — a factory must supply it for a purpose only when it does not natively build the richer Netty/Jetty objects that purpose needs, and the framework synthesizes those from it on `Optional.empty()`. The alternative is a strict contract where every well-known class is mandatory for every factory. Smaller details are also open: whether `SslContextFactory.Client` belongs in the well-known set at all (no in-tree consumer today), and the exact method/handle names (`createInstance` overloads vs a separate `subscribe` method; `TlsInstance`/`TlsSubscription`).
