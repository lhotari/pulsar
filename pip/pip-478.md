...
4. **The framework owns shared runtime resources.** HTTP clients, event loops, DNS caches, schedulers, and TLS engine construction are framework-managed and handed to plugins through contexts; a plugin never builds parallel infrastructure.
...
PIP-337's `PulsarSslFactory` / `PulsarSslConfiguration` are removed entirely (Motivation #4) and replaced by a purpose-driven, material-focused SPI. A **`TlsPurposeContext`** identifies *why* TLS is requested and in what role — a `ClientTlsPurposeContext` (binary client, HTTP lookup, OAuth2/generic) or a `ServerTlsPurposeContext` (broker, proxy, web service, plus the advertised-listener name on the server side). The provider answers `getTlsEndpointSpec(TlsPurposeContext)` with a **`TlsEndpointSpec`** that splits the *material* from the *configuration*. The material comes in exactly two kinds: **`InMemoryTlsMaterial`** (raw `PrivateKey`, key-cert chain, and trust certs — what the default file-based provider produces) and **`JcaTlsMaterial`** (a `KeyManagerFactory`/`TrustManagerFactory` pair for HSM/KMS-backed deployments whose keys are never extracted by the framework). The configuration part is `Optional` — protocols, ciphers, and role-specific flags (hostname verification, mTLS client-cert requirement, insecure modes) — and falls back to safe defaults when empty. The framework — not the plugin — turns the spec into an `SSLEngine`/`SSLContext` and caches it, rebuilding only when the spec changes. The SPI deliberately omits PIP-337's implementation-detail methods (`needsUpdate()`, `update()`, `getInternalSslContext()`, …).

The provider also drives a **`TlsMaterialListener`**: it notifies the listener once when the material for a purpose is first ready, then again whenever it changes. On the broker side the server must rebuild its listener `SSLContext` on rotation; on the client side the framework re-requests `getTlsEndpointSpec(...)` when a new TLS connection is created, so rotated material is picked up per new connection (whether the client should *additionally* subscribe a listener and proactively cycle existing connections is an [open issue](#open-issues)). The default `FileBasedTlsMaterialProvider` loads PEM and keystore files and reloads on rotation, and supports **dynamic registration** of additional file-based material sources per purpose after construction (used by the v4 `AuthenticationTls` bridge); a custom provider can integrate a Key Management System (KMS) and may differ between client and broker sides.

**Three usage tiers.** The TLS surface is layered so that complexity is proportional to need:

1. **File paths (almost everyone).** TLS material in PEM or keystore files, configured essentially as today — broker/proxy via the existing `ServiceConfiguration` properties, the v5 client via a simple builder-level TLS configuration object. The default `FileBasedTlsMaterialProvider` is wired automatically; the user never names a provider, a purpose, or a material type.
...
3. **Custom provider (keys never touch a file).** Organizations whose policy forbids on-disk private keys implement `PulsarTlsMaterialProvider` against their KMS / HSM / workload-identity system and return `JcaTlsMaterial` — a `KeyManagerFactory` / `TrustManagerFactory` pair, the conventional JCA integration point that JSSE and Netty both accept directly. The key stays non-extractable behind its JCA provider (e.g. PKCS#11 or a KMS JCA provider; signing happens inside the HSM/KMS), and the certificate chain is served by the `KeyManager` itself. The framework builds this material kind on the JDK `SSLEngine` path, since OpenSSL-based engines require extractable key bytes (see [Open Issues](#open-issues)).
...
- **`PulsarTlsMaterialProvider`** (interface, in `org.apache.pulsar.common.tls`) — the v5 TLS SPI. The framework calls `getTlsEndpointSpec(...)` with a `TlsPurposeContext` (described next) that identifies the TLS usage, and the provider returns the matching `TlsEndpointSpec` (material plus optional configuration, described below); it also registers/unregisters `TlsMaterialListener`s and is initialized and closed by the framework.

  ```java
  public interface PulsarTlsMaterialProvider {
      CompletableFuture<Void> initialize(TlsMaterialProviderInitContext context);
      /**
       * The endpoint spec (material + optional configuration) for the context, applying
       * the usage-identifier and purpose fallback chain when none is configured.
       */
      CompletableFuture<TlsEndpointSpec> getTlsEndpointSpec(TlsPurposeContext purpose);
...
- **`TlsEndpointSpec`** (interface, in `org.apache.pulsar.common.tls`) — what the provider returns for a purpose: the **material** (exactly one of two kinds) plus **optional configuration**. "Endpoint" is used in the TLS sense (RFC 8446): one side of a TLS connection, in its role — the spec describes how this endpoint presents itself (material) and how it behaves toward its peer (configuration). It does not identify a network address. Splitting material from configuration keeps "what the credentials are" separate from "how the engine should behave":

  ```java
  /**
   * The complete TLS specification for one endpoint role (client or server side of a
   * connection — not a network address): the material this endpoint presents, plus
   * optional behavioural configuration. Consumed by the framework to build and cache
   * the SSLEngine/SSLContext.
   */
  public interface TlsEndpointSpec {
      /** The TLS material — exactly one of the two kinds below. */
      TlsMaterial getMaterial();
      /** TLS configuration; empty means "use the defaults for the purpose's role". */
      default Optional<TlsConfig> getConfig() { return Optional.empty(); }
  }
  ```

  `PulsarTlsMaterialProvider` implementations are responsible for handling updates efficiently: `getTlsEndpointSpec` returns a **new** `TlsEndpointSpec` instance when the underlying material or configuration has changed, and the **same** instance as the previous call when it hasn't. This lets the framework cache the built `SSLEngine`/`SSLContext` and rebuild only on instance change.

- **`TlsMaterial`** (sealed interface, in `org.apache.pulsar.common.tls`) — the credentials themselves, in exactly one of two kinds. The kind declares, at the type level, whether the framework may touch raw key bytes:

  ```java
  /**
   * Sealed so the framework can dispatch exhaustively on the kind; the two kinds are
   * non-sealed so providers implement them freely.
   */
  public sealed interface TlsMaterial permits InMemoryTlsMaterial, JcaTlsMaterial {}

  /**
   * Raw key/cert material held in memory. Produced by the default file-based provider
   * from PEM (PKCS#8) or keystore (PKCS12/JKS) sources — one material kind regardless
   * of file format, so there is no separate configuration path per format. The private
   * key MUST be extractable.
   */
  public non-sealed interface InMemoryTlsMaterial extends TlsMaterial {
  ...
  /**
   * JCA-level material: key and trust handling stay behind the standard JCA factories
   * and the framework never extracts key bytes. For HSM / KMS / PKCS#11-backed
   * deployments where private keys must not exist outside the key store.
   */
  public non-sealed interface JcaTlsMaterial extends TlsMaterial {
      KeyManagerFactory getKeyManagerFactory();
      TrustManagerFactory getTrustManagerFactory();
  }
  ```

    - Immutable default implementations `DefaultInMemoryTlsMaterial` and `DefaultJcaTlsMaterial` ship with the SPI, plus a small adapter for mixed cases — `TlsMaterialSupport.trustManagerFactoryOf(Iterable<? extends X509Certificate>)` — so an HSM-held key can be combined with plain PEM trust anchors by wrapping the cert list into a `TrustManagerFactory`.
    - **`InMemoryTlsMaterial`** is what the default `FileBasedTlsMaterialProvider` produces and can be served by both engine paths (JDK and OpenSSL-based). Returning a non-extractable, provider-backed `PrivateKey` handle (`getEncoded() == null`) through this kind is a contract violation: the framework rejects it with an actionable error directing the provider to `JcaTlsMaterial` (in-memory JDK keystores cannot hold a key without an encoding, so conventional `KeyManager` construction would fail anyway).
    - **`JcaTlsMaterial`** covers the non-extractable-key use cases by construction. A `KeyManagerFactory.getInstance("PKIX")` initialized with a PKCS#11 or KMS-JCA-provider `KeyStore` yields `X509ExtendedKeyManager`s whose signing runs inside the HSM/KMS, and the certificate chain is served by the `KeyManager` itself (JSSE calls `chooseEngineServerAlias(...)`/`chooseEngineClientAlias(...)` and then `getCertificateChain(alias)` during the handshake) — there is no separate chain configuration on this kind. The framework always builds `JcaTlsMaterial` on the **JDK** `SSLEngine` path: Netty accepts both factories directly (`SslContextBuilder.forServer(KeyManagerFactory)` / `.keyManager(KeyManagerFactory)` / `.trustManager(TrustManagerFactory)`), whereas OpenSSL-based engines extract raw key bytes from the `KeyManager` and cannot serve non-extractable keys (see [Open Issues](#open-issues) for possible future BoringSSL keyless support). Beyond key storage, the `TrustManagerFactory` side also serves custom *validation policy*: PKIX revocation configuration (`PKIXRevocationChecker`, OCSP/CRL), enterprise PKI rules, or dynamically rotating trust bundles (e.g. SPIFFE).
    - `InMemoryTlsMaterial` and `JcaTlsMaterial` are working names — see [Open Issues](#open-issues) for alternatives.

- **`TlsConfig`** (interface, in `org.apache.pulsar.common.tls`) — how the engine should behave, split into a common part and role-specific subinterfaces. A provider may return the common `TlsConfig` or the `ClientTlsConfig` / `ServerTlsConfig` matching the purpose's role; a role mismatch (e.g. a `ServerTlsConfig` for a client purpose) is a configuration error. When `TlsEndpointSpec.getConfig()` is empty, the defaults below apply:

  ```java
  /** Configuration common to both roles. Empty iterables mean "engine defaults". */
  public interface TlsConfig {
      default Iterable<String> getTlsProtocols() { return List.of(); }
      default Iterable<String> getTlsCiphers() { return List.of(); }
  }

  public interface ServerTlsConfig extends TlsConfig {
      /** false by default; must be enabled for mTLS / TLS authentication. */
      default boolean isTrustedClientCertRequired() { return false; }
      /**
       * false by default; the server-side counterpart of the legacy
       * tlsAllowInsecureConnection mode — accept any client certificate without
       * trust-chain validation.
       */
      default boolean isTrustAnyClientCert() { return false; }
  }

  public interface ClientTlsConfig extends TlsConfig {
      /** true by default. */
      default boolean isHostnameVerificationRequired() { return true; }
      /** false by default; matches the existing insecure mode when true. Breaks mTLS if true. */
      default boolean isTrustAnyCaCert() { return false; }
  }
  ```

    - The insecure trust-any flags (`isTrustAnyCaCert()` / `isTrustAnyClientCert()`) instruct the framework to install a permissive trust manager, so they apply only together with `InMemoryTlsMaterial`; combining them with `JcaTlsMaterial` is a configuration error — the provider has already taken control of trust evaluation through its `TrustManagerFactory`.
    - `isHostnameVerificationRequired()` and `isTrustedClientCertRequired()` work with **both** material kinds: hostname verification is applied via endpoint identification on the engine's `SSLParameters`, and client-cert requirement via the engine's client-auth mode — both independent of how key and trust material were supplied. This is what makes the model cover the JCA-based non-extractable-key use cases without any special-casing in the configuration layer.
- **`TlsMaterialListener`** (interface, in `org.apache.pulsar.common.tls`) representing a listener for TLS material updates. The provider implementation will first call the listener once the material has been loaded and is ready to be used. It will then call the listener again whenever the material changes. This is mainly useful for broker side TLS material updates since the broker will need to change the TLS configuration for the server when the material changes. On the client side, the implementation will request the `TlsEndpointSpec` for each usage using the `getTlsEndpointSpec` method when a new TLS connection is created (see [Open Issues](#open-issues) on whether the client should additionally react to listener notifications).
...
**Client configuration and the experimental v5 `TlsPolicy`.** The default client-side `PulsarTlsMaterialProvider` is wired from the client builder. Rather than scattering many `tls*` setters across the builder, the TLS file/keystore settings live on a **`FileBasedTlsMaterialSource`**, created via its own builder and registered with the default `FileBasedTlsMaterialProvider` for one or more `TlsPurposeContext`s. The common case registers a single source shared across all client purposes; advanced deployments register distinct sources when trust domains differ (e.g. a dedicated source for `GENERIC` / OAuth2-IdP calls). Unconfigured purposes resolve through the fallback chain above.
...
**Removal.** The existing v4 `org.apache.pulsar.common.util.PulsarSslFactory` and `PulsarSslConfiguration` are removed completely — an intentional breaking change. Because the SPI now provides only material and configuration and the framework owns the `SSLEngine`/`SSLContext` (built and cached, rebuilt only when the `TlsEndpointSpec` instance changes), the `org.apache.pulsar.common.util.keystoretls.KeyStoreSSLContext` class is no longer needed either; the file-based provider reads keystores (PKCS12/JKS) directly for the private key, key-cert chain, and trust certificates. PIP-337 is not broadly used, so it is removed rather than retained alongside the new SPI.
...
- `org.apache.pulsar.common.tls` — `PulsarTlsMaterialProvider`, `TlsMaterialProviderInitContext`; purpose contexts `TlsPurposeContext` (with the nested `UsageIdentifier` record), `ClientTlsPurposeContext`, `ServerTlsPurposeContext`; `TlsEndpointSpec`; material kinds `TlsMaterial` / `InMemoryTlsMaterial` / `JcaTlsMaterial` with `DefaultInMemoryTlsMaterial` / `DefaultJcaTlsMaterial` and the `TlsMaterialSupport` helper; configuration `TlsConfig` / `ClientTlsConfig` / `ServerTlsConfig`; `TlsMaterialListener`; `FileBasedTlsMaterialProvider`, `DefaultBrokerTlsMaterialProvider`, `TlsMaterialSource`, `FileBasedClientTlsMaterialSource`, `FileBasedServerTlsMaterialSource`.
...
- **Private keys need never touch the filesystem.** A custom `PulsarTlsMaterialProvider` can keep keys in an HSM/KMS or a workload-identity system and expose only a provider-backed `PrivateKey` handle. The framework never logs, serializes, or copies key material beyond handing it to the TLS engine, so the SPI is compatible with organizational policies that forbid on-disk keys.
...
## 4. The two TLS material kinds: final naming, and engine support beyond the JDK path

**Issue.** The two-kind material model is settled in shape — `InMemoryTlsMaterial` (raw key/certs, extractable, served by both engine paths) and `JcaTlsMaterial` (`KeyManagerFactory`/`TrustManagerFactory`, never extracted, JDK `SSLEngine` path only) — and the enclosing envelope is named `TlsEndpointSpec`. Two smaller decisions remain: (i) the final names of the two material kinds, and (ii) whether the PIP should record a future opt-in that bridges JCA signing onto Netty's BoringSSL "keyless" mode for deployments that need both HSM-held keys and native-TLS handshake performance.

**Context.** For (i): `InMemoryTlsMaterial` describes where the material lives, but alternatives read differently — `RawTlsMaterial` (emphasizes the unwrapped key/cert objects) or `ParsedTlsMaterial` (emphasizes it is decoded from PEM/keystore sources). Pure bikeshed, but frozen once 5.0 ships. For (ii): Netty exposes BoringSSL's `SSL_PRIVATE_KEY_METHOD` (`OpenSslAsyncPrivateKeyMethod` plus `OpenSslX509KeyManagerFactory.newKeyless(chain)`), and a JCA-backed adapter is feasible (essentially a TLS-signature-scheme→JCA-algorithm map delegating to `java.security.Signature`), but Netty ships no such bridge today, the API is `@UnstableApi` and BoringSSL-only, TLS 1.3 RSA certificates require PSS signing support in the KMS, TLS 1.2 RSA key exchange needs a raw-decrypt operation most KMSs do not offer, and every handshake pays a KMS round trip.

**Alternatives.**
- (i) Keep the working names, or rename per the candidates above — to be settled in review before the vote.
- (ii)(a) *(suggested)* Document "JDK engine only" as the contract for `JcaTlsMaterial` and leave BoringSSL keyless out of the PIP entirely.
- (ii)(b) Record the BoringSSL keyless JCA adapter as documented future work for HSM-plus-native-TLS deployments, without committing to it.
...
## 6. Client-side reaction to TLS material rotation

**Issue.** The server-side rotation contract is clear: the provider notifies a `TlsMaterialListener` and the broker rebuilds the listener `SSLContext`. On the client side, the design has the framework re-request `getTlsEndpointSpec(...)` per **new** connection, which means existing connections keep their old material until they reconnect.

**Context.** Per-new-connection pickup matches v4 behaviour for file-based rotation and suffices for routine certificate renewal (established connections remain valid until naturally cycled). Listener-driven proactive reconnection would tighten the response to credential *revocation* at the cost of connection churn and extra framework machinery.

**Alternatives.**
- (a) *(suggested)* New connections only, with the framework's cached engine rebuilt whenever the provider returns a different `TlsEndpointSpec` instance. Document that revocation-grade rotation relies on the server side terminating connections.
- (b) The client also registers a `TlsMaterialListener` and proactively cycles affected connections when material changes.
