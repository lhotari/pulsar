...
public final class TlsPolicy {
    ...
    public String jsseProvider();        // optional: names the JSSE (SSLContext) java.security.Provider that
                                        // builds the SSLContext (e.g. BCJSSE for FIPS, with BC-FIPS registered
                                        // separately as its crypto provider). When set, the default
...
**`jsseProvider` — pinning the JSSE (SSLContext) provider.** `TlsPolicy.jsseProvider` names a `java.security.Provider` (matched by `Provider.getName()`) that supplies an `SSLContext` (`TLS`) implementation and builds the TLS context — the field FIPS and BouncyCastle-JSSE deployments need. For FIPS it names **BCJSSE** (the BouncyCastle JSSE provider from `bctls-fips`), with the FIPS-140-validated **BC-FIPS** provider registered separately as the crypto provider BCJSSE delegates to; a crypto-only provider such as BC-FIPS exposes no `SSLContext.TLS` and so cannot be named here directly. It is resolved via the **`ServiceLoader`** mechanism using the application class loader (`META-INF/services/java.security.Provider`), falling back to an already-registered provider (`Security.getProvider(name)`), and failing loudly if the name resolves to nothing. When `jsseProvider` is set, the default `FileBasedTlsFactory` builds the **JDK** Netty engine (`io.netty.handler.ssl.SslProvider#JDK`) with that provider installed as `SslContextBuilder#sslContextProvider(...)`, **taking precedence over** the factory-level engine selection — a `jsseProvider` value therefore always pins the JDK engine and never the native OpenSSL one, because Netty's OpenSSL engine cannot delegate to an arbitrary JSSE provider. `jsseProvider` is a **value-level, file-based-simple** field precisely because a deployment's JSSE provider travels with its material and security posture; this is a *different axis* from the raw JDK-vs-OpenSSL **engine** choice, which stays a factory concern and is deliberately not a `TlsPolicy` field (see the [engine-selection note](#pip-337-removal-impact)). Note that `ServiceLoader` resolution against the application class loader is distinct from `Security.getProvider`, which only finds a provider already statically registered in the JVM. A **custom** `PulsarTlsFactory` may interpret `jsseProvider` however it likes — or ignore it entirely; the field carries this defined meaning only for the default file-based path, which always sources its material from the file-based keys.
...
**JSSE (SSLContext) provider (`jsseProvider`).** A new `jsseProvider` config key names a `java.security.Provider` that supplies an `SSLContext` (`TLS`) implementation — the JSSE provider that builds the TLS context (e.g. **BCJSSE** for FIPS, with the FIPS-140-validated **BC-FIPS** provider registered separately as the crypto provider it delegates to; a crypto-only provider such as BC-FIPS exposes no `SSLContext.TLS` and cannot be named here directly). It is a **distinct key on a different axis** from the engine-only `sslProvider` (client) / `tlsProvider` (broker) settings, which stay a JDK-vs-OpenSSL **engine** switch and are unchanged. `jsseProvider` is exposed on the client (`ClientConfigurationData`, beside the existing `sslProvider`) and on the server (`ServiceConfiguration` for the listener/web material, plus its `brokerClient*` counterpart for the broker's outbound clients), and maps directly onto `TlsPolicy.jsseProvider`. Operators set it in `broker.conf` / `client.conf` (the common FIPS/custom-JSSE ops need) to pin a specific JSSE provider; when set, the default factory builds the **JDK** Netty engine with that provider building the `SSLContext`, overriding the engine choice (see [the `jsseProvider` design](#redesigned-pip-337-ssl-provider-pulsartlsfactory)). Resolution uses the `ServiceLoader` mechanism on the **application class loader** (`META-INF/services/java.security.Provider`) — distinct from `Security.getProvider`, which finds only a provider already statically registered in the JVM — with a fallback to an already-registered provider and a loud failure if the name is unresolvable. Existing v4 deployments need no config change: the v4 migration routes a non-engine-literal `sslProvider` value (a JSSE provider name) to `jsseProvider` automatically, so a `sslProvider=Conscrypt`-style setting keeps working — the engine literals (`JDK` / `OPENSSL` / `OPENSSL_REFCNT`) stay on the engine axis (see [v4 client `tls*` settings](#pip-337-removal-impact)).
