...
The same configurability is what makes **FIPS-compliant TLS transport** practical. Deployments under FedRAMP, DoD, PCI, or healthcare regimes must terminate TLS through a **FIPS-140-validated cryptographic provider** (e.g. BC-FIPS) running on a non-native engine — never through a non-validated native library. Pulsar's TLS configuration must therefore let an operator (a) select the TLS **engine** (JDK, not native OpenSSL/BoringSSL) per component and (b) pin the **JCA security provider** that backs it. This PIP's TLS SPI delivers both: the engine is already selectable and wired through every server component and the client, and `TlsPolicy.jcaProvider` (a goal of this PIP — see [Goals](#in-scope)) pins the validated provider. The broader FIPS-compliance concerns beyond the TLS transport — approved algorithms in message encryption and authentication, FIPS distribution packaging, and a fail-fast FIPS-mode validation switch — are a separate effort (see Out of Scope); this PIP covers the TLS-transport slice.
...
public final class TlsPolicy {
    ...
    public String jcaProvider();        // optional: names a java.security.Provider (e.g. a FIPS /
                                        // BouncyCastle / PKCS#11 crypto provider). When set, the default
                                        // FileBasedTlsFactory builds the JDK Netty engine with this
                                        // provider (SslProvider.JDK + SslContextBuilder.sslContextProvider),
                                        // taking precedence over the factory-level OpenSSL/JDK engine choice.
...
**`jcaProvider` — pinning a JCA crypto provider.** `TlsPolicy.jcaProvider` names a `java.security.Provider` (matched by `Provider.getName()`) to back the TLS engine's cryptographic operations — the field FIPS, BouncyCastle, and PKCS#11 deployments need. It is resolved via the **`ServiceLoader`** mechanism using the application class loader (`META-INF/services/java.security.Provider`), falling back to an already-registered provider (`Security.getProvider(name)`), and failing loudly if the name resolves to nothing. When `jcaProvider` is set, the default `FileBasedTlsFactory` builds the **JDK** Netty engine (`io.netty.handler.ssl.SslProvider#JDK`) with that provider installed as `SslContextBuilder#sslContextProvider(...)`, **taking precedence over** the factory-level engine selection — a `jcaProvider` value therefore always pins the JDK engine and never the native OpenSSL one, because Netty's OpenSSL engine cannot delegate to an arbitrary JCA provider. `jcaProvider` is a **value-level, file-based-simple** field precisely because a deployment's crypto provider travels with its material and security posture; this is a *different axis* from the raw JDK-vs-OpenSSL **engine** choice, which stays a factory concern and is deliberately not a `TlsPolicy` field (see the [engine-selection note](#pip-337-removal-impact)). Note that `ServiceLoader` resolution against the application class loader is distinct from `Security.getProvider`, which only finds a provider already statically registered in the JVM. A **custom** `PulsarTlsFactory` may interpret `jcaProvider` however it likes — or ignore it entirely; the field carries this defined meaning only for the default file-based path, which always sources its material from the file-based keys.
...
These keys are the successors of PIP-337's `sslFactoryPlugin` / `sslFactoryPluginParams` / `brokerClientSslFactoryPlugin` / `brokerClientSslFactoryPluginParams`, which are **removed outright** in 5.0 — not retained as deprecated fields; a stale value left in a config file is rejected loudly at startup by the removed-key validation. The disposition of every removed key is inventoried under [PIP-337 removal impact](#pip-337-removal-impact).

**v5 client builder.** Gains `tlsFactory(PulsarTlsFactory)` to supply a custom factory instance (adopted — closed with the client), and `tlsPolicy(TlsPolicy)` / `tlsPolicy(TlsPurpose, TlsPolicy)` to configure one or more `TlsPolicy` values — each covering cert/key/trust paths or a keystore, ciphers/protocols, an optional JCA crypto provider (`jcaProvider`), and hostname-verification / insecure-connection flags — bound to a `TlsPurpose` (the single-argument form binds to `CLIENT_DEFAULT`, the purpose all Pulsar-cluster client traffic resolves against directly; an unconfigured client purpose resolves terminally to the system default). The simple file-based case is a single builder-level value, subsuming the PIP-466-era experimental `TlsPolicy` (see the Detailed Design).
...
**JCA crypto provider (`jcaProvider`).** A new `jcaProvider` config key selects a `java.security.Provider` — a FIPS, BouncyCastle, or PKCS#11 crypto provider — to back TLS cryptography. It is a **distinct key on a different axis** from the engine-only `sslProvider` (client) / `tlsProvider` (broker) settings, which stay a JDK-vs-OpenSSL **engine** switch and are unchanged. `jcaProvider` is exposed on the client (`ClientConfigurationData`, beside the existing `sslProvider`) and on the server (`ServiceConfiguration` for the listener/web material, plus its `brokerClient*` counterpart for the broker's outbound clients), and maps directly onto `TlsPolicy.jcaProvider`. Operators set it in `broker.conf` / `client.conf` (the common FIPS/custom-crypto ops need) to pin a specific crypto provider; when set, the default factory builds the **JDK** Netty engine backed by that provider, overriding the engine choice (see [the `jcaProvider` design](#redesigned-pip-337-ssl-provider-pulsartlsfactory)). Resolution uses the `ServiceLoader` mechanism on the **application class loader** (`META-INF/services/java.security.Provider`) — distinct from `Security.getProvider`, which finds only a provider already statically registered in the JVM — with a fallback to an already-registered provider and a loud failure if the name is unresolvable. Existing v4 deployments need no config change: the v4 migration routes a non-engine-literal `sslProvider` value (a JCA provider name) to `jcaProvider` automatically, so a `sslProvider=BCFIPS`-style setting keeps working — the engine literals (`OPENSSL` / `OPENSSL_REFCNT`) stay on the engine axis (see [v4 client `tls*` settings](#pip-337-removal-impact)).
...
| `ClientConfigurationData.sslFactoryPlugin` / `sslFactoryPluginParams` | v4 client config | **Removed**; replaced by new `tlsFactoryClassName` / `tlsFactoryConfig` string fields (today the v4 config carries only a *transient* factory instance set via the v5 builder, no by-name path). A stale key in a `loadConf` map is rejected by the removed-key validation |
...
**v4 client `tls*` settings.** The ~15 `tls*` fields on `ClientConfigurationData` (`tlsKeyFilePath`, `tlsCertificateFilePath`, `tlsTrustCertsFilePath`, the `tlsKeyStore*` / `tlsTrustStore*` family, `tlsCiphers`, `tlsProtocols`, `tlsAllowInsecureConnection`, `tlsHostnameVerificationEnable`, `useKeyStoreTls`) all continue to work: the internal migration maps them onto a `TlsPolicy` bound to `CLIENT_DEFAULT`. The v4 `tlsKeyStoreType` and `tlsTrustStoreType` are mapped onto **separate** `TlsPolicy.keyStoreType()` / `trustStoreType()` fields (not collapsed into one), preserving v4 parity for a mixed setup such as a PKCS12 keystore with a JKS truststore; the same holds for the server-side `brokerClientTls{Key,Trust}StoreType` mapping. `sslProvider` (JDK vs OpenSSL engine selection) is deliberately **not** a `TlsPolicy` field — the engine is a factory concern, configured on the default `FileBasedTlsFactory` and mapped from the v4 field by the migration. That mapping splits the v4 `sslProvider` value along **two axes**. The `OPENSSL` / `OPENSSL_REFCNT` literals select the native Netty **engine** (mapped to `FileBasedTlsFactorySettings.engineProvider` — a factory concern, unchanged). Any other value is treated as a **JCA provider name** (`Conscrypt`, `SunJSSE`, `BCFIPS`, … — the values 4.x accepted) and is routed to **`TlsPolicy.jcaProvider`**, which pins the JDK engine backed by that `java.security.Provider` (resolved via `ServiceLoader`). The named provider is therefore honored on the binary/client path, **restoring the v4 behavior** rather than silently dropping it on upgrade. `jcaProvider` is a first-class `TlsPolicy` field on the *crypto-provider* axis — distinct from, and orthogonal to, the *engine* axis; it is **not** deferred to `FileBasedTlsFactorySettings`. The one residual delta is fail-fast: a provider name that resolves to no installed `java.security.Provider` now fails loudly at client build / server start, where 4.x silently fell back to the JDK default. Operators who prefer to set the crypto provider explicitly use the dedicated `jcaProvider` config key (see [Configuration](#configuration)) — it is the same field (see [the `jcaProvider` design](#redesigned-pip-337-ssl-provider-pulsartlsfactory)).
...
**Designed to hold for five-plus years.** These SPIs freeze public coordinates the moment 5.0 ships, so they were reviewed against that horizon, not just against 5.0's needs. Four decisions carry the weight: **`capability(Class)` is the sole discovery mechanism** — a plugin's abilities are found by asking for a capability interface, never by `instanceof`, so new capabilities are added without touching the framework's dispatch or any existing plugin; the **three-SPI module split** (`pulsar-client-api-v5` → `pulsar-http-client-api` → `pulsar-tls-factory-api`) fixes permanent coordinates with the dependency arrows pointing one way only (broker → http-api → tls-api), so a consumer of one SPI never drags in another's dependencies; **`TlsPurpose` is a flat `(role, name)` key**, a stable configuration coordinate rather than a type hierarchy or a dynamic value; and TLS material **rotates by rebuild-not-mutate**, so a rotated context is an atomically built new object, never a mutated live one. A dedicated three-model review (Fable 5, Opus 4.8, Codex GPT-5.5) independently concurred that all four hold for five-plus years as-is, and that every remaining extension point — a second HTTP challenge style, per-destination TLS material, a richer multi-valued header type, a material-carrying auth factory, backend pluggability — has a clean **additive** growth path that breaks nothing already shipped. That same review produced the subtractions recorded under [Minimalism review (Pulsar 5.0)](#minimalism-review-pulsar-50); the two are one idea seen from both sides — *"Perfection is achieved, not when there is nothing more to add, but when there is nothing left to take away"* (Antoine de Saint-Exupéry) — a surface small enough to defend, with a clear additive path for everything deferred.
...
This is the **one element the [minimalism review](#minimalism-review-pulsar-50) flagged as not yet earning its place**: all three reviews independently recommended cutting `SslContextFactory.Client` back out of the well-known set, on the grounds that it is symmetry-only, no in-tree factory supplies it, and framework synthesis already serves its sole consumer. It is retained here as a deliberate design choice — for the symmetry and the stated proxy→broker admin-TLS customization use case — and the decision is recorded as *open*: a clean, purely additive removal remains available if the symmetry is later judged not worth the extra obligations paragraph. Keeping it does not foreclose that; removing it later would not break any factory that relied on framework synthesis.
...
## Minimalism review (Pulsar 5.0)

Before the vote the whole SPI surface was put through a dedicated minimalism / five-year-durability pass: a three-model panel (Fable 5, Opus 4.8, Codex GPT-5.5), reading with simplicity and durability as the primary lens. The four major interface decisions were judged durable as-is (see the durability statement in the [preamble above](#resolved-design-decisions)) and the core type set was already near-minimal, so the pass produced a *small* set of subtractions rather than a redesign. Five elements were cut and three were weighed and deliberately kept; a bracketed `[n/3]` records how many of the three reviewers converged on a cut. Each cut has a clean **additive** add-back path, so none of them forecloses a future need — a subtraction that could only be undone by a breaking change would not have been made this late.

**Guiding principle.** *"Perfection is achieved, not when there is nothing more to add, but when there is nothing left to take away."* (Antoine de Saint-Exupéry.) A public SPI element is paid for on every future release, in documentation, tests, and the constraint it places on evolution. An element with no in-tree consumer, or a second way to express something the design already expresses once, is a cost without a return — and the cheapest time to remove it is before 5.0 freezes the surface.

### Cuts

**A — `AuthenticationFactory.tls(cert, key)` → no-arg `tls()` marker.** `[3/3]` Removed the material-carrying `tls(certFilePath, keyFilePath)` overload; kept the no-arg `tls()` that returns the built-in `"tls"` marker plugin. *Why:* it was a second material-carrying authentication path, re-opening the mTLS-material-in-an-auth-entry-point shape this PIP otherwise removes — mTLS material already has exactly one home, `tlsPolicy(TlsPolicy.pem(...))` — and it had no production caller (only a producer/consumer test used it). *Add-back (additive):* a material-carrying convenience factory method can be reintroduced later without disturbing the no-arg form. *Tradeoff:* a one-liner source-compatible with the v4 `AuthenticationFactory.TLS(cert, key)` idiom is no longer offered; a user migrating hand-written v4 code writes `tlsPolicy(...)` plus `tls()` instead. Accepted — v5 is a new API surface, and a *legacy* `AuthenticationTls` is still folded automatically by the retained v4→v5 bridge.

**B — `AuthenticationInitContext.params()`.** `[2/3]` Removed the `Map<String,String> params()` accessor and unthreaded the now-dead field/constructor-arg plumbing that fed only it. *Why:* redundant — a plugin already receives its configured params through `Authentication.configure(Map)` before `initializeAsync`, on the string-based (reflective) path; the programmatic path never had params. It had zero readers and the threaded value was always an empty map. *Add-back (additive):* a `params()` accessor can be re-added if a plugin ever genuinely needs its params at init time rather than caching them from `configure`. *Tradeoff:* negligible — `configure` runs first, so a plugin that wants params at init caches them itself. (With `params()` gone the context is member-identical to the internal `ClientAuthenticationServices`, but collapsing the two was **deferred**, not done: they differ in nullness contract — services may return null clock/otel, the plugin-facing context applies non-null defaults — and in visibility role, so the merge is a real design call, not a mechanical one.)

**C — `ProxyConfig` + `PulsarHttpClientConfig.proxy()`.** `[2/3]` Removed the `ProxyConfig` value type, the `proxy()` accessor, and the builder's `proxy(ProxyConfig)` setter. *Why:* a fail-loud violation — a shipped-but-silently-ignored knob. The framework wires SOCKS5 proxying from the client configuration (`ClientConfigurationData.getSocks5ProxyAddress`/`Scope`/`Username`/`Password`), never from `PulsarHttpClientConfig.proxy()`, so the field had no reader and was silently dropped; presenting a config setting that does nothing misleads operators. *Add-back (additive):* a per-client-config proxy can be wired if a concrete per-instance need emerges; client-wide SOCKS5 proxying is unaffected today. *Tradeoff:* no per-HTTP-instance proxy override — none is needed, since one client-wide setting covers the observed use.

**D — `TlsPurpose` fallback chains.** `[2/3]` Removed the `fallback()` accessor, the fallback field, and the `(name, fallback)` minting overloads (`client(String, TlsPurpose)` / `server(String, TlsPurpose)`); `TlsPurpose` is now a flat `(role, name)` key with a single terminal resolution step. *Why:* no production code used the fallback API — only a unit test and `FileBasedTlsFactory.resolve()` referenced it — and it created an identity wart: `equals`/`hashCode` had to *deliberately exclude* `fallback` so that `client("x")` and `client("x", CLIENT_DEFAULT)` addressed the same purpose→policy slot. The terminal rule (unconfigured client → system default; unconfigured server → configuration error) already captured every real need; the intermediate chain walk was dead code, and flat identity is plainly `(role, name)`. *Add-back (additive):* per-purpose fallback can be reintroduced if multi-level resolution is ever needed — the terminal rule is its degenerate single-level case. *Tradeoff:* a minted finer purpose no longer inherits another purpose's material through a link; a deployment that wants a finer client purpose to share cluster material seeds it at composition time instead. Accepted — the in-tree consumers use `CLIENT_DEFAULT` and the server purposes directly.

**F — `TlsAuthentication(String authMethodName)` constructor.** Removed the custom-method-name constructor and its mutable field; kept the no-arg constructor, and `authMethodName()` returns the `DEFAULT_AUTH_METHOD_NAME` (`"tls"`) constant. *Why:* the plugin always advertises the `"tls"` method — its only production caller (the legacy v4 TLS adapter) passed the default `"tls"` — so the custom-name option was unused surface plus a needless mutable field. *Add-back (additive):* a custom-method-name TLS plugin constructor can be re-added if one is ever needed. *Tradeoff:* none in practice — no in-tree plugin advertised a non-`"tls"` method through this class.

The following two were found by a second-pass (fixpoint) review that grepped production *call sites* rather than declarations:

**G — `AuthenticationCallContext.brokerPort()`.** Removed the accessor; `AuthenticationCallContext` is now host-only (`brokerHost()`). *Why:* it had **no readers** — every consumer uses `brokerHost()` only — **and** it was always fabricated as `0`, because the async exchange seam is host-only (`AsyncAuthenticationDriver.newAuthenticationExchange(String brokerHostName)`), so both production paths constructed the context with port `0`. A shipped accessor that always returns a constant is worse than absent: a plugin keying per-endpoint credentials on `host:port` would silently get `0` on every exchange. This is the same defect cut C removed (a shipped-but-misleading knob), and the exact inverse of the `TlsEndpoint` keep — `TlsEndpoint` stays because its plumbing carries *real* values, `brokerPort()` went because its didn't. The v4 precedent (`getAuthData(String)`) is host-only too. *Add-back (additive):* if per-destination binary authentication is ever needed, the port re-enters through the exchange **seam** (`newAuthenticationExchange`), which then populates the context — not through a field that lies until then. *Tradeoff:* none — nothing could have relied on a constant `0`.

**H — `HttpRequest.Form`.** Removed the `Form` variant from the sealed `Body`; the body slot is now the single member `Bytes` (raw content + `Content-Type`). *Why:* `Form` had **no in-tree producer** — the one production body writer, OAuth2's `TokenClient`, hand-encodes its `application/x-www-form-urlencoded` body into `Bytes`, and `Form` appeared only in the framework's decode branch. It shipped producer-unvalidated. The natural fix would have been to *exercise* it (make `TokenClient` produce `Form`), but that could not preserve wire behavior: verified against the AsyncHttpClient encoder, `Form`'s escaping and `Content-Type` match `TokenClient`'s, but `Form`'s field map is unordered (`Map.copyOf`) while `TokenClient` emits its parameters in a deterministic sorted order — so producing `Form` would reorder the token-request bytes on the wire. Rather than change a public value type's copy semantics or risk the OAuth2 wire format, the speculative member was cut; `Bytes` already carries form bodies (as `TokenClient` does today), so nothing on the wire changed. *Add-back (additive):* a `Form` (or streaming, or structured) body is a new permitted subtype of the still-sealed `Body` — the framework's single body-encoding site (an `instanceof` dispatch, backend non-pluggable) is internal, so adding one breaks no consumer. `Body` is kept as a single-member sealed interface (rather than flattening `HttpRequest` to take `Bytes` directly) precisely to keep that additive extension point open — flattening would turn that future body kind into a breaking change to the public `Optional<Body> body()` accessor — and it is deliberately **not** collapsed to a bare `byte[]`, which would lose the `Content-Type`. *Tradeoff:* a caller constructing a form body writes the `application/x-www-form-urlencoded` bytes itself, exactly as the one in-tree producer already does.

A third fixpoint pass drew the distinguishing line for HTTP request/config surface — **cut what carries contract weight (an interaction rule, a merge precedence, or a duplicate path) with no consumer; keep zero-rule projections that serve third-party plugin authors** — and removed three more paths:

**I — `PulsarHttpClientConfig.defaultHeaders()` / `Builder.defaultHeader(name, value)`.** Removed the config-level default-headers path (accessor, builder method, backing field, and the framework's apply loop). *Why:* it had **no producer** anywhere (main or test) yet shipped an untested interaction contract — a "config defaults first, per-request headers win" merge precedence — and it duplicated two things the design already expresses once: per-request `HttpRequest.Builder.header()` (what the in-tree producers actually use) and the `userAgent` field (the one real default header). *Add-back (additive):* a config-level default-headers path can return if a concrete need appears. *Tradeoff:* a plugin wanting a static header on every request sets it per request — one line, and the only pattern in tree.

**J — `HttpRequest.Builder.headers(Map<String, String>)`.** Removed the bulk header setter; per-entry `header(name, value)` remains. *Why:* zero callers anywhere — every producer, even the one that iterates a map (the SASL HTTP driver), calls `header(k, v)` per entry — so it was purely a second way to express what `header()` already does. *Add-back (additive):* trivial. *Tradeoff:* none observed.

**K — `HttpRequest` per-request timeout override (`Builder.timeout(Duration)` / `timeout()`).** Removed the per-request timeout; the per-instance `PulsarHttpClientConfig.requestTimeout` is the sole *request* timeout knob (the connect and read timeouts remain as their own per-instance settings). *Why:* no producer set a per-request timeout (main or test); the per-instance timeout covers every observed need. Unlike C/G it was honored (not a lie), so this is the no-consumer-plus-duplicate-path argument alone. *Add-back (additive):* a per-request deadline returns as an `Optional` builder field if a one-off-deadline flow (e.g. OAuth2 device-grant polling) ever needs it on a shared client. *Tradeoff:* until then, a one-off deadline would require a separate client instance — acceptable given no in-tree need.

The kept `maxResponseBodyBytes` knob was **deliberately not** cut alongside these, though it too is unset in tree: it is categorically different — a **safety guard**, not a duplicate path. The 16 MiB buffered-response cap is enforced unconditionally (the framework fails the future rather than truncating), so removing the knob would turn the cap into a hard wall a plugin with a legitimately larger response could not raise — a trap, the inverse of fail-loud — and unlike the three above it has an in-tree test producer. A safety guard and its escape valve travel together.

### Kept (weighed and held under the same lens)

**`SSLParameters` companion — kept.** It is one more optional well-known class, and the question was whether a bare `SSLContext` could stand in. It cannot: `SSLParameters` is **load-bearing for the zero-non-JDK-dependency tier-3 factory**. An HSM/KMS factory that supplies only an `SSLContext` needs it to express the engine-level baseline it cannot otherwise state — the protocol floor, cipher suites, client-auth mode, and endpoint-identification algorithm — because the JDK exposes `SSLContext.getDefaultSSLParameters()` but **no setter** for a context's default parameters. Without the companion, an `SSLContext`-only factory could not restrict protocols or ciphers or declare its verification / client-auth policy, and the "implement one `SSLContext`, at zero non-JDK dependencies" tier-3 pitch collapses. *Tradeoff acknowledged:* the deterministic four-rule merge spec is the price (factory baseline forms the engine floor; per-connection SNI always wins over any factory baseline; endpoint identification is factory-wins-else-consumer; a server-side factory's `SSLParameters` is authoritative for client-auth). But that logic lives in the framework once, and the rules are irreducible for correctness — per-connection SNI and authoritative server-side client-auth are not simplifiable away.

**`SinglePassAuthentication` — kept.** It is a convenience composite over the four capability interfaces, and a plugin could instead implement the capability interfaces directly. It stays because it is the **tier-1 "implement one interface" pitch** and it has **four in-tree implementors** — `TokenAuthenticationV5`, `BasicAuthenticationV5`, `OAuth2AuthenticationV5`, and `AthenzAuthenticationV5`. A single-pass plugin implements one interface instead of choosing the right pair of the four capabilities. *Tradeoff:* one more public type, justified by four consumers and the reduced-friction common case.

**`TlsEndpoint` + the endpoint `createInstance` overload — kept.** It is **inert for the default factory today** — `FileBasedTlsFactory` ignores the endpoint — which invites the charge of speculation. It stays because it is **already plumbed** (the client passes the SNI host/port through `PulsarChannelInitializer`) and is an **ignorable default method** (`createInstance(purpose, endpoint, class)` delegates to the endpoint-less form), so it costs simple factories nothing. It is cheap pre-wiring for a future per-destination / SPIFFE / workload-identity factory; retrofitting a destination parameter *after* release would ripple through every factory signature — a change keeping it avoids. *Tradeoff:* it ships as inert surface on the default path, but re-plumbing the per-request destination later would be both real work and a signature change, whereas keeping it is one default method nobody has to notice. It remains a *hint* only — it never replaces hostname verification or SNI, which are applied at engine creation from the peer address.

### One kept element the review flagged as not yet earning its place

`SslContextFactory.Client`'s membership in the well-known class set is the single element all three reviews independently recommended cutting (symmetry-only; no in-tree factory supplies it; framework synthesis already serves its sole consumer, the proxy's `AdminProxyHandler`). It is retained as a deliberate design choice for the symmetry and the proxy→broker admin-TLS customization use case, and the decision is recorded as **open** — a clean additive removal remains available. The full tension, and why it was kept, is recorded under [Resolved decision 4](#4-strictness-of-the-well-known-class-contract).
...
- [GitHub PR 25890 (tracking implementation)](https://github.com/apache/pulsar/pull/25890) — note: this tracking PR reflects an **earlier** iteration of the design; its title and description predate the current proposal and will be updated (with a "design evolved" note) before the vote.
