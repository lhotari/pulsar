...
- **`TlsPurpose`** (a value type, in `org.apache.pulsar.common.tls`) — identifies *why* TLS is requested and in what role. It is a **simple named key**, not a type hierarchy: a role (client or server), a well-known name, and an optional qualifier. Well-known purposes are exposed as constants; a factory serves distinct material per purpose, and any purpose with nothing configured falls back to the role's default (single level). This deliberately drops the earlier draft's sealed `Client`/`Server` split, its `UsageIdentifier(Class<?>, String)` addressing dimension, its structured `host()` (SNI cert-selection is unused — Pulsar sets `setSniRequired(false)`), and its multi-hop enum fallback *chain*.
  ...
  public final class TlsPurpose {
      ...
      public Role role();
      /** Well-known name, e.g. "default", "oauth2", "broker", "proxy", "web". */
      public String name();
      /** Optional finer key within a purpose, e.g. a server advertised-listener name. */
      public Optional<String> qualifier();
      /** Material used when none is configured for this purpose; empty means the client
       *  system default (OS trust store, no client certificate) or, on the server, a
       *  configuration error. */
      public Optional<TlsPurpose> fallback();

      /** A qualified variant, e.g. BROKER.withQualifier("internal"); falls back to this purpose. */
      public TlsPurpose withQualifier(String qualifier);
      ...
      // Well-known SERVER purposes (qualifier = advertised-listener name when listener-specific)
  ...
  Users configuring the client only ever reference client purposes, and in practice only two buckets matter: `CLIENT_DEFAULT` (all Pulsar-cluster traffic — binary protocol, HTTP topic lookup, and the admin client all resolve here) and `CLIENT_OAUTH2` for OAuth2/IdP calls, which fall back to the **system default** (OS trust store, no client certificate) rather than to the cluster material, since the IdP is a different trust domain. The framework may internally request finer client purposes (a dedicated binary/lookup/admin purpose), but they resolve to `CLIENT_DEFAULT` unless a deployment explicitly configures one differently.

  On the server side, `BROKER` / `PROXY` / `WEB` cover the binary listeners and the web server; a `qualifier` distinguishes per-listener material when advertised listeners differ — e.g. `BROKER.withQualifier("internal")` for in-cluster broker-to-broker traffic (possibly self-signed / mTLS) versus `BROKER.withQualifier("external")` for a publicly-trusted CA without mTLS. Unconfigured server purposes fall back to the broker's configured default server material. Replication and other broker-to-broker purposes are added only when a concrete need appears; broker-side advertised-listener-aware authentication/authorization is a separate future PIP.
...
**Default file-based factory.** `FileBasedTlsFactory` implements `PulsarTlsFactory`, supporting all well-known instance classes natively — Netty contexts on the configured engine (JDK, or OpenSSL-based where available). It is configured with a `TlsPolicy` per `TlsPurpose` (see below), and internally turns each policy into a **`TlsMaterialSource`** — a package-internal object that owns the runtime behavior: it polls its backing files/keystore for changes, caches the loaded material while the source is unchanged, and notifies subscribers on change. The factory itself owns the purpose→source registry, the purpose matching and single-level fallback, and the reload fan-out to subscribers; `TlsMaterialSource` owns only the load/watch/cache of one material set. This is a clean split — `TlsPolicy` is the declarative *what* (user-facing), `TlsMaterialSource` is the runtime *how* (internal).

Beyond the policies supplied at construction, the factory supports **dynamic configuration** per purpose afterwards:

```java
/** Replace the policy configured for a purpose. */
void setPolicy(TlsPurpose purpose, TlsPolicy policy);

/** Add material for a purpose without discarding what is already configured for it
 *  (used by the v4 AuthenticationTls bridge, which augments a user-configured policy). */
void addPolicy(TlsPurpose purpose, TlsPolicy policy);
```

`setPolicy` replaces outright; `addPolicy` augments, so a later addition takes precedence for overlapping material without discarding a user-configured policy. The augment path exists for v4 API compatibility, where some TLS configuration comes from the client builder and some from a legacy `AuthenticationTls` plugin. Configuration is thread-safe (copy-on-write) relative to in-flight `createInstance` calls. These methods belong to the default `FileBasedTlsFactory`; a user who installs a fully custom `PulsarTlsFactory` configures its TLS material directly, and the v4 bridge then has nothing to add.

The v4 `AuthenticationTls` / `AuthenticationKeyStoreTls` bridge uses `addPolicy` to install a plugin's TLS material for the relevant client purposes without replacing a user-configured policy (in-scope item #7 and the [TLS override hook](#legacyv4authenticationadapter-v4--v5)). The broker's default `DefaultBrokerTlsFactory` is a thin `FileBasedTlsFactory` wrapper configured from the existing `ServiceConfiguration` properties.

**Client configuration: the `TlsPolicy` value.** The default client-side `PulsarTlsFactory` is wired from the client builder, and **`TlsPolicy` is the single TLS type a user ever touches** — a flat, immutable value describing *what* material to use and the policy flags. It subsumes both the experimental PIP-466 `TlsPolicy` (which was PEM-only) and the internal material-source machinery: the builder turns each `TlsPolicy` into an internal `TlsMaterialSource` and configures the `FileBasedTlsFactory` for the relevant purpose(s).
...
public final class TlsPolicy {
    ...
    public String storeType();          // JKS / PKCS12
...
- `org.apache.pulsar.common.tls` — `PulsarTlsFactory` with the handles `TlsInstance` / `TlsSubscription` and `TlsFactoryInitContext`; the `TlsPurpose` named-key value type; the `TlsContexts` build-support helper; `FileBasedTlsFactory`, `DefaultBrokerTlsFactory`, and the internal `TlsMaterialSource`. (The well-known instance classes include `io.netty.handler.ssl.SslContext` and Jetty's `SslContextFactory.Server`; custom factories targeting the shaded client must publish a relocated artifact — see the shading note in the Detailed Design.)
- `org.apache.pulsar.client.api.v5.config` — the flat `TlsPolicy` value (PEM and keystore, via a `format` discriminator), the single user-facing TLS type on the client builder.
...
## 2. Config-file (de)serialization of the v5 client configuration (future)

**Issue.** Not every deployment can build the client programmatically; some keep configuration in a properties or JSON file. The v5 client (PIP-466) has no config-file (de)serialization mechanism yet, and TLS is the most structured part of that configuration. Should the v5 client be able to load (and perhaps save) its configuration — including TLS — from a file, and does the design here accommodate that?

**Context.** This PIP does **not** build such a loader (out of scope), but it deliberately shapes the TLS types so one is additive rather than a rework. `TlsPolicy` is a **flat value with a `format` discriminator** (not a polymorphic hierarchy), and `TlsPurpose` is a **plain named key** — both map cleanly to flat config keys (e.g. `tls.default.trustCertsFilePath=…`, `tls.oauth2.trustStorePath=…`) with no polymorphic type-tag handling. The settled decision that HTTP-client plugins select TLS material **by purpose key** (Detailed Design) rather than inline material also keeps credentials out of code paths a config file could not express. What remains open is whether and when to add the loader, and which format(s) to support.

**Alternatives.**
- (a) *(suggested)* Defer to a follow-up; keep the flat, config-friendly `TlsPolicy` / `TlsPurpose` shapes so the loader can be added without reshaping the API.
- (b) Define the config-file schema and a properties/JSON binding as part of this work, accepting the extra scope now.
- (c) Support programmatic configuration only and document that file-based client configuration is not a goal for the v5 client.
