...
FIPS 140-3 also governs how a **Sensitive Security Parameter** (here, a TLS private key) may cross the cryptographic module's boundary, and the requirement differs by assurance level. A **Level 1** software module such as BC-FIPS permits plaintext key entry through its API from within the same operational environment, so Pulsar parsing a PEM private key and handing it to the provider is compliant — the file-based `TlsPolicy` (with `jcaProvider` naming the validated provider) serves this case directly. Higher assurance levels require encrypted key entry or a trusted channel: a **Level 3** deployment keeps keys inside a separate hardware module (a PKCS#11 HSM) whose keys never leave its boundary, so the private key is never handed to Pulsar at all — it is referenced by handle. That is the HSM / never-in-files case above, served by a custom `PulsarTlsFactory` (or a PKCS#11 `jcaProvider`) that builds its TLS context against the HSM. The decoupled SPI serves both levels without special-casing either.
