...
**K — `HttpRequest` per-request timeout override (`Builder.timeout(Duration)` / `timeout()`).** Removed the per-request timeout; the per-instance `PulsarHttpClientConfig.requestTimeout` is the sole timeout knob. *Why:* no producer set a per-request timeout (main or test); the per-instance timeout covers every observed need. Unlike C/G it was honored (not a lie), so this is the no-consumer-plus-duplicate-path argument alone. *Add-back (additive):* a per-request deadline returns as an `Optional` builder field if a one-off-deadline flow (e.g. OAuth2 device-grant polling) ever needs it on a shared client. *Tradeoff:* until then, a one-off deadline would require a separate client instance — acceptable given no in-tree need.
