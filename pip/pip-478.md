...
The `.internal.` subpackage signals "stable internal — application code should not implement this." `ClientCnx.newConnectCommand()` and `ClientCnx.handleAuthChallenge(...)` check `authentication instanceof AsyncAuthenticationDriver async` and route through the async API when present. **Generic across all challenge types** — connect, REFRESH, SASL multi-round, custom challenge-response. The marker is the only carve-out from the otherwise-untouched v4 client.
