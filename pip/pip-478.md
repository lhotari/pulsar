...
- `org.apache.pulsar.common.tls` (also in `pulsar-common-api`) — `PulsarTlsFactory` with the `TlsHandle` handle and `TlsFactoryInitContext`; the `TlsPurpose` named-key value type and the `TlsEndpoint` destination hint; the flat `TlsPolicy` value (PEM and keystore, via a `format` discriminator — the single user-facing TLS type, consumed by the client builder and the server components alike); the `TlsContexts` build-support helper; `FileBasedTlsFactory`, `DefaultBrokerTlsFactory`, and the internal `TlsMaterialSource`. (The well-known instance classes include `io.netty.handler.ssl.SslContext` and Jetty's `SslContextFactory.Server` — those remain `compileOnly`-style optional references for factories; custom factories targeting the shaded client must publish a relocated artifact, see the shading note in the Detailed Design.)
