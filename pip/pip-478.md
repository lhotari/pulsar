...
**v4 client `tls*` settings.** The ~15 `tls*` fields on `ClientConfigurationData` (`tlsKeyFilePath`, `tlsCertificateFilePath`, `tlsTrustCertsFilePath`, the `tlsKeyStore*` / `tlsTrustStore*` family, `tlsCiphers`, `tlsProtocols`, `tlsAllowInsecureConnection`, `tlsHostnameVerificationEnable`, `useKeyStoreTls`) all continue to work: the internal migration maps them onto a `TlsPolicy` bound to `CLIENT_DEFAULT`. The v4 `tlsKeyStoreType` and `tlsTrustStoreType` are mapped onto **separate** `TlsPolicy.keyStoreType()` / `trustStoreType()` fields (not collapsed into one), preserving v4 parity for a mixed setup such as a PKCS12 keystore with a JKS truststore; the same holds for the server-side `brokerClientTls{Key,Trust}StoreType` mapping. `sslProvider` (JDK vs OpenSSL engine selection) is deliberately **not** a `TlsPolicy` field — the engine is a factory concern, configured on the default `FileBasedTlsFactory` and mapped from the v4 field by the migration. That mapping recognizes only the `OPENSSL` / `OPENSSL_REFCNT` literals as selecting the native Netty engine; any other value — including a JCE *provider name* such as `Conscrypt` or `SunJSSE`, which the old code accepted — maps to the JDK engine. A v4 deployment that set `sslProvider` to a JCA provider name therefore silently gets the default JDK engine on the binary/client path after upgrade (the web/Jetty path still receives the string as its JCA provider). This is a niche behavior delta, not a security regression. Provider-name **engine** selection is not resurrected — but selecting a specific JCE *crypto* provider (the real intent behind a `sslProvider=Conscrypt`-style value) is now first-class through a **separate field on a different axis**: `TlsPolicy.jcaProvider` (and its matching client/server config key — see [Configuration](#configuration)) names a `java.security.Provider`, resolved via `ServiceLoader`, and pins the JDK engine backed by it. That axis is orthogonal to `sslProvider`'s JDK-vs-OpenSSL **engine** choice: the engine choice stays a factory concern (`FileBasedTlsFactorySettings.engineProvider`, mapped from `sslProvider`), while the crypto-provider choice travels **on `TlsPolicy`** with the material. A v4 deployment that used a JCA provider name for crypto-provider selection migrates that intent to `jcaProvider` (see [the `jcaProvider` design](#redesigned-pip-337-ssl-provider-pulsartlsfactory)).
...
- **Security-relevant behavior changes (5.0), surfaced here so they aren't buried in the removal inventory:**
  - **`sslProvider` silent JDK downgrade.** A v4 deployment that set `sslProvider` to a JCE *provider name* (e.g. `Conscrypt`, `SunJSSE`) — which the old code accepted — now silently gets the default JDK Netty engine on the binary/client path (only the `OPENSSL` / `OPENSSL_REFCNT` literals select the native engine); the web/Jetty path still receives the string as its JCA provider. A niche behavior delta, not a security regression (see [v4 client `tls*` settings](#pip-337-removal-impact)).
...
- Existing applications using `pulsar-client-api` (v4) need **no source changes** — except the rare application that called the removed PIP-337 `sslFactoryPlugin(...)` / `sslFactoryPluginParams(...)` builder methods, which no longer compile and must move to `tlsFactoryClassName(...)` / `tlsFactoryConfig(...)` (see below) — but 5.0 carries several **runtime behavior deltas** they should review before upgrading:
  ...
  - **`sslProvider` mapping narrows**: only the `OPENSSL` / `OPENSSL_REFCNT` literals select the native Netty engine; a JCE *provider name* (e.g. `Conscrypt`, `SunJSSE`) that the old code accepted now maps to the JDK engine on the binary/client path (see [v4 client `tls*` settings](#pip-337-removal-impact)).
