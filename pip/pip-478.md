...
- **`PulsarTlsFactory`** (interface, in `org.apache.pulsar.common.tls`) — the v5 TLS SPI. The framework requests an instance of a well-known class for a `TlsPurpose` (described next); the factory returns a configured instance when it supports the combination, or `Optional.empty()` when it does not:
  ...
  `Optional.empty()` from `createInstance` means exactly one thing: the factory does not support the requested (purpose, class) combination. It is **not** a purpose-resolution signal — how a factory maps a purpose to configured material is factory-internal. The default `FileBasedTlsFactory` resolves the requested purpose first, then its single-level fallback (see `TlsPurpose`), and fails the request when nothing is configured along that path.
...
**Issue.** The design makes `javax.net.ssl.SSLContext` required only as a *fallback* — a factory must supply it for a purpose only when it does not natively build the richer Netty/Jetty objects that purpose needs, and the framework synthesizes those from it on `Optional.empty()`. The alternative is a strict contract where every well-known class is mandatory for every factory. Smaller details are also open: the exact method/handle names (`createInstance` overloads vs a separate `subscribe` method; `TlsInstance`/`TlsSubscription`).
...
- (c) Naming details (`createInstance`+`Consumer` overload vs `subscribe`; handle names) — to be settled in review before the vote.
