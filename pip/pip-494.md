...
`producerScopedPublishThrottlingEnabled` gates this. It is a safety guard with its escape valve: Layer 3 changes who gets served on a loaded broker, with no wire signal and no client-visible error. An operator meeting this at 03:00 needs a config flip, not a rollback. 
...
A fairness policy would first have to choose an **allocation identity**, and that choice is a behaviour contract that cannot be changed after ship: sharing a topic's rate equally *between producers* gives a topic with 100 producers a hundred times the capacity of a topic with one, so producer count is not an entitlement and authentication does not make it one. It would also need weights and their source, a definition of demand the broker can actually observe, a redistribution cadence affordable at thousands of producers per broker, and a threat model for a client that can shape what an estimator sees. None of that is settled here; a follow-up PIP can consume this proposal's per-producer pacing. Nor is a pluggable policy seam the interim answer: PIP-310 (*Support Custom Publish Rate Limiters*) asked the same of the rate limiters and was not adopted, on the grounds that fragmenting the implementation serves users worse than fixing the built-in one. The same holds here — a seam with no consuming twin is contract weight, and one policy Pulsar stands behind is the more durable shape.
