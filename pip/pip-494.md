...
`PublishRateLimiterImpl.handlePublishThrottling` already calls `throttleAction.accept(producer)` with the producer whose publish drained the bucket, and queues that same producer for unthrottling. Today that action pauses the producer's *connection*. After this change it paces the *producer*. That is the whole of Layer 3: the limiter's decision, and the producer it already names, are unchanged — only the action is.
...
`connectionsPerBroker` is client-wide: raising it spreads everything across more connections and cannot isolate one producer. A per-producer isolation API would address the residue and is tracked as [#23403](https://github.com/apache/pulsar/issues/23403). It is **independently shippable and not specified here**, for two reasons: with Layers 2 and 3 in place the broker-side reason for isolation largely goes away, and the client-side residue cannot be sized until they ship; and specifying it first would make it the workaround people build on.
