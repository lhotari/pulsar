...
- **A grant expires** because `releaseUnused()` cannot be the only way capacity returns: a release forgotten on an exception path leaks capacity that never comes back, permanently shrinking the client's budget (the mirror of [#26469](https://github.com/apache/pulsar/issues/26469), an in-tree *double* release). A deadline states the grant's lifetime instead of leaving it implicit; RSocket's `LEASE` frame takes the same position with a mandatory time-to-live, after which the lease's request count is implicitly 0.
...
**Pacing does not tell a client why it is slow.** That is the same defect this document holds against `setAutoRead(false)` in *Motivation*; only an explicit signal would fix it, which is why one is sequenced next rather than never (see *Alternatives*).
