...
**What a queued response costs is exact, not estimated.** A response is serialized when it is queued, and the queue is charged the frame's own length — `Commands.serializeWithSize` (`Commands:2005`) already produces exactly that, and `newSendReceiptCommand` (`:448`) is five identifiers, so a receipt is on the order of tens of bytes. 64 MiB is therefore on the order of a million held responses; against a per-producer bound of 1000, roughly a thousand fully-paced producers before the broker-wide budget binds. That is the quantity an operator is really choosing. None of it is an estimate, which is why this bound carries a figure where the client-side per-send retention (*Layer 1b*) deliberately does not.
