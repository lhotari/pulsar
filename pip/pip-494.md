...
**And who gets it.** Layer 1 is a Java client API. C++, Go, Python and Node producers get Layer 2 without it, as do Java producers that never call `capacity()`. For them the rate decision still lands on the right producer instead of the connection — which is the larger of the two benefits — but it arrives as latency rather than as capacity, exactly as it does today. Layer 1b bounds their window once enabled; until then their pacing authority is whatever `memoryLimit` and `maxPendingMessages` give it. Ports of the grant API are additive and out of scope here.
...
AIMD is a *search* law: it finds a capacity nobody can compute, and it is used that way by central servers with complete local knowledge. Breakwater controls a global credit pool so that measured queueing delay tracks a target derived from the RPC's SLO, increasing it additively and decreasing it "multiplicatively, proportional to the level of overload" — the loop runs centrally, on the server's own measurement, and its decrease is deliberately floored rather than a blind halving. (Breakwater does pursue fairness, but in how it *distributes* credits, not in the control law: it "tries to approximate max-min fairness" only while credits are plentiful, and abandons it when there are fewer credits than clients.) That is when AIMD is right, and it is not this case: every condition Layer 2 acts on already carries a rate an operator configured, so there is no capacity to search for. Discovering an *unconfigured* safe limit is a real problem — Kora reports that the safe broker-wide CPU point is variable and that "there is no easy way to measure and attribute CPU usage to a tenant", so it triggers backpressure on request-queue depth instead — but that is a different proposal with a different signal, and it is out of scope here.
...
- `producerResponsePacingMaxQueuedResponses` — bound on the number of responses held for one producer, escalating the same way. **Default 1000**, matching `maxPendingPublishRequestsPerConnection`, which is the existing bound on the same shape of per-connection accumulation. It bounds the pacing queue's memory, which no existing broker limit covers.

All three defaults are proposals, not derivations: 5000 ms is chosen to leave five sixths of a default `sendTimeoutMs` for batching, transit and persistence, and 1000 to match the neighbouring bound. The `pulsar.broker.producer.response.pacing.age` histogram exists so that operators can replace them with measured values, and so that the project can revise them on evidence rather than argument.
