 ...
 * (PIP-478).
 *
 * <p>The framework owns and closes these shared services (HTTP clients, scheduler); the
 * implementation may retain references for the lifetime of the client and releases its own resources
 * in {@link Authentication#close()}.
 */
public interface AuthenticationInitContext {

    /**
     * Factory for {@link org.apache.pulsar.client.api.v5.http.PulsarHttpClient} instances. The
     ...
     * {@link org.apache.pulsar.client.api.v5.http.PulsarHttpClientConfig} and receive a configured
     * instance owned by the framework. Multiple instances with different TLS / timeouts / proxy may
     * be obtained for different uses (e.g. OAuth2 mTLS to the IdP vs HTTP topic lookup).
     *
    ...
    /**
     * @return an executor for off-loop authentication work; never the Netty event loop
     */
