...
public interface ChallengeResponseHandler {
     ...
     *
     * <p>The framework places the returned bytes in {@code CommandAuthResponse}. Completion is
     * decided by the broker (it replies with {@code CommandConnected} when satisfied), so the handler
     * does not signal it; cross-round conversation state is kept in the context's state slot.
     *
